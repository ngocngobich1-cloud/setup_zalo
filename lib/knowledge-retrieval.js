...
const ALIAS_EVIDENCE_MIN_HITS = 2;
...
const ALIASES = {
  ...
  hoc_phi: ["gia", "chi_phi", "phi"],
  lich_hoc: ["thoi_khoa_bieu", "lich", "buoi", "hinh_thuc", "zoom"],
  gio_hoc: ["buoi", "hinh_thuc", "zoom"],
  hoc_ngay: ["buoi", "hinh_thuc", "zoom"],
  lich_zoom: ["buoi", "hinh_thuc", "khoa_bieu"],
  buoi_hoc: ["hinh_thuc", "zoom", "khoa_bieu"],
...
export function retrieveKnowledge({ query, corpus, ceiling = KNOWLEDGE_TURN_CEILING }) {
  ...
  const originalTerms = new Set(original);
  ...
  const safeEvidenceByIntent = original.map((term) => {
  ...
  for (const chunk of chunks) {
    let score = 0; let aliasScore = 0; let rareMatch = false;
    ...
    if (safeEvidenceByIntent.some((evidence) => {
      let hits = 0;
    ...
    ranked.push({ chunk, score: score * coverage, baseScore });
  }
  ranked.sort(compareRank);
  const topScore = ranked[0]?.score || 0;
  const cutoffReferenceScore = ranked.reduce((max, candidate) => Math.max(max, candidate.baseScore), 0);
  ...
  return { units, stats: {
    ...
    topScore, cutoffReferenceScore, cutoffScore, selectedUnitCount: units.length,
