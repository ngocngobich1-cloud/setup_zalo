...
export function taoBoGom({
  ...
  function mocAdmission(tin, fallback) {
    const persisted = Number(tin?.__durableAdmittedAt);
    return Number.isFinite(persisted) && persisted > 0 ? Math.min(persisted, fallback) : fallback;
  }

  function doTreConLai(muc) {
    const now = bayGio();
    return Math.max(0, Math.min(
      choMs,
      choMs - Math.max(0, now - muc.lanCuoi),
      tranMs - Math.max(0, now - muc.batDau)
    ));
  }
  ...
  function them(tin, originToken = null) {
    ...
    const now = bayGio();
    const admittedAt = mocAdmission(tin, now);
    const muc = mucDangMo || {
      tins: [],
      batDau: admittedAt,
      lanCuoi: admittedAt,
    ...
    muc.batDau = Math.min(muc.batDau, admittedAt);
    muc.lanCuoi = Math.max(muc.lanCuoi, admittedAt);
    muc.tins.push(tin);
    ...
    const chot = () => {
      ...
      if (conDangGo(tin.threadId, tin.senderId) && bayGio() - muc.batDau < tranMs) {
        muc.dongHo = datHen(chot, Math.max(0, Math.min(
          nhipKiemMs,
          tranMs - (bayGio() - muc.batDau)
        )));
        return;
      }
      dang.delete(khoa);
    ...
    muc.dongHo = datHen(chot, doTreConLai(muc));
    dang.set(khoa, muc);
  }
