 ...
 * @param {(threadId, senderId, mocTinCuoi) => boolean} p.conDangGo
 *        mocTinCuoi la luc tin gan nhat cua nguoi do ve toi noi
...
export function taoBoGom({ choMs = CHO_GOM_MS, tranMs = TRAN_CHO_MS, conDangGo, khiChot, bayGio = Date.now }) {
  ...
  function them(tin) {
    ...
    const chot = () => {
      if (conDangGo(tin.threadId, tin.senderId, muc.tinCuoi) && bayGio() - muc.batDau < tranMs) {
