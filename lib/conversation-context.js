...
export async function buildRecentHistory(ownerUid, threadId, boQuaMessageId, tuyChon = {}) {
  // Khong ro tai khoan thi khong doc lich su cua bat ky ai: cung mot threadId co
  // the ton tai duoi hai tai khoan Zalo khac nhau.
  ...
  let rows;
  try {
    rows = await getThreadMessages(ownerUid, threadId, soTin + 10);
  } catch (error) {
  ...
  const boQuaIds = boQuaMessageId === null || boQuaMessageId === undefined
    ? new Set()
    : boQuaMessageId instanceof Set
      ? new Set([...boQuaMessageId].map(String))
      : Array.isArray(boQuaMessageId)
        ? new Set(boQuaMessageId.map(String))
        : new Set([String(boQuaMessageId)]);
  const chon = [];
  let tong = 0;
  // Duyet tu MOI NHAT ve cu, de khi cham tran thi cai bi bo lai la tin cu nhat.
  for (let i = rows.length - 1; i >= 0; i--) {
    const m = rows[i];
    if (boQuaIds.has(String(m.id))) continue;
