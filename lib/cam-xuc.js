...
/**
 * DANH SACH TRANG RIENG CHO NGUOI DUNG APP - KHONG phai cua bot.
 *
 * Bot van chi duoc HEART/LIKE (CHO_PHEP o tren): bot tu chon luc nao tha, nen
 * mot cai 😡 chon nham la mat khach. Nguoi that ngoi bam thi tu chiu trach
 * nhiem cho lua chon cua minh, nen duoc dung du sau bieu tuong.
 *
 * Van la danh sach TRANG chu khong phai passthrough: Zalo co ~60 bieu tuong,
 * nhung UI da duyet chi co sau cai. Mo het enum ra la de trinh duyet gui gi
 * cung duoc, va giao dien khong con la nguon su that nua.
 */
const CHO_PHEP_APP = {
...
/** Ten rieng cho hanh dong GO bieu tuong dang tha. Zalo nhan chuoi rong. */
export const GO_CAM_XUC = "NONE";

/**
 * @returns {string|null} ma Zalo, hoac null neu ten khong nam trong danh sach
 * trang cua app. NONE tra ve Reactions.NONE (chuoi rong) de go bieu tuong.
 */
export function layBieuTuongApp(ten) {
  if (ten === GO_CAM_XUC) return Reactions.NONE;
  return Object.prototype.hasOwnProperty.call(CHO_PHEP_APP, ten) ? CHO_PHEP_APP[ten] : null;
}

export function danhSachChoPhepApp() {
  return Object.keys(CHO_PHEP_APP);
}

/** Ma Zalo -> ten nghiep vu, tra cuu nguoc. */
const TEN_THEO_MA = new Map(Object.entries(CHO_PHEP_APP).map(([ten, ma]) => [ma, ten]));

/**
 * Doi ma Zalo ve ten nghiep vu de giao dien khong bao gio phai biet tu vung cua
 * provider. Nguoi khac tha mot bieu tuong ngoai sau cai da duyet thi tra
 * "KHAC": van dem dung so luot, nhung khong keo ca enum cua Zalo vao trinh duyet.
 */
export function tenBieuTuongApp(ma) {
  return TEN_THEO_MA.get(String(ma ?? "")) || "KHAC";
}
