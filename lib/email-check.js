...
import { timThuDaGui as timThuDaGuiThat, timThuTraVe as timThuTraVeThat } from "./zoho-mail.js";
import { addLog } from "./activity-log.js";

let timThuDaGui = timThuDaGuiThat;
let timThuTraVe = timThuTraVeThat;

/** Seam hep cho regression co lap; khong thay the getZohoConfig hay duong production khac. */
export function capHinhTimThuZohoChoKiemThu({ timThuDaGui: daGui, timThuTraVe: traVe } = {}) {
  timThuDaGui = typeof daGui === "function" ? daGui : timThuDaGuiThat;
  timThuTraVe = typeof traVe === "function" ? traVe : timThuTraVeThat;
}

/** ai-chat tiem ham lay uid tai khoan Zalo dang dang nhap vao day. */
...
/**
 * Tinh nang nay sinh ra de bao voi khach "mail da gui roi ạ". Nhung Zoho tra loi
 * cho BAT KY dia chi nao hoi den, nen ve ky thuat mot nguoi la co the do xem
 * Maivizen tung gui thu cho ai - lo thong tin cua khach KHAC.
 *
 * Khong doi tinh nang (chi da quyet), chi dat tran: mot nguoi hoi vai dia chi la
 * binh thuong, hoi hang chuc dia chi khac nhau thi khong con la hoi thu cua minh.
 */
const TRA_MOI_GIO = 8;
const lichSuTra = new Map(); // nguoiHoi -> [moc thoi gian]

function conLuotTra(nguoiHoi) {
  const khoa = String(nguoiHoi || "?");
  const moc = Date.now() - 60 * 60 * 1000;
  const ds = (lichSuTra.get(khoa) || []).filter((t) => t > moc);
  if (ds.length >= TRA_MOI_GIO) {
...
export async function traCuu({ email, nguon = "bot", nguoiHoiTen = "", nguoiHoiUid = "" }) {
  ...
  // Cua so cho khach tu tra: dung de bao "mail da gui chua". Mot nguoi hoi vai
  // dia chi la binh thuong; hoi hang chuc dia chi khac nhau thi khong con la
  // hoi thu cua minh nua ma la do xem Maivizen tung gui cho ai. Chan lai va
  // ghi ro vao nhat ky de chi nhin thay.
  // Chi tra bao nhieu cung duoc - do la hop mail cua chinh chi.
  // Doi chieu bang tien to "admin": duong lenh admin dung "admin_zalo".
  const laAdmin = String(nguon || "").startsWith("admin");
  if (!laAdmin && !conLuotTra(nguoiHoiUid || nguoiHoiTen)) {
    await addLog({
      event: "email_tra_qua_nhieu",
      level: "warn",
      summary: `Chặn tra email: ${nguoiHoiTen || nguoiHoiUid || "?"} đã tra quá ${TRA_MOI_GIO} địa chỉ trong 1 giờ`,
      detail: { nguoiHoiTen, nguoiHoiUid, email },
    }).catch(() => {});
    return null;
  }
  ...
  if (ketQua.baoAdmin && nguon === "bot" && baoAdmin) {
    ...
    if (loiNhan) await baoAdmin(chu, loiNhan).catch(() => {});
  }
