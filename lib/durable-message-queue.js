...
export function durableIds(tins) {
...
export async function ganDurableGenerationChoWork(work, generation) {
  ...
  const rows = await assignDurableGeneration(members.map((row) => row.id), generationKey);
  const assignedRows = rows.filter((row) => row.generationKey === generationKey);
  if (!assignedRows.length) return null;
  generation.durableGenerationKey = generationKey;
  generation.durableJobIds = assignedRows.map((row) => row.id);
  generation.expectedMemberCount = generation.durableJobIds.length;
  return generationKey;
}

export async function giaHanLeaseDurableGeneration(generation, leaseMs = 300_000) {
  const expected = Number(generation?.expectedMemberCount || 0);
  const generationKey = generation?.durableGenerationKey;
  if (!generationKey || expected <= 0) return false;
  const affectedRows = await giaHanDurableGeneration(generationKey, leaseMs);
  return affectedRows === expected;
}
...
export async function hoanTatDurableGeneration(_work, generation, runnerError = null) {
  ...
  if (generation.durableAuthorityLost) return false;
  ...
  if (error && !generation.durableOutboxPrepared) {
    // Machine code tuong minh thang viec classify lai human-readable error text.
    const explicitCode = String(generation.durableFailureCode || "").trim();
    const code = explicitCode || classifyProviderFailure(error);
