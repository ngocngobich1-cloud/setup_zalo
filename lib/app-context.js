...
import { capabilityRoutingEnabled, FALLBACK_CAPABILITY_ORDER } from "./ai-model-router.js";
import { splitModel } from "./opencode.js";
...
export const CONFIGURATION_SCOPE = Object.freeze({
  ...
  RUNTIME_SINGLETON_WITH_IDENTITY_GATE: "RUNTIME_SINGLETON_WITH_IDENTITY_GATE",
  NOT_APPLICABLE: "NOT_APPLICABLE",
});

export const SETTING_STATE = Object.freeze({
  ENABLED: "ENABLED",
  DISABLED: "DISABLED",
  UNKNOWN: "UNKNOWN",
});
...
export const ZALO_LOGIN_STATE = Object.freeze({
  LOGGED_IN: "LOGGED_IN",
  LOGGED_OUT: "LOGGED_OUT",
  UNKNOWN: "UNKNOWN",
});

export const ZALO_CONNECTION_STATE = Object.freeze({
  CONNECTED: "CONNECTED",
...
let zaloRuntimeAccessors = null;

/** zalo-service tiem ba accessor read-only de app-context khong import runtime stack. */
export function configureZaloRuntimeReader(accessors = null) {
  if (accessors === null) {
    zaloRuntimeAccessors = null;
    return;
  }
  const currentOwner = accessors?.currentOwner;
  const loggedIn = accessors?.loggedIn;
  const connectionState = accessors?.connectionState;
  if (![currentOwner, loggedIn, connectionState].every((fn) => typeof fn === "function")) {
    throw new TypeError("Zalo runtime reader cần đủ ba accessor read-only.");
  }
  zaloRuntimeAccessors = Object.freeze({ currentOwner, loggedIn, connectionState });
}

async function docZaloRuntime(ownerUid) {
  if (!zaloRuntimeAccessors) throw new Error("Zalo runtime reader chưa được inject.");

  const rawCurrentOwner = await zaloRuntimeAccessors.currentOwner();
  const currentOwner = rawCurrentOwner === null || rawCurrentOwner === undefined || rawCurrentOwner === ""
    ? null
    : String(rawCurrentOwner).trim();
  if (currentOwner && currentOwner !== ownerUid) return { identityGate: "MISMATCH" };

  const loggedIn = await zaloRuntimeAccessors.loggedIn();
  if (typeof loggedIn !== "boolean") throw new Error("Zalo logged-in state không xác định.");
  if (currentOwner === null && loggedIn !== false) return { identityGate: "AMBIGUOUS" };

  return {
    identityGate: "PASS",
    loggedIn,
    connectionState: await zaloRuntimeAccessors.connectionState(),
  };
}

const DEFAULT_READERS = Object.freeze({
  admin: (ownerUid) => getAdminZalo(ownerUid),
  aiConfig: (ownerUid) => getAiChatConfig(ownerUid),
  aiCredentialStatus: (ownerUid) => listOwnerProviderCredentialStatus(ownerUid),
  capabilityRouting: () => capabilityRoutingEnabled(),
  zaloRuntime: (ownerUid) => docZaloRuntime(ownerUid),
  accountBot: (ownerUid) => getAccountConfig(ownerUid),
  knowledgeFiles: (ownerUid) => getAllKnowledgeFiles(ownerUid),
  autoReplyCount: (ownerUid) => getAutoReplyRuleCount(ownerUid),
  pdfAutomationCount: (ownerUid) => getEnabledPdfAutomationRuleCount(ownerUid),
  scheduledMessageCount: (ownerUid) => getPendingScheduledMessageCount(ownerUid),
  zoom: () => docPresence({
...
function trangThaiModel(aiConfig, credentials, field) {
  if (!aiConfig.known) return CONFIGURATION.UNKNOWN;
  const model = String(aiConfig.value?.[field] || "").trim();
  if (!model) return CONFIGURATION.NOT_CONFIGURED;
  const identity = splitModel(model);
  if (!identity?.providerID) return CONFIGURATION.UNKNOWN;
  if (!credentials.known || !Array.isArray(credentials.value)) return CONFIGURATION.UNKNOWN;
  return credentials.value.some(
    (item) => String(item?.providerId || "").trim() === identity.providerID
  )
    ? CONFIGURATION.CONFIGURED
    : CONFIGURATION.PARTIALLY_CONFIGURED;
}

function trangThaiBatTat(read, value) {
  if (!read.known) return SETTING_STATE.UNKNOWN;
  const selected = value(read.value);
  if (typeof selected !== "boolean") return SETTING_STATE.UNKNOWN;
  return selected ? SETTING_STATE.ENABLED : SETTING_STATE.DISABLED;
}

function docSoDem(read) {
  if (!read.known) return { configuration: CONFIGURATION.UNKNOWN, count: CONFIGURATION.UNKNOWN };
  const count = Number(read.value);
  if (!Number.isInteger(count) || count < 0) {
    return { configuration: CONFIGURATION.UNKNOWN, count: CONFIGURATION.UNKNOWN };
  }
  return {
    configuration: count > 0 ? CONFIGURATION.CONFIGURED : CONFIGURATION.NOT_CONFIGURED,
    count,
  };
}

function mapZaloConnection(value) {
  return ({
  ...
  })[String(value || "")] || ZALO_CONNECTION_STATE.UNKNOWN;
}

function docZaloState(read) {
  if (!read.known || read.value?.identityGate !== "PASS") {
    return {
  ...
  return {
    loggedIn: read.value.loggedIn === true
      ? ZALO_LOGIN_STATE.LOGGED_IN
      : read.value.loggedIn === false
        ? ZALO_LOGIN_STATE.LOGGED_OUT
        : ZALO_LOGIN_STATE.UNKNOWN,
    connectionState: mapZaloConnection(read.value.connectionState),
  };
}
...
export async function buildAppContextWithReaders(ownerAuthority, readerOverrides = {}) {
  ...
  const normalizedOverrides = { ...readerOverrides };
  if (Object.hasOwn(normalizedOverrides, "assistant") && !Object.hasOwn(normalizedOverrides, "aiConfig")) {
    normalizedOverrides.aiConfig = normalizedOverrides.assistant;
  }
  const readers = { ...DEFAULT_READERS, ...normalizedOverrides };
  const [
    admin,
    aiConfig,
    aiCredentialStatus,
    ...
    docAnToan(readers.aiConfig, ownerUid),
    docAnToan(readers.aiCredentialStatus, ownerUid),
    docAnToan(readers.capabilityRouting, ownerUid),
    docAnToan(readers.zaloRuntime, ownerUid),
    docAnToan(readers.accountBot, ownerUid),
    docAnToan(readers.knowledgeFiles, ownerUid),
    docAnToan(readers.autoReplyCount, ownerUid),
    docAnToan(readers.pdfAutomationCount, ownerUid),
    docAnToan(readers.scheduledMessageCount, ownerUid),
  ...
  const primaryAiConfiguration = trangThaiModel(aiConfig, aiCredentialStatus, "opencodeModel");
  const secondaryAiConfiguration = trangThaiModel(aiConfig, aiCredentialStatus, "opencodeFallbackModel");
  const secondaryCapabilities = aiConfig.known && Array.isArray(aiConfig.value?.opencodeFallbackCapabilities)
    ? FALLBACK_CAPABILITY_ORDER.filter((item) => aiConfig.value.opencodeFallbackCapabilities.includes(item))
    : null;
  const secondaryCapabilityConfiguration = secondaryCapabilities === null
    ? CONFIGURATION.UNKNOWN
    : secondaryCapabilities.length > 0
      ? CONFIGURATION.CONFIGURED
      : CONFIGURATION.NOT_CONFIGURED;
  const capabilityRoutingState = trangThaiBatTat(capabilityRouting, (value) => value);
  const runtimeFailoverState = trangThaiBatTat(aiConfig, (value) => value?.opencodeFailoverEnabled);
  const accountBotState = trangThaiBatTat(accountBot, (value) => value?.botEnabled);
  const zaloState = docZaloState(zaloRuntime);
  const knowledgeEnabledState = trangThaiBatTat(aiConfig, (value) => value?.useKnowledge);
  const knowledgeFilesExist = !knowledgeFiles.known || !Array.isArray(knowledgeFiles.value)
    ? EXISTENCE_STATE.UNKNOWN
    : knowledgeFiles.value.length > 0
      ? EXISTENCE_STATE.EXISTS
      : EXISTENCE_STATE.DOES_NOT_EXIST;
  const existingKnowledgeIds = knowledgeFiles.known && Array.isArray(knowledgeFiles.value)
    ? new Set(knowledgeFiles.value.map((file) => String(file?.id)))
    : null;
  const knowledgeSelectedCount = aiConfig.known
    && Array.isArray(aiConfig.value?.knowledgeFileIds)
    && existingKnowledgeIds
    ? [...new Set(aiConfig.value.knowledgeFileIds.map((id) => String(id)))]
      .filter((id) => existingKnowledgeIds.has(id)).length
    : CONFIGURATION.UNKNOWN;
  const autoReply = docSoDem(autoReplyCount);
  const pdfAutomation = docSoDem(pdfAutomationCount);
  const scheduledMessages = docSoDem(scheduledMessageCount);
  ...
  return {
    ...
    semantics: {
      ...
      accountBotScope: "Bot.accountEnabled chỉ phản ánh công tắc Bot cấp tài khoản. Trạng thái Bot của từng hội thoại có thể khác và không nằm trong C1 context.",
      automationSeparation: "Công tắc Bot cấp tài khoản, Auto Reply rule count, PDF Automation enabled rule count và Scheduled Message pending count là bốn dữ kiện độc lập.",
    },
    capabilities: [
      khaNang({
      ...
      khaNang({
        ...
        label: "AI chính",
        availability: AVAILABILITY.AVAILABLE,
        configuration: primaryAiConfiguration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" }],
      }),
      khaNang({
        id: "AI.secondary",
        label: "AI bổ trợ",
        availability: AVAILABILITY.AVAILABLE,
        configuration: secondaryAiConfiguration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" }],
      }),
      khaNang({
        id: "AI.secondaryCapabilities",
        label: "Phần việc đã cấu hình cho AI bổ trợ",
        availability: AVAILABILITY.AVAILABLE,
        configuration: secondaryCapabilityConfiguration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" }],
        details: {
          configuredCapabilities: secondaryCapabilities ?? CONFIGURATION.UNKNOWN,
          semantics: "Chỉ phản ánh ý định đã cấu hình; không khẳng định đang dùng được ở runtime.",
        },
      }),
      khaNang({
        id: "AI.capabilityRouting",
        label: "Định tuyến theo capability",
        availability: AVAILABILITY.AVAILABLE,
        configurationScope: CONFIGURATION_SCOPE.APP_GLOBAL,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" }],
        details: { state: capabilityRoutingState },
      }),
      khaNang({
        id: "AI.runtimeFailover",
        label: "Chuyển sang AI bổ trợ khi AI chính tạm thời lỗi",
        availability: AVAILABILITY.AVAILABLE,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" }],
        details: { state: runtimeFailoverState },
      }),
      khaNang({
        id: "Zalo.runtime",
        label: "Trạng thái kết nối Zalo",
        availability: AVAILABILITY.AVAILABLE,
        configurationScope: CONFIGURATION_SCOPE.RUNTIME_SINGLETON_WITH_IDENTITY_GATE,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "Tài khoản" }],
        details: zaloState,
      }),
      khaNang({
        id: "Bot.accountEnabled",
        label: "Công tắc Bot cấp tài khoản",
        availability: AVAILABILITY.AVAILABLE,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "Hội thoại / Inbox" }],
        details: {
          state: accountBotState,
          authorityScope: "ACCOUNT_LEVEL_ONLY",
          perConversationState: "NOT_IN_C1_CONTEXT",
        },
      }),
      khaNang({
        id: "Knowledge.configuration",
        label: "Cấu hình Tri thức",
        availability: AVAILABILITY.AVAILABLE,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [
          { kind: NAVIGATION_KIND.SCREEN, screenLabel: "Tri thức" },
          { kind: NAVIGATION_KIND.SCREEN, screenLabel: "AI Chat" },
        ],
        details: {
          enabled: knowledgeEnabledState,
          filesExist: knowledgeFilesExist,
          selectedCount: knowledgeSelectedCount,
        },
      }),
      khaNang({
        id: "AutoReply.rules",
        label: "Quy tắc trả lời tự động",
        availability: AVAILABILITY.AVAILABLE,
        configuration: autoReply.configuration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "Trả lời tự động" }],
        details: { ruleCount: autoReply.count },
      }),
      khaNang({
        id: "PdfAutomation.enabledRules",
        label: "Quy tắc tự động gửi tài liệu đang bật",
        availability: AVAILABILITY.AVAILABLE,
        configuration: pdfAutomation.configuration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "Tự động gửi tài liệu" }],
        details: { enabledRuleCount: pdfAutomation.count },
      }),
      khaNang({
        id: "Scheduling.pendingMessages",
        label: "Lịch nhắn đang chờ",
        availability: AVAILABILITY.AVAILABLE,
        configuration: scheduledMessages.configuration,
        configurationScope: CONFIGURATION_SCOPE.OWNER,
        navigationPaths: [{ kind: NAVIGATION_KIND.SCREEN, screenLabel: "Lịch hẹn" }],
        details: { pendingCount: scheduledMessages.count },
      }),
      khaNang({
...
export function renderAppContext(appContext) {
  return [
    ...
    "Với trạng thái công tắc, ENABLED nghĩa là đang bật và DISABLED nghĩa là đang tắt. Với Zalo, chỉ nói đã đăng nhập/kết nối khi dữ liệu runtime đã qua kiểm tra đúng tài khoản; UNKNOWN thì nói chưa xác minh được.",
    "Danh sách phần việc của AI bổ trợ chỉ là ý định đã cấu hình. Không biến IMAGE_INPUT, FILE_INPUT hay WEB_SEARCH thành tuyên bố model hiện dùng được; đặc biệt không nói WEB_SEARCH hiện hoạt động nếu dữ liệu chỉ cho biết đã cấu hình.",
    "Với Bot.accountEnabled, chỉ nói ‘Bot tổng của tài khoản hiện đang bật’ hoặc ‘Bot tổng của tài khoản hiện đang tắt’ theo state. Luôn nói rõ trạng thái Bot ở từng hội thoại có thể khác và không nằm trong context này; không chuyển trạng thái cấp tài khoản thành kết luận bao trùm các hội thoại hay kết luận Bot đang phản hồi trong hội thoại.",
    "Luôn giữ riêng bốn dữ kiện: công tắc Bot cấp tài khoản, số quy tắc Trả lời tự động, số quy tắc Tự động gửi tài liệu đang bật và số Lịch hẹn đang chờ. Không dùng trạng thái của dữ kiện này để suy ra trạng thái của dữ kiện khác.",
    "Số tài liệu Tri thức đã chọn chỉ tính các ID vẫn còn tồn tại trong kho của đúng tài khoản.",
