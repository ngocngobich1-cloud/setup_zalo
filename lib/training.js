...
import { renderAppAdvisorPolicy } from "./app-advisor.js";
import {
  CAPABILITIES,
  ROUTE_MODES,
  SURFACES,
...
async function guiVaLuu(ownerUid, config, sessionId, text, files, routeContext) {
  ...
  const factualText = renderAppContext(appContext);
  const advisorText = renderAppAdvisorPolicy();
  const baseParts = [{ type: "text", text: `${factualText}\n\n${advisorText}` }];
