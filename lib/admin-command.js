...
import { handleAdminClarificationMessage } from "./admin-clarification.js";
import {
...
const ADMIN_AI_SLOT_WAIT_MS = 60_000;
const ADMIN_AI_BUSY_RESPONSE = "Vizen đang bận xử lý nhiều yêu cầu. Chị thử lại sau một phút giúp em nhé.";
...
export function docMucKhan(cauLenh) {
  ...
  const tiepNoiTu = String.raw`[\p{L}\p{N}\p{M}_]`;
  const mucHai = new RegExp(`(?<!${tiepNoiTu})(?:khẩn|khan cap|gấp|gap)(?!${tiepNoiTu})`, "u");
  const mucMot = new RegExp(
    `(?<!${tiepNoiTu})(?:quan trọng|quan trong)(?!${tiepNoiTu})|` +
      `(?<!${tiepNoiTu})nhớ(?!${tiepNoiTu}).*(?<!${tiepNoiTu})nhé(?!${tiepNoiTu})`,
    "u"
  );
  if (mucHai.test(s)) return 2;
  if (mucMot.test(s)) return 1;
  return 0;
}
...
export function laXacNhanOK(text) {
...
async function danhSachDichDen(ownerUid) {
  const threads = await listThreads(ownerUid, { recentOnly: true });
...
export async function xuLyLenh(message, gui, guiRiengDaTim = null) {
  ...
  const ownerUid = String(message?.__adminClarificationOwnerUid || layChuTaiKhoan() || "").trim();
  // Caller (zalo-service) has already authenticated this private Admin sender.
  // Correlation tokens must preempt every legacy draft/OK/intent branch.
  if (typeof handleAdminClarificationMessage === "function") {
    const clarification = await handleAdminClarificationMessage({ ownerUid, message });
    if (clarification.handled) return null;
  }
  const khoa = khoaThaoTacCho(ownerUid, message.threadId);
  ...
  if (dangCho) {
    if (laXacNhanOK(noiDung)) {
      ...
      if (dangCho.loai === "duyet_nhom") return duyetVaoNhom(dangCho, ownerUid);
  ...
  const dichDen = await danhSachDichDen(ownerUid);
  ...
  if (!ownerUid) return "Chưa đăng nhập Zalo nên em chưa thể phân tích lệnh.";
  const savedConfig = await getAiChatConfig(ownerUid);
  const config = await resolveEffectiveModelConfig(savedConfig || {});

  let ketQua;
  try {
    ketQua = await withGlobalAiSlot({
      ownerUid,
      threadId: message?.threadId || null,
      orderKey: Date.now(),
      timeoutMs: ADMIN_AI_SLOT_WAIT_MS,
      onGranted: ({ waitMs }) => addLog({
        event: "admin_ai_slot_wait_ms",
        level: "info",
        summary: `Admin đã chờ slot AI ${waitMs}ms.`,
        detail: { ownerUid, threadId: message?.threadId || null, waitMs },
      }).catch(() => {}),
    }, async () => {
      if (phanTichLenhAdminGia) {
        return phanTichLenhAdminGia(config, noiDung, dichDen);
      }
      const primaryProvider = splitModel(config.opencodeModel)?.providerID;
      // Global slot nam NGOAI credential READ lease. Moi business/Zalo side
      // effect cua xuLyLenh van o ngoai ca hai bien gioi nay.
      return withOwnerCredentialReadSet(
        ownerUid,
        [primaryProvider],
        () => phanTichLenh(config, noiDung, dichDen)
      );
    });
  } catch (error) {
    if (isAiSlotTimeoutError(error)) {
      cho.delete(khoa);
      await addLog({
        event: "admin_ai_slot_wait_timeout",
        level: "warn",
        summary: "Admin chờ slot AI quá 60 giây; đã dọn trạng thái pending.",
        detail: { ownerUid, threadId: message?.threadId || null },
      }).catch(() => {});
      return ADMIN_AI_BUSY_RESPONSE;
    }
  ...
  if (ketQua?.hanhDong === "day_ghi_nho") return dayGhiNho(ketQua, message, ownerUid);
  if (ketQua?.hanhDong === "day_sua") return daySua(ketQua, message, ownerUid);
  ...
  if (ketQua?.hanhDong === "xem_cho_duyet") return xemChoDuyet(ketQua, dichDen, ownerUid);
  if (ketQua?.hanhDong === "duyet_vao_nhom") return chuanBiDuyet(ketQua, dichDen, khoa, ownerUid);
 ...
 */
function coThaoTacDangCho(message, ownerUid) {
  const dang = cho.get(khoaThaoTacCho(ownerUid, message?.threadId));
...
async function dayGhiNho(ketQua, message, ownerUid) {
  ...
  if (coThaoTacDangCho(message, ownerUid)) return LOI_DANG_CO_THAO_TAC_KHAC;
...
async function daySua(ketQua, message, ownerUid) {
  const nguoi = await timNguoiDeDay(ketQua?.dichTen);
  if (!nguoi.ok) return loiTimNguoi(nguoi);
...
function khoaDuyetGanDay(ownerUid, groupId) {
  return `${String(ownerUid || "")}::${String(groupId || "")}`;
}

async function xemChoDuyet(ketQua, dichDen, ownerUid) {
  const dich = dichDen.find((d) => String(d.id) === String(ketQua.dichId));
  if (!dich) return "Em không tìm thấy nhóm đó ạ.";
  ...
  try {
    ...
    choDuyetGanDay.set(khoaDuyetGanDay(ownerUid, dich.id), ds);
...
async function chuanBiDuyet(ketQua, dichDen, khoa, ownerUid) {
  const dich = dichDen.find((d) => String(d.id) === String(ketQua.dichId));
  if (!dich) return "Em không tìm thấy nhóm đó ạ.";

  const ds = choDuyetGanDay.get(khoaDuyetGanDay(ownerUid, dich.id));
...
async function duyetVaoNhom(dangCho, ownerUid) {
  ...
  try {
    ...
    choDuyetGanDay.delete(khoaDuyetGanDay(ownerUid, dangCho.dichId));
