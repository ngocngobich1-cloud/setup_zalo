...
export async function xuLyLenh(message, gui, guiRiengDaTim = null) {
  ...
  try {
    if (phanTichLenhAdminGia) {
      ketQua = await phanTichLenhAdminGia(config, noiDung, dichDen);
    } else {
      const primaryProvider = splitModel(config.opencodeModel)?.providerID;
      // Chi AI parser nam trong credential READ lease. Moi business/Zalo side
      // effect cua xuLyLenh van o ngoai lock nay.
      ketQua = await withOwnerCredentialReadSet(
        ownerUid,
        [primaryProvider],
