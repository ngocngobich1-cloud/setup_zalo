...
export async function xuLyLenh(message, gui) {
  ...
  // Chu "xac nhan" khong can bo phan tich de hieu, va mot cau bat ky cung phai
  ...
  // Go "xac nhan" nhung chang co gi dang cho ca (vi du app vua khoi dong lai
  // lam mat ban nhap). Noi that va dung o day - khong phi mot lot goi OpenCode
  // chi de hieu hai chu nay.
  if (bocDau(noiDung) === CHU_XAC_NHAN) {
    return "Hiện không có nội dung nào đang chờ xác nhận.";
  }
 ...
 * cho admin go "xac nhan" roi moi ghi.
 *
...
/** Chi mot chu nay moi la dong y. "ok"/"ừ"/"được" CO Y khong duoc tinh. */
const CHU_XAC_NHAN = "xac nhan";
 ...
 *
 * Vi sao phai hoi: chi co MOT chu "xac nhan". De hai thu cung cho thi chu do
...
const LOI_DANG_CO_THAO_TAC_KHAC = [
  "Hiện đang có một thao tác khác chờ xác nhận.",
  "Hãy xác nhận hoặc huỷ thao tác đó trước, rồi dạy em tiếp nhé.",
].join("\n");
...
function cauDocLai(ten, quyTac, laSua) {
  return [
    ...
    `“${quyTac}”`,
    "",
    'Nếu đúng, hãy trả lời “xác nhận” để em lưu. Nếu sai, hãy nhắn “sai rồi”.',
  ].join("\n");
}
...
async function dayGhiNho(ketQua, message) {
  ...
  // Mot chu "xac nhan" chi duoc ung voi MOT thao tac. Dang co thao tac khac cho
...
/**
 * Admin da go "xac nhan" -> gio moi duoc ghi.
 *
 ...
 * bo phan tich. Chu "xac nhan" khong can AI de hieu.
 *
...
async function xuLyBanNhapDay(message) {
  ...
  // mot ban nhap tu nay giờ nam cho, roi vai phut sau mot chu "xac nhan" lac
  // vao lam ghi nham.
  choDay.delete(khoa);
  return null;
}
