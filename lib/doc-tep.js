...
export async function taiTep(tep, options = {}) {
  ...
  if (mime.startsWith("image/") && buf.length < NHO_NHAT_BYTE && !options.choPhepAnhNho) {
...
const NHIEM_VU_STICKER = [
  "Bạn là specialist trung tính đang quan sát Sticker khách vừa gửi.",
  "Mô tả ngắn gọn, khách quan nhân vật/vật thể, biểu cảm, hành động, chữ nhìn thấy được và sắc thái giao tiếp có thể quan sát.",
  "Chỉ nói điều thật sự nhìn thấy; nêu rõ chỗ không chắc. Không suy đoán ngoài hình ảnh.",
  "Không viết câu trả lời cho khách, không chào hỏi.",
].join("\n");

export const STICKER_VISION_FAILURE_MARKER =
  "(Khách vừa gửi một Sticker nhưng hệ thống không đọc được nội dung hình ảnh.)";
...
export async function docVaTomTat(config, tep, options = {}) {
  ...
  const operation = async () => {
    ...
    try {
      const ketQua = await runOneShot(selectedConfig, options.title || `Đọc tệp - ${tep.ten}`, [
        {
          type: "file",
          mime: tep.mime,
          filename: tep.ten,
        ...
        { type: "text", text: options.task || NHIEM_VU },
      ]);
      if (secondary) {
        await addLog({
...
export async function xuLyTep(config, message, options = {}) {
  ...
  const tep = sticker
    ? sticker.url ? { href: sticker.url, ten: "sticker", loaiTin: "chat.sticker" } : null
    : layTepTuTin(message);
  if (!tep) return sticker
    ? { khoiChoAgent: `${STICKER_VISION_FAILURE_MARKER}\n\n`, tomTat: null, loi: "STICKER_IMAGE_UNAVAILABLE" }
    : { khoiChoAgent: "", tomTat: null, loi: null };

  try {
    let daTai;
    try {
      daTai = await taiTep(tep, { choPhepAnhNho: Boolean(sticker) });
    } catch (fetchError) {
      if (!sticker?.refreshUrl) throw fetchError;
      const refreshedUrl = await sticker.refreshUrl(tep.href);
      if (!refreshedUrl || refreshedUrl === tep.href) throw fetchError;
      daTai = await taiTep({ ...tep, href: refreshedUrl }, { choPhepAnhNho: true });
    }
    if (sticker && !daTai.mime.startsWith("image/")) throw new Error("Sticker asset is not an image");
    const { tomTat, tokens, routeDecision } = await docVaTomTat(config, daTai, sticker
      ? { ...options, title: "Mô tả Sticker", task: NHIEM_VU_STICKER }
      : options);
    ...
    if (sticker) return {
      ...
      khoiChoAgent: `# STICKER KHÁCH VỪA GỬI\n${tomTat}\n\n`,
    };
    ...
    if (sticker) {
      await addLog({
        event: "doc_tep",
        level: "warn",
        summary: "Không đọc được ảnh Sticker khách gửi",
        detail: { nguoiGui: message?.senderName || "" },
      }).catch(() => {});
      return {
        tomTat: null,
        loi: "STICKER_VISION_UNAVAILABLE",
        khoiChoAgent: `${STICKER_VISION_FAILURE_MARKER}\n\n`,
      };
    }
