...
export async function ensureSession(config, threadId, bootstrapContext, onEvent) {
  const existing = await getOpencodeSessionInfo(threadId);
  ...
  if (existing && existing.turns >= PHIEN_MAX_LUOT) {
    ...
    await deleteOpencodeSession(threadId);
  } else if (existing) {
    try {
  ...
  await saveOpencodeSession(threadId, session.id);
