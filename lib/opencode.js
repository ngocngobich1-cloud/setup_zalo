...
import { WEB_PROBE_STATES } from "./ai-model-router.js";
import { FAILURE_CODES, classifyProviderFailure } from "./provider-failure.js";
...
export const KNOWLEDGE_SESSION_CUMULATIVE_CAP = 30000;
const knowledgeBySession = new Map();
export const knowledgeLedger = Object.freeze({
  has(sessionId, hash) { return knowledgeBySession.get(String(sessionId))?.hashes.has(hash) || false; },
  add(sessionId, hashes, chars) {
    const key = String(sessionId);
    const entry = knowledgeBySession.get(key) || { hashes: new Set(), chars: 0 };
    const fresh = hashes.filter((hash) => !entry.hashes.has(hash));
    if (!fresh.length) return;
    for (const hash of fresh) entry.hashes.add(hash);
    entry.chars += Math.max(0, Number(chars) || 0);
    knowledgeBySession.set(key, entry);
  },
  cumulative(sessionId) { return knowledgeBySession.get(String(sessionId))?.chars || 0; },
  reset(sessionId) { knowledgeBySession.delete(String(sessionId)); },
});
 ...
 */
export const KHONG_TOOL = Object.freeze({
  bash: false,
  read: false,
  write: false,
  edit: false,
...
export const TOOL_WEB = Object.freeze({
  ...KHONG_TOOL,
  webfetch: true,
  websearch: true,
});
...
export function getWebProbeState(canonicalModel) {
  return webProbeCache.get(String(canonicalModel || "")) || WEB_PROBE_STATES.UNKNOWN;
}

export function clearWebProbeCache() {
  webProbeCache.clear();
}
...
function credentialRequestForContext(path, options, needsCredentialContext) {
  ...
  if (sessionRequest && sessionRequest.type !== "create") {
    ...
    if (!boundDirectory) {
      knowledgeLedger.reset(sessionId);
...
export async function call(config, path, options = {}) {
  ...
  try {
    ...
    if (request.sessionRequest?.type === "create") {
      ...
      knowledgeLedger.reset(request.sessionRequest.sessionId);
    }
    return response;
  } catch (error) {
 ...
 */
export const CHAT_MIN_CONTEXT = 8192;
    ...
    return (response.providers || [])
      ...
      .map((provider) => ({
        ...
        models: Object.entries(provider.models || {})
          ...
          .map(([id, model]) => {
            const canonicalModel = `${provider.id}/${id}`;
            const text = isChatModel(model);
            const attachment = model?.capabilities?.attachment === true;
            const toolcall = model?.capabilities?.toolcall === true;
            const webProbeState = getWebProbeState(canonicalModel);
            return {
            id: canonicalModel,
            label: model.name || id,
            ...
            capabilities: {
              text,
              image: text && attachment && model?.capabilities?.input?.image === true,
              file: text && attachment && model?.capabilities?.input?.pdf === true,
              web: toolcall && webProbeState === WEB_PROBE_STATES.SUPPORTED,
              toolcall,
            },
            webProbeState,
          };
 ...
 */
export function buildBootstrapMessage({ soul, roleTone, allowedTopics, recentHistory }) {
  const parts = [];
  parts.push("# SOUL");
  ...
  parts.push(
    ...
    "Dữ kiện business chỉ được lấy từ khối [TRI THỨC LIÊN QUAN] được đính kèm theo từng lượt. Nếu không có khối đó, hoặc khối không chứa đáp án, không được bịa thông tin business.",
    "",
  ...
  // Dat SAT CUOI, sau toan bo phan tinh. Soul + nhiem vu giong het nhau o moi
...
export function providerResponseError(infoError) {
  const error = new Error(
    infoError?.data?.message
    || infoError?.message
    || infoError?.name
    || "OpenCode trả về lỗi inference."
  );
  error.code = "OPENCODE_PROVIDER_ERROR";
  error.infoError = infoError;
  const status = Number(infoError?.status || infoError?.statusCode || infoError?.data?.status || 0);
  if (status) error.status = status;
  return error;
}

async function runOneShotUnlocked(config, title, prompt, options = {}) {
  ...
  try {
    const response = await call(config, `/session/${encodeURIComponent(session.id)}/message`, {
      ...
      body: JSON.stringify({
        ...
        tools: options.tools || KHONG_TOOL,
        parts: Array.isArray(prompt) ? prompt : [{ type: "text", text: prompt }],
      }),
      ...(options.timeoutMs ? { timeoutMs: options.timeoutMs } : {}),
    });
    if (response?.info?.error) {
      throw providerResponseError(response.info.error);
    }
    return { text: extractReply(response), tokens: response?.info?.tokens || null, response };
  } finally {
    await deleteSessions(config, [session.id]);
  }
}

export async function runOneShot(config, title, prompt, options = {}) {
  const providerId = splitModel(config?.opencodeModel)?.providerID;
  return withCredentialPlaneRead(
    { providerId },
    () => runOneShotUnlocked(config, title, prompt, options)
  );
}

function hasCompletedTool(response, tool) {
...
export function observedWebToolEvidence(response) {
  return hasCompletedTool(response, "websearch");
}

function provesWebUnsupported(error) {
  const text = (() => {
    try {
      return JSON.stringify(error?.infoError || error || {}).toLowerCase();
    } catch {
      return String(error?.message || "").toLowerCase();
    }
  })();
  return /unknown tool|tool not (?:available|supported)|does not support tool|tool.?call.*unsupported|permission.*(?:websearch|webfetch)|(?:websearch|webfetch).*permission/.test(text);
}

/** Explicit-only runtime probe. PASS chi khi response co websearch tool part completed. */
export async function probeWebSupport(config, canonicalModel, options = {}) {
  const selected = splitModel(canonicalModel);
  if (!selected) throw new TypeError("Model kiểm tra Web không hợp lệ.");
  const modelConfig = { ...config, opencodeModel: canonicalModel };
  try {
    const result = await runOneShot(
      ...
      [
        {
          type: "text",
          text: "Bắt buộc dùng websearch để tìm trang chủ OpenCode rồi trả lời một câu ngắn kèm URL. Không dùng tool nào khác.",
        },
      ],
      { tools: TOOL_WEB, timeoutMs: options.timeoutMs || 30000 }
    );
    if (observedWebToolEvidence(result.response)) {
      webProbeCache.set(canonicalModel, WEB_PROBE_STATES.SUPPORTED);
      return { supported: true, state: WEB_PROBE_STATES.SUPPORTED };
    }
    // Text tu khai khong phai proof; cung khong du de ket luan model unsupported.
    return {
      supported: false,
      state: WEB_PROBE_STATES.UNKNOWN,
      error: "Không quan sát được bằng chứng websearch đã chạy.",
    };
  } catch (error) {
    if (provesWebUnsupported(error)) {
      webProbeCache.set(canonicalModel, WEB_PROBE_STATES.UNSUPPORTED);
      return {
        supported: false,
        state: WEB_PROBE_STATES.UNSUPPORTED,
        error: "Runtime xác nhận model không hỗ trợ Web tool.",
      };
    }
    // Credential, billing, quota va transient failure khong duoc cache UNSUPPORTED.
    throw error;
  }
}
...
async function ensureSessionUnlocked(config, ownerUid, threadId, bootstrapContext, onEvent) {
  ...
  if (existing && (existing.turns >= PHIEN_MAX_LUOT
    || knowledgeLedger.cumulative(existing.sessionId) >= KNOWLEDGE_SESSION_CUMULATIVE_CAP)) {
    ...
    try {
      ...
      knowledgeLedger.reset(existing.sessionId);
      await deleteOpencodeSession(ownerUid, threadId);
    }
  }
  ...
  await call(config, `/session/${encodeURIComponent(session.id)}/message`, {
    ...
    body: JSON.stringify({
      agent: config.opencodeAgent || "general",
      ...modelForMessage(config),
      // Context-only insert: bootstrap khong duoc tieu ton mot inference call
      // trong budget cua logical user turn. Kill switch OFF giu dung baseline.
 ...
 */
const TEST_MAX_MODELS = 1;
export const PROVIDER_TEST_TIMEOUT_MS = 20000;
export const CREDENTIAL_TEST_CLEANUP_RESERVE_MS = 500;

export function classifyProviderTestFailure(error) {
  if (error?.code === "OPENCODE_RUNTIME_ERROR") return "OPENCODE_RUNTIME_ERROR";
  const classified = classifyProviderFailure(error);
  if (classified === FAILURE_CODES.INVALID_KEY) return "INVALID_KEY";
  if ([FAILURE_CODES.RATE_LIMITED, FAILURE_CODES.QUOTA_EXHAUSTED].includes(classified)) return "NO_QUOTA";
  if (classified === FAILURE_CODES.TIMEOUT) return "TIMEOUT";
  if (classified === FAILURE_CODES.PROVIDER_UNAVAILABLE) return "PROVIDER_UNAVAILABLE";
  return "UNKNOWN";
}
...
export async function testProviderKey(config, providerId, options = {}) {
  const id = String(providerId || "").trim();
  if (!id) throw providerTestError("PROVIDER_UNAVAILABLE");
  const suppliedDeadline = Number(options.deadlineAt);
  const deadlineAt = Number.isFinite(suppliedDeadline)
    ? suppliedDeadline
    : Date.now() + PROVIDER_TEST_TIMEOUT_MS;
  const remainingMs = () => Math.max(0, deadlineAt - Date.now());
  const operationRemainingMs = () => Math.max(
    0,
    remainingMs() - CREDENTIAL_TEST_CLEANUP_RESERVE_MS
  );
  const boundedTimeoutMs = (
    cap = PROVIDER_TEST_TIMEOUT_MS,
    { reserveCleanup = false } = {}
  ) => {
    const remaining = reserveCleanup ? operationRemainingMs() : remainingMs();
    if (remaining <= 0) throw providerTestError("TIMEOUT");
  ...
  try {
    return await withCredentialPlaneRead({ providerId: id }, async () => {
      let sessionId = null;
      try {
        const providers = await call(config, "/config/providers", {
          method: "GET",
        ...
        const uuTien = dangChon?.providerID === id && all.includes(dangChon.modelID)
          ? dangChon.modelID
          : null;
        const danhSach = (uuTien ? [uuTien] : all).slice(0, TEST_MAX_MODELS);

        const session = await call(config, "/session", {
          method: "POST",
          timeoutMs: boundedTimeoutMs(PROVIDER_TEST_TIMEOUT_MS, { reserveCleanup: true }),
...
export async function sendPrompt(config, sessionId, text, options = {}) {
  const providerId = splitModel(config?.opencodeModel)?.providerID;
  return withCredentialPlaneRead({ providerId }, async () => {
    ...
    // A successful HTTP insert persists the user turn even when inference
    // returns info.error. Keep the ledger truthful across provider failover.
    const freshKnowledge = (options.knowledgeUnits || [])
      .filter((unit) => !knowledgeLedger.has(sessionId, unit.contentHash));
    knowledgeLedger.add(sessionId, freshKnowledge.map((unit) => unit.contentHash),
      freshKnowledge.reduce((sum, unit) => sum + unit.charCount, 0));
    if (response?.info?.error) {
      throw providerResponseError(response.info.error);
    }
    return {
