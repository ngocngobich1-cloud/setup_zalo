...
function canLyDo(classification, email, phone = null) {
  ...
  if (classification === "AMBIGUOUS_REGISTERED_EMAILS") {
    return "SĐT khách cung cấp khớp với nhiều email đăng ký khác nhau; cần Admin kiểm tra để xác định đúng hồ sơ/email.";
  }
...
export function phanLoaiCustomerStatus(data, email, phone = null) {
  ...
  if (data.found === false) {
    if (data.ambiguous === true && data.reason === "multiple_registered_emails") {
      const classification = "AMBIGUOUS_REGISTERED_EMAILS";
      return {
        outcome: "NEEDS_ADMIN",
        classification,
