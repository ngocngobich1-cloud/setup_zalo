...
async function goiWebsiteRaw(config, endpoint = null) {
  if (!configDayDu(config)) nem("WEBSITE_CONFIG_INCOMPLETE");
  const url = validateWebsiteApiUrl(endpoint || config.apiUrl);
...
async function goiWebsite(config) {
  return validateWebsiteResponse(await goiWebsiteRaw(config));
}
...
/**
 * Full-fidelity snapshot path for local Data sync. This deliberately bypasses
 * validateWebsiteResponse(), whose legacy preview contract requires both phone
 * and email and projects every row down to those two fields.
 */
export async function fetchWebsiteCustomersRaw() {
  const config = await docConfigNoiBo();
  if (!configDayDu(config) || !config.verified) nem("WEBSITE_NOT_CONNECTED");
  return goiWebsiteRaw(config);
}

/** Customer Bot path: one status request with optional phone and email identifiers. */
export async function fetchWebsiteCustomerStatus(identifiers) {
  const { email, phone } = typeof identifiers === "string"
    ? { email: identifiers, phone: null }
    : (identifiers || {});
  const config = await docConfigNoiBo();
  if (!configDayDu(config)) nem("WEBSITE_CONFIG_INCOMPLETE");
  if (!config.verified) nem("WEBSITE_NOT_CONNECTED");

  validateWebsiteApiUrl(config.apiUrl);
  const endpoint = new URL(
    "/api/vizen/customer-status",
    new URL(config.apiUrl).origin
  );
  if (phone) endpoint.searchParams.set("phone", phone);
  if (email) endpoint.searchParams.set("email", email);
  return goiWebsiteRaw(config, endpoint);
}
