...
import { runBackgroundAiTask } from "./global-ai-limiter.js";
 ...
 */
export async function ducKetNeuDenLuot(
  config,
  messageObj,
  ...
  try {
    ...
    const background = await runBackgroundAiTask(async () => {
      dangDucKet.add(khoaDucKet);
      try {
        const summarize = () => ducKetHoSo(
          config,
          { uid, ten, threadId: messageObj.threadId },
          ownerUid
        );
        return typeof withCredentialRead === "function"
          ? await withCredentialRead(summarize)
          : await summarize();
      } finally {
    ...
    if (!background.ran) {
      ...
      await addLog({
        event: "background_summary_skipped",
        level: "warn",
        summary: "Bỏ qua lượt đúc kết hồ sơ vì AI đang có tải foreground.",
        detail: { ownerUid, uid, consecutiveSkips: consecutiveSummarySkips },
      }).catch(() => {});
      if (consecutiveSummarySkips === 10) {
        console.warn("[ho-so] background_summary_skip_warning: đã bỏ qua 10 lượt liên tiếp.");
        await addLog({
          event: "background_summary_skip_warning",
          level: "warn",
          summary: "Đúc kết hồ sơ đã bị bỏ qua 10 lượt liên tiếp vì tải AI.",
          detail: { ownerUid, uid, consecutiveSkips: consecutiveSummarySkips },
        }).catch(() => {});
      }
      return null;
