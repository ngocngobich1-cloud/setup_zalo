...
export async function bocPrompt(sessionId, messageObj, userMessage) {
  ...
  if (uid && sessionId) {
    ...
    if (!daNap.has(uid)) {
      const hoSo = await getCustomerMemory(layChuTaiKhoan(), uid).catch(() => null);
  ...
  let khoiChiDan = "";
  if (laChatRieng && uid) {
    const chiDan = await getOwnerInstruction(layChuTaiKhoan(), uid).catch(() => "");
    if (chiDan.trim()) {
      khoiChiDan =
...
export async function ducKetHoSo(config, { uid, ten, threadId }) {
  const lichSu = await buildRecentHistory(layChuTaiKhoan(), threadId, null, {
    soTin: DUC_KET_SO_TIN,
    maxChars: DUC_KET_MAX_CHARS,
  });
  if (!lichSu) return null;

  const cu = await getCustomerMemory(layChuTaiKhoan(), uid);
  ...
  await resetCustomerTurns(layChuTaiKhoan(), uid);
...
export async function ducKetNeuDenLuot(config, messageObj) {
  ...
  try {
    const soLuot = await bumpCustomerTurns(layChuTaiKhoan(), uid, ten);
