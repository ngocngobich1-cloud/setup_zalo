...
export function capHinhScheduler({ gui, thongBaoAdmin, layChu }) {
  guiTin = gui;
  baoAdmin = thongBaoAdmin;
  if (layChu) layChuTaiKhoan = layChu;
}

/** Uid tai khoan Zalo dang dang nhap; zalo-service/server tiem vao. */
...
async function chayMotVong() {
  ...
  try {
    ...
    // Chua dang nhap Zalo -> khong co lich nao den han. Gui bang phien cua tai
    // khoan khac la gui nham nguoi, nen tha bo lo con hon gui sai.
    const chu = layChuTaiKhoan();
    if (!chu) return;
    const denHan = await layLichDenHan(chu, bayGio);
