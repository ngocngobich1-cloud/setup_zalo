  ...
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
        ...
        WHERE generation_key = ? AND status <> 'DONE'`,
...
export async function requeueStaleDurableGeneration(generationKey, { now = Date.now() } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const delivery = await txGet(
      `SELECT COUNT(*) AS started FROM outbound_outbox
        WHERE generation_key = ? AND status IN ('SENDING','SENT')`,
      [String(generationKey)]
    );
    if (Number(delivery?.started || 0) > 0) {
    ...
    const abandoned = await txRun(
      `UPDATE outbound_outbox
        ...
        WHERE generation_key = ? AND status <> 'SENT'`,
...
export async function markOutboundFailure(id, { status, errorCode, nextAttemptAt = 0, now = Date.now() }) {
  const nextStatus = status === "RETRY" ? "RETRY" : "BLOCKED";
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const row = await txGet("SELECT status FROM outbound_outbox WHERE id = ?", [Number(id)]);
    const result = await txRun(
      `UPDATE outbound_outbox
    ...
    return { changed: result.changes > 0, enteredBlocked: result.changes > 0 && row?.status !== "BLOCKED" && nextStatus === "BLOCKED" };
  });
}
...
export async function recoverExpiredOutboundIntents(now = Date.now()) {
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE outbound_outbox
