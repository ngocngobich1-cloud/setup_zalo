  ...
  await run(
    `
      INSERT INTO account_config (owner_uid, bot_enabled, allowed_group_id, allowed_sender_ids,
                                  otp_zalo_thread_id, otp_zalo_label, admin_zalo_uid, admin_zalo_label, updated_at)
      VALUES ($ownerUid, COALESCE($botEnabled,0), COALESCE($groupId,''), COALESCE($senderIds,'[]'),
              COALESCE($otpThread,''), COALESCE($otpLabel,''), COALESCE($adminUid,''), COALESCE($adminLabel,''), $now)
...
export async function saveAiChatConfig({
  ...
  await run(
    `UPDATE ai_chat_config
     SET allowed_topics = ?, role_tone = ?, allowed_group_id = ?, allowed_sender_ids = ?,
