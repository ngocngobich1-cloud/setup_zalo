...
import { migrateData01WebsiteStore } from "./migrations/data-01-website-store.js";
import { migrateP9ZaloUidProfile } from "./migrations/p9-zalo-uid-profile.js";
import { normalizeFailoverEnabled, normalizeFallbackCapabilities } from "./ai-model-router.js";

const { dataDir: DATA_DIR, dbPath: DB_PATH } = resolveDatabasePaths();
let db;
let sharedTransactionChain = Promise.resolve();
...
function withSharedConnectionTransaction(operation) {
  const execute = async () => {
    let transactionOpen = false;
    try {
      await run("BEGIN IMMEDIATE");
      transactionOpen = true;
      const result = await operation({ run, all, get });
      await run("COMMIT");
  ...
  const result = sharedTransactionChain.then(execute, execute);
  sharedTransactionChain = result.catch(() => {});
  return result;
}

// Narrow persistence boundary for lib/website-data.js. Keeping the transaction
// here ensures every operation uses the same initialized SQLite connection and
// avoids opening a second connection or assuming runtime foreign-key behavior.
export const websiteDataRun = (sql, params = []) => run(sql, params);
export const websiteDataAll = (sql, params = []) => all(sql, params);
export const websiteDataGet = (sql, params = []) => get(sql, params);

export async function withWebsiteDataTransaction(operation) {
  return withSharedConnectionTransaction(operation);
}
...
const FIRST_INVITE_BACKFILL_MARKER = "first_invite_backfill_done";

/**
 * Danh dau moi owner da ton tai truoc ranh gioi Part 1 la owner lich su.
 *
 * Marker nay co y doc/ghi bang SQL tho. app_secrets la kho bi mat ma hoa, trong
 * khi y nghia cua marker chi la SU TON TAI CUA ROW; dung getAppSecret() o day co
 * the nham mot row khong giai ma duoc thanh marker chua ton tai va chay lai.
 */
export async function backfillFirstInviteSeenOnce() {
  let transactionOpen = false;
  try {
    await run("BEGIN IMMEDIATE");
    transactionOpen = true;

    const marker = await get(`SELECT key FROM app_secrets WHERE key = ?`, [FIRST_INVITE_BACKFILL_MARKER]);
    if (marker) {
      await run("COMMIT");
      transactionOpen = false;
      return { applied: false, ownersUpdated: 0 };
    }

    const rows = await all(`SELECT owner_uid, setup_data FROM account_config`);
    for (const row of rows) {
      const raw = row.setup_data;
      let setupData = {};
      if (raw !== null && raw !== undefined && String(raw).trim() !== "") {
        try {
          setupData = JSON.parse(String(raw));
        } catch (error) {
          throw new Error(
            `BLOCKED_FIRST_INVITE_BACKFILL_DATA_INVALID: owner_uid=${row.owner_uid}: ${error.message}`
          );
        }
        if (!setupData || typeof setupData !== "object" || Array.isArray(setupData)) {
          throw new Error(
            `BLOCKED_FIRST_INVITE_BACKFILL_DATA_INVALID: owner_uid=${row.owner_uid}: setup_data khong phai JSON object`
          );
        }
      }
      await run(`UPDATE account_config SET setup_data = ? WHERE owner_uid = ?`, [
        JSON.stringify({ ...setupData, firstSetupInviteSeen: true }),
        String(row.owner_uid),
      ]);
    }

    // Marker PHAI duoc ghi sau cung, khi tat ca owner lich su da cap nhat xong.
    await run(`INSERT OR IGNORE INTO app_secrets (key, value) VALUES (?, ?)`, [
      FIRST_INVITE_BACKFILL_MARKER,
      "1",
    ]);
    await run("COMMIT");
    transactionOpen = false;
    return { applied: true, ownersUpdated: rows.length };
  } catch (error) {
    if (transactionOpen) await run("ROLLBACK").catch(() => {});
    throw error;
  }
...
export async function initDb() {
  ...
  await addColumnIfMissing("ALTER TABLE threads ADD COLUMN bot_enabled INTEGER NOT NULL DEFAULT 1");
  ...
  // P1 durable inbound admission. receipt_key intentionally includes owner,
  // conversation and provider message id: a Zalo msgId is not globally unique.
  await run(`
    CREATE TABLE IF NOT EXISTS durable_message_jobs (
  ...
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_durable_jobs_runnable
    ON durable_message_jobs(status, next_attempt_at)
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_durable_jobs_lease
    ON durable_message_jobs(lease_until)
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_durable_jobs_conversation
    ON durable_message_jobs(account_id, conversation_id, admitted_at)
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_durable_jobs_generation
    ON durable_message_jobs(generation_key)
  `);

  // P1 durable outbound intent. generation_key owns the N:1 relationship;
  // required_outbound_count closes the crash gap between multi-bubble intents.
  await run(`
    CREATE TABLE IF NOT EXISTS outbound_outbox (
      ...
      outbound_slot INTEGER NOT NULL,
      required_outbound_count INTEGER NOT NULL DEFAULT 1,
  ...
  await run(`
    ...
    ON outbound_outbox(status, next_attempt_at)
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_outbox_lease
    ON outbound_outbox(lease_until)
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_outbox_generation
    ON outbound_outbox(generation_key)
  `);

  // Admin Clarification V1: additive, owner-scoped and restart-safe. waiting_slot
  // is NULL outside the waiting window so a partial UNIQUE index can enforce
  // exactly one live request per owner + private customer conversation.
  await run(`
    CREATE TABLE IF NOT EXISTS admin_clarifications (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      owner_uid TEXT NOT NULL,
      customer_thread_id TEXT NOT NULL,
      customer_thread_type INTEGER NOT NULL DEFAULT 0,
      requester_uid TEXT,
      requester_name TEXT,
      correlation_token TEXT NOT NULL,
      status TEXT NOT NULL,
      waiting_slot INTEGER,
      opened_context_boundary TEXT,
      latest_context_boundary TEXT,
      frozen_context_boundary TEXT,
      admin_message_id TEXT,
      admin_sender_uid TEXT,
      admin_answer TEXT,
      admin_replied_at INTEGER,
      expires_at INTEGER NOT NULL,
      admin_notification_state TEXT NOT NULL DEFAULT 'PENDING',
      admin_notification_id TEXT,
      customer_ack_state TEXT NOT NULL DEFAULT 'PENDING',
      customer_ack_id TEXT,
      customer_final_reply_text TEXT,
      customer_final_reply_id TEXT,
      ...
      updated_at INTEGER NOT NULL,
      closed_at INTEGER,
      UNIQUE(owner_uid, correlation_token)
    )
  `);
  await run(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_admin_clarification_one_waiting
    ON admin_clarifications(owner_uid, customer_thread_id)
    WHERE waiting_slot = 1
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_admin_clarification_recovery
    ON admin_clarifications(owner_uid, status, updated_at)
  `);
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS pdf_automation_rules (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      owner_uid TEXT NOT NULL,
      ...
      created_at INTEGER NOT NULL,
      updated_at INTEGER NOT NULL,
      UNIQUE(owner_uid, keyword_norm)
    )
  `);
  await run(`
    CREATE INDEX IF NOT EXISTS idx_pdf_automation_owner_enabled
    ON pdf_automation_rules(owner_uid, enabled, id)
  `);
  ...
  // Slice 1 Website Data: additive, atomic and idempotent persistence only.
  await migrateData01WebsiteStore({ run, all, get });

  // Credential AI thuoc ve dung Zalo owner. Bang nay duoc tao SAU P9 vi P9
  // moi la ranh gioi da xac lap danh tinh owner canonical. Khong backfill key
  // global cu: owner khong co row phai bat dau voi tap key rong.
  await run(`
    CREATE TABLE IF NOT EXISTS owner_provider_credentials (
      owner_uid   TEXT    NOT NULL,
      provider_id TEXT    NOT NULL,
      secret_enc  TEXT    NOT NULL,
      created_at  INTEGER NOT NULL,
      updated_at  INTEGER NOT NULL,
      PRIMARY KEY (owner_uid, provider_id)
    )
  `);
  ...
  await addColumnIfMissing(
    "ALTER TABLE ai_chat_config ADD COLUMN opencode_fallback_model TEXT NOT NULL DEFAULT ''"
  );
  // Cac cot routing la additive SAU P9. Future P9-style rebuild phai bao toan
  // moi additive ai_chat_config column, khong dua chung vao column list P9 cu.
  await addColumnIfMissing(
    "ALTER TABLE ai_chat_config ADD COLUMN opencode_fallback_capabilities TEXT NOT NULL DEFAULT '[]'"
  );
  await addColumnIfMissing(
    "ALTER TABLE ai_chat_config ADD COLUMN opencode_failover_enabled INTEGER NOT NULL DEFAULT 0"
  );
  await backfillFirstInviteSeenOnce();
...
export async function listThreads(ownerUid, { recentOnly = true } = {}) {
  ...
  const rows = await all(
    `
    SELECT local_id, owner_uid, remote_thread_id, thread_type, title, avatar, bot_enabled,
...
/**
 * Luu cau hinh Bot cua DUNG owner + remote thread. UPDATE co dieu kien cho biet
 * day la transition that hay no-op ma khong can doc truoc roi moi ghi.
 */
export async function setThreadBotEnabled(ownerUid, remoteThreadId, enabled) {
  if (!ownerUid) return null;
  if (typeof enabled !== "boolean") throw new Error("enabled phai la boolean.");
  const nextValue = enabled ? 1 : 0;
  const result = await run(
    `UPDATE threads SET bot_enabled = ?
     WHERE owner_uid = ? AND remote_thread_id = ? AND bot_enabled <> ?`,
    [nextValue, String(ownerUid), String(remoteThreadId), nextValue]
  );

  // Transition path khong await them: caller co the tang thread epoch ngay khi
  // callback UPDATE tra ve, truoc khi bat ky automatic continuation nao gui di.
  if (result.changes > 0) {
    return { changed: true, previousEnabled: !enabled, enabled };
  }

  const thread = await getThread(ownerUid, remoteThreadId);
  if (!thread) return null;
  return {
    changed: false,
    previousEnabled: thread.botEnabled,
    enabled: thread.botEnabled,
    thread,
  };
}
...
function durableReceiptKey(accountId, conversationId, sourceMessageId) {
...
function withDurableWrite(operation) {
  return withSharedConnectionTransaction(operation);
}

export const MAX_DEFAULT_DELIVERY_ATTEMPTS = 6;
export const MAX_CREDENTIAL_CONTROL_ATTEMPTS = 2;
export const MAX_REPLAY_AGE_MS = 1_800_000;
const TERMINAL_NEXT_ATTEMPT_AT = Number.MAX_SAFE_INTEGER;

async function generationDeliveryMayHaveOccurredTx(txGet, generationKey) {
  const delivery = await txGet(
    `SELECT COUNT(*) AS possible FROM outbound_outbox
      WHERE generation_key = ?
        AND (status IN ('SENDING','SENT') OR last_error_code = 'LEASE_EXPIRED')`,
    [String(generationKey)]
  );
  return Number(delivery?.possible || 0) > 0;
}

async function terminalizeOutboundGenerationTx({ txRun, row, errorCode, now, incrementCurrentAttempt }) {
  const terminalCode = String(errorCode || "DELIVERY_ATTEMPTS_EXHAUSTED");
  const outbox = await txRun(
    `UPDATE outbound_outbox
        SET status = 'FAILED_TERMINAL', lease_until = NULL,
            next_attempt_at = ?,
            attempt_count = attempt_count + CASE WHEN id = ? AND ? = 1 THEN 1 ELSE 0 END,
            last_error_code = CASE WHEN id = ? THEN ? ELSE COALESCE(last_error_code, 'GENERATION_FAILED_TERMINAL') END,
            updated_at = ?
      WHERE generation_key = ?
        AND status IN ('PENDING','SENDING','RETRY','BLOCKED')`,
    [TERMINAL_NEXT_ATTEMPT_AT, Number(row.id), incrementCurrentAttempt ? 1 : 0,
      Number(row.id), terminalCode, now, String(row.generation_key)]
  );
  const jobs = await txRun(
    `UPDATE durable_message_jobs
        SET status = 'FAILED_TERMINAL', lease_until = NULL,
            next_attempt_at = ?, last_error_code = ?, updated_at = ?
      WHERE generation_key = ?
        AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
    [TERMINAL_NEXT_ATTEMPT_AT, terminalCode, now, String(row.generation_key)]
  );
  return {
    ...
    outboxRows: Number(outbox.changes || 0),
    jobRows: Number(jobs.changes || 0),
  };
}

function mapDurableJob(row) {
  if (!row) return null;
  return {
    id: Number(row.id),
    accountId: row.account_id,
...
function mapOutbox(row) {
  ...
  return {
    ...
    outboundKind: row.outbound_kind,
    outboundSlot: Number(row.outbound_slot),
    requiredOutboundCount: Number(row.required_outbound_count || 1),
...
export function composeDurableReceiptKey(accountId, conversationId, sourceMessageId) {
  return durableReceiptKey(accountId, conversationId, sourceMessageId);
}

export async function admitDurableMessageJob({ accountId, conversationId, sourceMessageId, admittedAt = Date.now() }) {
  const account = String(accountId || "").trim();
  const conversation = String(conversationId || "").trim();
  const source = String(sourceMessageId || "").trim();
  if (!account || !conversation || !source) throw new Error("Durable admission thieu identity.");
  const receiptKey = durableReceiptKey(account, conversation, source);
  const now = Date.now();
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const inserted = await txRun(
      `INSERT OR IGNORE INTO durable_message_jobs
        (account_id, conversation_id, receipt_key, source_message_id, status,
         attempt_count, next_attempt_at, admitted_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, 'PENDING', 0, 0, ?, ?, ?)`,
      [account, conversation, receiptKey, source, Number(admittedAt) || now, now, now]
    );
    const row = mapDurableJob(await txGet(
      "SELECT * FROM durable_message_jobs WHERE receipt_key = ?",
      [receiptKey]
    ));
    return row ? { ...row, created: inserted.changes > 0 } : null;
  });
}

export async function getDurableMessageJob(id) {
  return mapDurableJob(await get("SELECT * FROM durable_message_jobs WHERE id = ?", [Number(id)]));
}

export async function listDurableMessageJobs({ accountId = null, conversationId = null } = {}) {
  const clauses = [];
  const params = [];
  if (accountId != null) { clauses.push("account_id = ?"); params.push(String(accountId)); }
  if (conversationId != null) { clauses.push("conversation_id = ?"); params.push(String(conversationId)); }
  const rows = await all(
    `SELECT * FROM durable_message_jobs${clauses.length ? ` WHERE ${clauses.join(" AND ")}` : ""}
     ORDER BY id`,
    params
  );
  return rows.map(mapDurableJob);
}

export async function getDurableJobMessage(id) {
  const row = await get(
    `SELECT m.*, t.remote_thread_id, t.thread_type
       FROM durable_message_jobs j
       INNER JOIN threads t
         ON t.owner_uid = j.account_id AND t.remote_thread_id = j.conversation_id
       INNER JOIN messages m
         ON m.thread_id = t.local_id AND m.id = j.source_message_id
      WHERE j.id = ?`,
    [Number(id)]
  );
  return row ? { ...mapMessage(row), threadType: Number(row.thread_type) } : null;
}

export async function claimDurableMessageJob(id, { now = Date.now(), leaseMs = 90_000 } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const result = await txRun(
      ...
      [now + leaseMs, now, Number(id), now, now]
    );
    if (!result.changes) return null;
    return mapDurableJob(await txGet("SELECT * FROM durable_message_jobs WHERE id = ?", [Number(id)]));
  });
}

export async function claimNextDurableMessageJobs({ now = Date.now(), leaseMs = 90_000, limit = 100 } = {}) {
  return withDurableWrite(async ({ run: txRun, all: txAll, get: txGet }) => {
    const first = await txGet(
      `SELECT account_id, conversation_id FROM durable_message_jobs
    ...
    if (!first) return [];
    const candidates = await txAll(
      `SELECT id FROM durable_message_jobs
        WHERE account_id = ? AND conversation_id = ?
          AND status IN ('PENDING','RETRY','BLOCKED')
          AND next_attempt_at <= ?
          AND (lease_until IS NULL OR lease_until <= ?)
        ORDER BY admitted_at, id LIMIT ?`,
      [first.account_id, first.conversation_id, now, now, Number(limit)]
    );
    const claimed = [];
    for (const candidate of candidates) {
      const result = await txRun(
        `UPDATE durable_message_jobs
            SET status = 'PROCESSING', lease_until = ?, updated_at = ?
          WHERE id = ? AND status IN ('PENDING','RETRY','BLOCKED')
            AND next_attempt_at <= ? AND (lease_until IS NULL OR lease_until <= ?)`,
        [now + leaseMs, now, candidate.id, now, now]
      );
      if (result.changes) claimed.push(candidate.id);
    }
    if (!claimed.length) return [];
    const placeholders = claimed.map(() => "?").join(",");
    return (await txAll(
      `SELECT * FROM durable_message_jobs WHERE id IN (${placeholders}) ORDER BY admitted_at, id`,
      claimed
    )).map(mapDurableJob);
  });
}

export async function assignDurableGeneration(jobIds, generationKey, { now = Date.now(), leaseMs = 300_000 } = {}) {
  const ids = [...new Set((jobIds || []).map(Number).filter(Number.isInteger))];
  if (!ids.length || !generationKey) return [];
  return withDurableWrite(async ({ run: txRun, all: txAll }) => {
    const placeholders = ids.map(() => "?").join(",");
    await txRun(
      `UPDATE durable_message_jobs
          SET generation_key = ?, status = 'PROCESSING', lease_until = ?, updated_at = ?
        WHERE id IN (${placeholders})
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [String(generationKey), now + leaseMs, now, ...ids]
    );
    return (await txAll(
      `SELECT * FROM durable_message_jobs WHERE id IN (${placeholders}) ORDER BY id`, ids
    )).map(mapDurableJob);
  });
}

/**
 * Gia hạn đúng các member còn thuộc generation. Số row thay đổi là authority
 * proof: thiếu dù chỉ một member nghĩa là generation đã mất quyền tiếp tục.
 */
export async function giaHanDurableGeneration(generationKey, leaseMs = 300_000, { now = Date.now() } = {}) {
  if (!generationKey || !Number.isFinite(Number(leaseMs)) || Number(leaseMs) <= 0) return 0;
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
        ...
        WHERE generation_key = ?
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')
          AND admitted_at > ?`,
      [now + Number(leaseMs), now, String(generationKey), now - MAX_REPLAY_AGE_MS]
    );
    return Number(result?.changes || 0);
  });
}

export async function requeueDurableGeneration(generationKey, { now = Date.now(), incrementAttempt = true, errorCode = null } = {}) {
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
          SET generation_key = NULL, status = 'PENDING', lease_until = NULL,
              next_attempt_at = 0, attempt_count = attempt_count + ?,
              last_error_code = ?, updated_at = ?
        WHERE generation_key = ?
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [incrementAttempt ? 1 : 0, errorCode, now, String(generationKey)]
    );
    return result.changes;
  });
}

export async function requeueStaleDurableGeneration(generationKey, { now = Date.now() } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    if (await generationDeliveryMayHaveOccurredTx(txGet, generationKey)) {
      return { requeued: 0, abandoned: 0, deliveryStarted: true };
    }
    const abandoned = await txRun(
      `UPDATE outbound_outbox
          SET status = 'BLOCKED', lease_until = NULL,
              next_attempt_at = 9007199254740991,
              last_error_code = 'GENERATION_STALE', updated_at = ?
        WHERE generation_key = ? AND status IN ('PENDING','RETRY','BLOCKED')`,
      [now, String(generationKey)]
    );
    const requeued = await txRun(
      `UPDATE durable_message_jobs
          SET generation_key = NULL, status = 'PENDING', lease_until = NULL,
              next_attempt_at = 0, attempt_count = attempt_count + 1,
              last_error_code = 'GENERATION_STALE', updated_at = ?
        WHERE generation_key = ?
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [now, String(generationKey)]
    ...
    return {
      ...
      abandoned: Number(abandoned.changes || 0),
      deliveryStarted: false,
    };
  });
}

export async function markDurableJobsBlocked(jobIds, errorCode, { now = Date.now(), nextAttemptAt = now + 30_000 } = {}) {
  const ids = [...new Set((jobIds || []).map(Number).filter(Number.isInteger))];
  ...
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
          SET generation_key = NULL, status = 'BLOCKED', lease_until = NULL,
              next_attempt_at = ?, last_error_code = ?, updated_at = ?
        WHERE id IN (${placeholders})
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [nextAttemptAt, String(errorCode || "AUTHORITY_UNAVAILABLE"), now, ...ids]
    );
    return result.changes;
  });
}

export async function markDurableGenerationFailure(generationKey, { status, errorCode, nextAttemptAt = 0, now = Date.now() }) {
  const nextStatus = status === "RETRY" ? "RETRY" : "BLOCKED";
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
        ...
        WHERE generation_key = ?
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [nextStatus, Number(nextAttemptAt) || 0, String(errorCode || "UNKNOWN_PROVIDER_ERROR"), now, String(generationKey)]
    );
    return result.changes;
  });
}

export async function settleDurableGeneration(generationKey, { allowNoOutbound = false, now = Date.now() } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const terminalMembers = await txGet(
      `SELECT COUNT(*) AS terminal FROM durable_message_jobs
        WHERE generation_key = ? AND status IN ('FAILED_TERMINAL','EXPIRED')`,
      [String(generationKey)]
    );
    if (Number(terminalMembers?.terminal || 0) > 0) return false;
    const stats = await txGet(
      `SELECT COUNT(*) AS total,
              SUM(CASE WHEN status = 'SENT' THEN 1 ELSE 0 END) AS sent,
              MAX(required_outbound_count) AS required
         FROM outbound_outbox WHERE generation_key = ?`,
      [String(generationKey)]
    );
    const total = Number(stats?.total || 0);
    const sent = Number(stats?.sent || 0);
    const required = Number(stats?.required || 0);
    if (!allowNoOutbound && (total === 0 || total < required || sent !== total)) {
      await txRun(
        `UPDATE durable_message_jobs SET status = 'WAITING_OUTBOX', lease_until = NULL, updated_at = ?
          WHERE generation_key = ?
            AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
        [now, String(generationKey)]
      );
    ...
    await txRun(
      `UPDATE durable_message_jobs
        SET status = 'DONE', lease_until = NULL, next_attempt_at = 0,
              last_error_code = NULL, completed_at = ?, updated_at = ?
        WHERE generation_key = ?
          AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')`,
      [now, now, String(generationKey)]
    );
    return true;
  });
}

export async function recoverExpiredDurableJobs(now = Date.now()) {
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE durable_message_jobs
          SET status = 'PENDING', generation_key = NULL, lease_until = NULL,
              next_attempt_at = 0, updated_at = ?
        WHERE status = 'PROCESSING' AND lease_until IS NOT NULL AND lease_until <= ?
          AND (generation_key IS NULL OR
            (SELECT COUNT(*) FROM outbound_outbox o
              WHERE o.generation_key = durable_message_jobs.generation_key)
            < COALESCE((SELECT MAX(o.required_outbound_count) FROM outbound_outbox o
              WHERE o.generation_key = durable_message_jobs.generation_key), 1)
          )`,
      [now, now]
    );
    return result.changes;
...
export async function expireDurableWork({ now = Date.now(), limit = 100 } = {}) {
  const sweepNow = Number(now);
  const boundedLimit = Math.max(1, Math.min(500, Number(limit) || 100));
  const cutoff = sweepNow - MAX_REPLAY_AGE_MS;
  return withDurableWrite(async ({ run: txRun, all: txAll, get: txGet }) => {
    const terminalEvents = [];
    let expiredJobs = 0;
    let expiredOutbox = 0;

    const standalone = await txAll(
    ...
    for (const job of standalone) {
      const changed = await txRun(
        `UPDATE durable_message_jobs
            SET status = 'EXPIRED', lease_until = NULL, next_attempt_at = ?,
                last_error_code = 'REPLAY_AGE_EXPIRED', updated_at = ?
          WHERE id = ? AND generation_key IS NULL
            AND status IN ('PENDING','PROCESSING','BLOCKED','RETRY','WAITING_OUTBOX')
            AND admitted_at <= ?`,
        [TERMINAL_NEXT_ATTEMPT_AT, sweepNow, Number(job.id), cutoff]
      );
      if (!changed.changes) continue;
      expiredJobs += Number(changed.changes || 0);
      terminalEvents.push({
        generationKey: null,
        accountId: job.account_id,
        conversationId: job.conversation_id,
        terminalStatus: "EXPIRED",
        errorCode: "REPLAY_AGE_EXPIRED",
        jobRows: Number(changed.changes || 0),
        outboxRows: 0,
      });
    }

    const remaining = Math.max(0, boundedLimit - standalone.length);
    const generations = remaining > 0 ? await txAll(
    ...
    for (const candidate of generations) {
      if (await generationDeliveryMayHaveOccurredTx(txGet, candidate.generation_key)) continue;
      const outbox = await txRun(
        `UPDATE outbound_outbox
            SET status = 'EXPIRED', lease_until = NULL, next_attempt_at = ?,
                last_error_code = 'REPLAY_AGE_EXPIRED', updated_at = ?
          WHERE generation_key = ?
            AND status IN ('PENDING','RETRY','BLOCKED')`,
        [TERMINAL_NEXT_ATTEMPT_AT, sweepNow, String(candidate.generation_key)]
      );
      const jobs = await txRun(
        `UPDATE durable_message_jobs
            SET status = 'EXPIRED', lease_until = NULL, next_attempt_at = ?,
      ...
      const jobRows = Number(jobs.changes || 0);
      const outboxRows = Number(outbox.changes || 0);
      if (!jobRows && !outboxRows) continue;
      expiredJobs += jobRows;
      expiredOutbox += outboxRows;
      terminalEvents.push({
    ...
    return { expiredJobs, expiredOutbox, terminalEvents };
  });
}

export async function ensureOutboundIntent({ generationKey, accountId, conversationId, outboundKind, outboundSlot, requiredOutboundCount, payload, now = Date.now() }) {
  const idempotencyKey = JSON.stringify([String(generationKey), Number(outboundSlot), String(outboundKind)]);
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    await txRun(
      `INSERT OR IGNORE INTO outbound_outbox
        (generation_key, account_id, conversation_id, idempotency_key, outbound_kind,
         outbound_slot, required_outbound_count, payload_json, status, attempt_count,
         next_attempt_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', 0, 0, ?, ?)`,
      [String(generationKey), String(accountId), String(conversationId), idempotencyKey,
        String(outboundKind), Number(outboundSlot), Number(requiredOutboundCount) || 1,
        JSON.stringify(payload || {}), now, now]
    );
    return mapOutbox(await txGet("SELECT * FROM outbound_outbox WHERE idempotency_key = ?", [idempotencyKey]));
  });
}

export async function ensureOutboundIntents({ generationKey, accountId, conversationId, intents, now = Date.now() }) {
  const requiredOutboundCount = intents.length;
  if (!generationKey || !requiredOutboundCount) return [];
  return withDurableWrite(async ({ run: txRun, all: txAll }) => {
    const keys = [];
    for (const [outboundSlot, intent] of intents.entries()) {
      const idempotencyKey = JSON.stringify([
        String(generationKey), Number(outboundSlot), String(intent.outboundKind),
      ]);
      ...
      await txRun(
        `INSERT OR IGNORE INTO outbound_outbox
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PENDING', 0, 0, ?, ?)`,
        [String(generationKey), String(accountId), String(conversationId), idempotencyKey,
          String(intent.outboundKind), Number(outboundSlot), requiredOutboundCount,
          JSON.stringify(intent.payload || {}), now, now]
      );
    }
    const placeholders = keys.map(() => "?").join(",");
    return (await txAll(
      `SELECT * FROM outbound_outbox WHERE idempotency_key IN (${placeholders}) ORDER BY outbound_slot, id`,
      keys
    )).map(mapOutbox);
  });
}

export async function listOutbox({ generationKey = null } = {}) {
  const rows = generationKey == null
    ? await all("SELECT * FROM outbound_outbox ORDER BY id")
    : await all("SELECT * FROM outbound_outbox WHERE generation_key = ? ORDER BY outbound_slot, id", [String(generationKey)]);
  return rows.map(mapOutbox);
}

export async function claimOutboundIntent(id, { now = Date.now(), leaseMs = 120_000 } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const result = await txRun(
      `UPDATE outbound_outbox
    ...
    if (!result.changes) return null;
    return mapOutbox(await txGet("SELECT * FROM outbound_outbox WHERE id = ?", [Number(id)]));
  });
}

export async function claimNextOutboundIntent({ now = Date.now(), leaseMs = 120_000 } = {}) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const candidate = await txGet(
      `SELECT candidate.id FROM outbound_outbox AS candidate
        WHERE candidate.status IN ('PENDING','RETRY','BLOCKED') AND candidate.next_attempt_at <= ?
          AND (candidate.lease_until IS NULL OR candidate.lease_until <= ?)
          AND (SELECT COUNT(*) FROM outbound_outbox siblings
                WHERE siblings.generation_key = candidate.generation_key)
              >= candidate.required_outbound_count
          AND NOT EXISTS (
                SELECT 1 FROM outbound_outbox previous
                 WHERE previous.generation_key = candidate.generation_key
                   AND previous.outbound_slot < candidate.outbound_slot
                   AND previous.status <> 'SENT'
              )
        ORDER BY candidate.created_at, candidate.outbound_slot, candidate.id LIMIT 1`,
      [now, now]
    );
    if (!candidate) return null;
    const result = await txRun(
      `UPDATE outbound_outbox SET status = 'SENDING', lease_until = ?, updated_at = ?
        WHERE id = ? AND status IN ('PENDING','RETRY','BLOCKED')
          AND next_attempt_at <= ? AND (lease_until IS NULL OR lease_until <= ?)`,
      [now + leaseMs, now, candidate.id, now, now]
    );
    if (!result.changes) return null;
    return mapOutbox(await txGet("SELECT * FROM outbound_outbox WHERE id = ?", [candidate.id]));
  });
}
...
export async function markOutboundSent(
  ...
  now = Date.now(),
  { generationKey = null, nextAttemptDelayMs = 0 } = {}
) {
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const result = await txRun(
      `UPDATE outbound_outbox
          SET status = 'SENT', lease_until = NULL, provider_message_id = ?,
              last_error_code = NULL, sent_at = ?, updated_at = ?
        WHERE id = ? AND status = 'SENDING'`,
      [providerMessageId == null ? null : String(providerMessageId), now, now, Number(id)]
    );
    const delayMs = Math.max(0, Number(nextAttemptDelayMs) || 0);
    if (generationKey && delayMs) {
      const current = await txGet("SELECT status FROM outbound_outbox WHERE id = ?", [Number(id)]);
      if (current?.status === "SENT") {
        const next = await txGet(
          `SELECT id FROM outbound_outbox
            WHERE generation_key = ? AND status IN ('PENDING','RETRY','BLOCKED')
            ORDER BY outbound_slot, id LIMIT 1`,
          [String(generationKey)]
        );
        if (next) {
          await txRun(
            `UPDATE outbound_outbox
                SET next_attempt_at = MAX(next_attempt_at, ?), updated_at = ?
              WHERE id = ? AND status IN ('PENDING','RETRY','BLOCKED')`,
            [now + delayMs, now, Number(next.id)]
          );
        }
      }
    }
...
export async function markOutboundFailure(id, {
  status,
  errorCode,
  nextAttemptAt = 0,
  now = Date.now(),
  effectiveCap = MAX_DEFAULT_DELIVERY_ATTEMPTS,
} = {}) {
  const nextStatus = status === "RETRY" ? "RETRY" : "BLOCKED";
  return withDurableWrite(async ({ run: txRun, get: txGet }) => {
    const row = await txGet("SELECT * FROM outbound_outbox WHERE id = ?", [Number(id)]);
    ...
    const postUpdateAttemptCount = Number(row.attempt_count || 0) + 1;
    const cap = Math.max(1, Number(effectiveCap) || MAX_DEFAULT_DELIVERY_ATTEMPTS);
    if (postUpdateAttemptCount >= cap) {
      const terminalEvent = await terminalizeOutboundGenerationTx({
        txRun,
        row,
        errorCode,
        now,
        incrementCurrentAttempt: true,
      });
      return {
        changed: true,
        attemptCount: postUpdateAttemptCount,
    ...
    const result = await txRun(
      `UPDATE outbound_outbox
          SET status = ?, lease_until = NULL, attempt_count = attempt_count + 1,
              next_attempt_at = ?, last_error_code = ?, updated_at = ?
        WHERE id = ? AND status = 'SENDING'`,
      [nextStatus, Number(nextAttemptAt) || 0, String(errorCode || "UNKNOWN_PROVIDER_ERROR"), now, Number(id)]
    );
    return {
      changed: result.changes > 0,
      attemptCount: result.changes > 0 ? postUpdateAttemptCount : Number(row.attempt_count || 0),
      status: result.changes > 0 ? nextStatus : row.status,
      enteredBlocked: result.changes > 0 && nextStatus === "BLOCKED",
      terminalized: false,
      terminalEvent: null,
    };
  });
...
export async function releaseOutboundIntent(id, { errorCode, nextAttemptAt = Date.now() + 30_000, now = Date.now() } = {}) {
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE outbound_outbox
          SET status = 'PENDING', lease_until = NULL, next_attempt_at = ?,
              last_error_code = ?, updated_at = ?
        WHERE id = ? AND status = 'SENDING'`,
      [Number(nextAttemptAt) || now, String(errorCode || "PROVIDER_NOT_CALLED"), now, Number(id)]
    );
    return result.changes > 0;
  });
}

export async function recoverExpiredOutboundIntents(now = Date.now(), { limit = 100 } = {}) {
  const boundedLimit = Math.max(1, Math.min(500, Number(limit) || 100));
  return withDurableWrite(async ({ run: txRun, all: txAll, get: txGet }) => {
    const candidates = await txAll(
    ...
    const terminalEvents = [];
    for (const candidate of candidates) {
      const row = await txGet("SELECT * FROM outbound_outbox WHERE id = ?", [Number(candidate.id)]);
      if (!row || row.status !== "SENDING" || row.lease_until == null || Number(row.lease_until) > now) continue;
      const postUpdateAttemptCount = Number(row.attempt_count || 0) + 1;
      if (postUpdateAttemptCount >= MAX_DEFAULT_DELIVERY_ATTEMPTS) {
        terminalEvents.push(await terminalizeOutboundGenerationTx({
          txRun,
          row,
      ...
      const result = await txRun(
        `UPDATE outbound_outbox
            SET status = 'RETRY', lease_until = NULL, next_attempt_at = ?,
                attempt_count = attempt_count + 1, last_error_code = 'LEASE_EXPIRED', updated_at = ?
          WHERE id = ? AND status = 'SENDING'
            AND lease_until IS NOT NULL AND lease_until <= ?`,
        [now, now, Number(row.id), now]
      );
      recovered += Number(result.changes || 0);
    }
    return { recovered, terminalEvents };
  });
}

export async function abandonOutboundGeneration(generationKey, reason = "GENERATION_STALE", now = Date.now()) {
  return withDurableWrite(async ({ run: txRun }) => {
    const result = await txRun(
      `UPDATE outbound_outbox
          SET status = 'BLOCKED', lease_until = NULL,
              ...
              last_error_code = ?, updated_at = ?
        WHERE generation_key = ? AND status IN ('PENDING','RETRY','BLOCKED')`,
      [String(reason), now, String(generationKey)]
    );
    return result.changes;
  });
}
...
export async function insertMessage(ownerUid, message) {
  ...
  // Ban ghi da ton tai. Truoc day den day la het: OR IGNORE nuot lang le ban
  // sau, nen tin tu gui (rawJson = null, ghi ngay luc gui) khong bao gio nhan
  // duoc danh tinh that khi Zalo doi lai tin do. Mat cliMsgId/uidFrom la mat
  // luon kha nang tha cam xuc, thu hoi, xoa va bao da xem cho chinh tin ay.
  if (Number(ketQua?.changes || 0) === 0) {
    const daBoSung = await boSungDanhTinhTin(localId, message);
    return { changes: 0, lastID: ketQua?.lastID ?? 0, enriched: daBoSung };
  }
  return ketQua;
}

/** Cac cot chi duoc DIEN VAO khi dang trong, khong bao gio bi ghi de. */
const COT_BO_SUNG_DUOC = ["sender_id", "sender_name", "msg_type"];

/**
 * Chi BO SUNG phan danh tinh do Zalo cap, khong dung vao noi dung nguoi dung
 * thay: content, ts, is_self giu nguyen ban canonical dau tien.
 *
 * Khong tao dong moi, khong ban socket - nguoi goi van thay changes === 0 nen
 * duong ghi cu khong doi hanh vi.
 */
async function boSungDanhTinhTin(localId, message) {
  const danhTinhMoi = rutDanhTinhProvider(message.rawJson);
  const cu = await get(
    "SELECT sender_id, sender_name, msg_type, raw_json FROM messages WHERE thread_id = ? AND id = ?",
  ...
  const dat = [];
  const thamSo = [];
  // raw_json chi duoc ghi khi ban CU chua co danh tinh ma ban MOI thi co: day la
  // dung mot huong (ngheo -> giau), khong bao gio lam mat danh tinh da co.
  if (danhTinhMoi && !rutDanhTinhProvider(cu.raw_json ? safeJson(cu.raw_json) : null)) {
    dat.push("raw_json = ?");
    thamSo.push(JSON.stringify(message.rawJson));
  }
  for (const cot of COT_BO_SUNG_DUOC) {
    const giaTriMoi = cot === "sender_id"
      ? message.senderId
      : cot === "sender_name"
        ? message.senderName
        : message.msgType;
    if (cu[cot] === null && giaTriMoi !== null && giaTriMoi !== undefined) {
      dat.push(`${cot} = ?`);
      thamSo.push(giaTriMoi);
    }
  }
  if (!dat.length) return false;

  thamSo.push(localId, String(message.id));
  await run(`UPDATE messages SET ${dat.join(", ")} WHERE thread_id = ? AND id = ?`, thamSo);
  return true;
}

/**
 * Rut danh tinh Zalo tu rawJson da luu. Thieu mot trong ba la KHONG du de goi
 * provider - tra null chu khong bia not phan con lai.
 */
export function rutDanhTinhProvider(rawJson) {
  const dt = rawJson?.data;
  if (!dt) return null;
  const msgId = dt.msgId === null || dt.msgId === undefined ? "" : String(dt.msgId);
  const cliMsgId = dt.cliMsgId === null || dt.cliMsgId === undefined ? "" : String(dt.cliMsgId);
  const uidFrom = dt.uidFrom === null || dt.uidFrom === undefined ? "" : String(dt.uidFrom);
  if (!msgId || !cliMsgId || !uidFrom) return null;
  return {
    msgId,
    cliMsgId,
    uidFrom,
    idTo: dt.idTo === null || dt.idTo === undefined ? "" : String(dt.idTo),
    msgType: dt.msgType ?? null,
    st: dt.st ?? null,
    at: dt.at ?? null,
    cmd: dt.cmd ?? null,
    ts: dt.ts ?? null,
...
/**
 * Tra ve dung mot tin nhan CO THE HANH DONG, trong pham vi so huu cua chu dang
 * dang nhap. Trinh duyet chi noi duoc "cuoc nao, tin nao"; moi thu con lai
 * (threadType, cliMsgId, uidFrom) lay tu du lieu may chu tu giu.
 *
 * @returns {Promise<{ok:true, thread:object, message:object, identity:object|null}
 *   | {ok:false, code:"NOT_FOUND"|"ACTION_IDENTITY_UNAVAILABLE"}>}
 */
export async function resolveOwnedActionMessage(ownerUid, remoteThreadId, messageId, { requireIdentity = true } = {}) {
  if (!ownerUid || !remoteThreadId || messageId === null || messageId === undefined || messageId === "") {
  ...
  const row = await get(
    `SELECT m.*, t.remote_thread_id FROM messages m
     INNER JOIN threads t ON t.local_id = m.thread_id
     WHERE t.owner_uid = ? AND t.remote_thread_id = ? AND m.id = ?`,
    [String(ownerUid), String(remoteThreadId), String(messageId)]
  );
  if (!row) return { ok: false, code: "NOT_FOUND" };

  const message = mapMessage(row);
  const identity = rutDanhTinhProvider(message.rawJson);
  // Tin cu ghi truoc khi co duong bo sung danh tinh thi DUNG LAI o day. Khong
  // goi provider de doan lai msgId/cliMsgId: doan sai la tac dong nham tin nguoi khac.
  if (requireIdentity && !identity) return { ok: false, code: "ACTION_IDENTITY_UNAVAILABLE" };

  return { ok: true, thread, message, identity };
}

/**
 * Tinh lai dong tom tat cua DUNG mot cuoc tro chuyen tu tin con lai moi nhat.
 * Dung sau khi xoa/thu hoi lam tin cuoi khong con dai dien duoc nua.
 */
export async function recomputeThreadPreview(ownerUid, remoteThreadId) {
  if (!ownerUid || !remoteThreadId) return null;
  const localId = khoaCucBo(ownerUid, remoteThreadId);
  const row = await get(
    "SELECT content, ts FROM messages WHERE thread_id = ? ORDER BY ts DESC, rowid DESC LIMIT 1",
    [localId]
  );
  await run(
    "UPDATE threads SET last_message = ?, last_message_at = ?, updated_at = ? WHERE local_id = ? AND owner_uid = ?",
    [row?.content ?? null, row?.ts ?? null, Date.now(), localId, String(ownerUid)]
  );
  return getThread(ownerUid, remoteThreadId);
}

/**
 * Doi mot tin thanh trang thai DA THU HOI ngay tai cho. Giu lai dong de khung
 * chat khong bi thung mot lo, dung nhu Zalo that.
 */
export async function markMessageRecalled(ownerUid, remoteThreadId, messageId, placeholder) {
  if (!ownerUid || !remoteThreadId) return false;
  const localId = khoaCucBo(ownerUid, remoteThreadId);
  const ketQua = await run(
    "UPDATE messages SET content = ?, msg_type = 'chat.recalled' WHERE thread_id = ? AND id = ? AND msg_type IS NOT 'chat.recalled'",
    [String(placeholder), localId, String(messageId)]
  );
  return Number(ketQua?.changes || 0) > 0;
}

/** Xoa DONG cuc bo cua mot tin. Chi goi sau khi Zalo da xac nhan xoa thanh cong. */
export async function deleteLocalMessage(ownerUid, remoteThreadId, messageId) {
  if (!ownerUid || !remoteThreadId) return false;
  const localId = khoaCucBo(ownerUid, remoteThreadId);
  const ketQua = await run("DELETE FROM messages WHERE thread_id = ? AND id = ?", [
    localId,
    String(messageId),
  ]);
  return Number(ketQua?.changes || 0) > 0;
...
function mapThread(row) {
  return {
    ...
    title: row.title,
    avatar: row.avatar,
    botEnabled: Number(row.bot_enabled) !== 0,
...
export async function getThreadMessagesThroughBoundary(ownerUid, remoteThreadId, boundaryId, limit = 200) {
  if (!ownerUid || !remoteThreadId || !boundaryId) return [];
  const rows = await all(
    `SELECT m.*, t.remote_thread_id FROM messages m
     INNER JOIN threads t ON t.local_id = m.thread_id
     WHERE t.owner_uid = ? AND t.remote_thread_id = ?
       AND m.ts <= (
         SELECT boundary.ts FROM messages boundary
         WHERE boundary.thread_id = t.local_id AND boundary.id = ?
       )
     ORDER BY m.ts DESC LIMIT ?`,
    [String(ownerUid), String(remoteThreadId), String(boundaryId), Number(limit)]
  );
  return rows.reverse().map(mapMessage);
}

function mapAdminClarification(row) {
  if (!row) return null;
  return {
    id: row.id,
    ownerUid: row.owner_uid,
    customerThreadId: row.customer_thread_id,
    customerThreadType: Number(row.customer_thread_type),
    requesterUid: row.requester_uid,
    requesterName: row.requester_name,
    correlationToken: row.correlation_token,
    status: row.status,
    waitingSlot: row.waiting_slot,
    openedContextBoundary: row.opened_context_boundary,
    latestContextBoundary: row.latest_context_boundary,
    frozenContextBoundary: row.frozen_context_boundary,
    adminMessageId: row.admin_message_id,
    adminSenderUid: row.admin_sender_uid,
    adminAnswer: row.admin_answer,
    adminRepliedAt: row.admin_replied_at,
    expiresAt: row.expires_at,
    adminNotificationState: row.admin_notification_state,
    adminNotificationId: row.admin_notification_id,
    customerAckState: row.customer_ack_state,
    customerAckId: row.customer_ack_id,
    customerFinalReplyText: row.customer_final_reply_text,
    customerFinalReplyId: row.customer_final_reply_id,
...
export async function createAdminClarification(input) {
  const now = Number(input.now || Date.now());
  try {
    const result = await run(
      `INSERT INTO admin_clarifications (
         owner_uid, customer_thread_id, customer_thread_type,
         requester_uid, requester_name, correlation_token, status, waiting_slot,
         opened_context_boundary, latest_context_boundary, expires_at,
         admin_notification_state, customer_ack_state, created_at, updated_at
       ) VALUES (?, ?, 0, ?, ?, ?, 'ADMIN_NOTIFY_PENDING', 1, ?, ?, ?, 'PENDING', 'PENDING', ?, ?)`,
      [
        String(input.ownerUid), String(input.customerThreadId),
        input.requesterUid == null ? null : String(input.requesterUid),
        input.requesterName == null ? null : String(input.requesterName),
        String(input.correlationToken),
        input.openedContextBoundary == null ? null : String(input.openedContextBoundary),
        input.openedContextBoundary == null ? null : String(input.openedContextBoundary),
        Number(input.expiresAt), now, now,
      ]
    );
    return { created: true, row: await getAdminClarificationById(result.lastID) };
  } catch (error) {
    if (!String(error?.message || "").includes("UNIQUE constraint failed")) throw error;
    const waiting = await getWaitingAdminClarification(input.ownerUid, input.customerThreadId);
    if (waiting) return { created: false, row: waiting };
    throw error;
  }
}

export async function getAdminClarificationById(id) {
  return mapAdminClarification(await get(`SELECT * FROM admin_clarifications WHERE id = ?`, [Number(id)]));
}

export async function getAdminClarificationByToken(ownerUid, correlationToken) {
  if (!ownerUid || !correlationToken) return null;
  return mapAdminClarification(await get(
    `SELECT * FROM admin_clarifications WHERE owner_uid = ? AND correlation_token = ?`,
    [String(ownerUid), String(correlationToken)]
  ));
}

export async function getWaitingAdminClarification(ownerUid, customerThreadId) {
  if (!ownerUid || !customerThreadId) return null;
  return mapAdminClarification(await get(
    `SELECT * FROM admin_clarifications
     WHERE owner_uid = ? AND customer_thread_id = ? AND waiting_slot = 1
     LIMIT 1`,
    [String(ownerUid), String(customerThreadId)]
  ));
}

export async function getRecentFailedAdminClarificationAck(ownerUid, customerThreadId, excludeId, createdSince) {
  return mapAdminClarification(await get(
    `SELECT * FROM admin_clarifications
     WHERE owner_uid = ? AND customer_thread_id = ? AND id != ?
       AND status = 'ADMIN_NOTIFY_FAILED' AND customer_ack_state IN ('SENDING', 'SENT')
       AND created_at >= ?
     ORDER BY created_at DESC LIMIT 1`,
    [String(ownerUid), String(customerThreadId), Number(excludeId), Number(createdSince)]
  ));
}

export async function touchWaitingAdminClarification(ownerUid, customerThreadId, boundary, now = Date.now()) {
  const result = await run(
    `UPDATE admin_clarifications
     SET latest_context_boundary = ?, updated_at = ?
     WHERE owner_uid = ? AND customer_thread_id = ?
       AND status = 'WAITING_ADMIN' AND waiting_slot = 1 AND expires_at > ?`,
    [String(boundary), Number(now), String(ownerUid), String(customerThreadId), Number(now)]
  );
  return Number(result.changes || 0) === 1;
}

export async function completeAdminClarificationAck(id, messageId, confirmed, now = Date.now()) {
  const result = await run(
    `UPDATE admin_clarifications
     SET customer_ack_state = ?, customer_ack_id = ?, updated_at = ?
     WHERE id = ? AND customer_ack_state = 'SENDING'`,
    [confirmed ? "SENT" : "SEND_UNKNOWN", messageId == null ? null : String(messageId), Number(now), Number(id)]
  );
  return Number(result.changes || 0) === 1;
}

export async function claimAdminClarificationAnswer({
  ownerUid, correlationToken, adminMessageId, adminSenderUid, adminAnswer, now = Date.now(),
}) {
  const result = await run(
    `UPDATE admin_clarifications
     SET status = 'ADMIN_REPLIED', waiting_slot = NULL,
         frozen_context_boundary = latest_context_boundary,
         admin_message_id = ?, admin_sender_uid = ?, admin_answer = ?,
         admin_replied_at = ?, updated_at = ?, error_stage = NULL, error_detail = NULL
     WHERE owner_uid = ? AND correlation_token = ?
       AND status = 'WAITING_ADMIN' AND waiting_slot = 1 AND expires_at > ?`,
    [
      String(adminMessageId), String(adminSenderUid), String(adminAnswer),
      Number(now), Number(now), String(ownerUid), String(correlationToken), Number(now),
    ]
  );
  if (Number(result.changes || 0) !== 1) return null;
  return getAdminClarificationByToken(ownerUid, correlationToken);
}

const ADMIN_CLARIFICATION_PATCH_COLUMNS = Object.freeze({
  adminNotificationState: "admin_notification_state",
  adminNotificationId: "admin_notification_id",
  customerAckState: "customer_ack_state",
  customerAckId: "customer_ack_id",
  customerFinalReplyText: "customer_final_reply_text",
  customerFinalReplyId: "customer_final_reply_id",
  errorStage: "error_stage",
  errorDetail: "error_detail",
  closedAt: "closed_at",
});

export async function transitionAdminClarification(id, expectedStatuses, nextStatus, patch = {}, options = {}) {
  const expected = Array.isArray(expectedStatuses) ? expectedStatuses : [expectedStatuses];
  if (!expected.length) return null;
  const sets = ["status = ?", "updated_at = ?"];
  const params = [String(nextStatus), Number(options.now || Date.now())];
  for (const [key, value] of Object.entries(patch)) {
    const column = ADMIN_CLARIFICATION_PATCH_COLUMNS[key];
    if (!column) throw new Error(`Cot admin clarification khong duoc cap nhat: ${key}`);
    sets.push(`${column} = ?`);
    params.push(value);
  }
  if (options.releaseWaitingSlot) sets.push("waiting_slot = NULL");
  if (options.incrementGenerationAttempts) sets.push("generation_attempts = generation_attempts + 1");
  const placeholders = expected.map(() => "?").join(", ");
  params.push(Number(id), ...expected.map(String));
  if (Number.isInteger(options.maxGenerationAttempts)) {
    params.push(options.maxGenerationAttempts);
  }
  const result = await run(
    `UPDATE admin_clarifications SET ${sets.join(", ")}
     WHERE id = ? AND status IN (${placeholders})${
       Number.isInteger(options.maxGenerationAttempts) ? " AND generation_attempts < ?" : ""
     }`,
    params
  );
  if (Number(result.changes || 0) !== 1) return null;
  return getAdminClarificationById(id);
}

export async function expireWaitingAdminClarifications(now = Date.now(), ownerUid = null, customerThreadId = null) {
  const where = ["status = 'WAITING_ADMIN'", "waiting_slot = 1", "expires_at <= ?"];
  const params = [Number(now)];
  if (ownerUid) { where.push("owner_uid = ?"); params.push(String(ownerUid)); }
  if (customerThreadId) { where.push("customer_thread_id = ?"); params.push(String(customerThreadId)); }
  const result = await run(
    `UPDATE admin_clarifications
     SET status = 'EXPIRED', waiting_slot = NULL, closed_at = ?, updated_at = ?
     WHERE ${where.join(" AND ")}`,
    [Number(now), Number(now), ...params]
  );
  return Number(result.changes || 0);
}

export async function listRecoverableAdminClarifications(ownerUid = null, customerThreadId = null) {
  const statuses = [
    "ADMIN_NOTIFY_PENDING", "ADMIN_NOTIFY_SENDING",
    "ADMIN_REPLIED", "BLOCKED_BY_AUTHORITY", "CUSTOMER_REPLY_GENERATING",
    "CUSTOMER_REPLY_GENERATION_FAILED", "CUSTOMER_REPLY_READY", "CUSTOMER_REPLY_SENDING",
  ];
  ...
  if (customerThreadId) { where.push("customer_thread_id = ?"); params.push(String(customerThreadId)); }
  const rows = await all(
    `SELECT * FROM admin_clarifications WHERE ${where.join(" AND ")} ORDER BY updated_at, id`,
    params
  );
  return rows.map(mapAdminClarification);
}

/** Projection-safe: chi dem rule cua owner, khong materialize command/reply_text. */
export async function getAutoReplyRuleCount(ownerUid) {
  const owner = requireAutoReplyOwnerUid(ownerUid);
  const row = await get(
  ...
  return Number(row?.rule_count) || 0;
}
...
/* --- TU DONG GUI TAI LIEU PDF --- */

function mapPdfAutomationRule(row, { withBlob = false } = {}) {
  if (!row) return null;
  return {
    id: row.id,
    keyword: row.keyword,
    ...
    pdfMime: row.pdf_mime,
    pdfSize: row.pdf_size,
    enabled: Boolean(row.enabled),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    ...(withBlob ? { pdfData: row.pdf_data } : {}),
  };
}

/** Metadata cua moi rule cua dung mot owner; tuyet doi khong keo BLOB ve UI. */
export async function listPdfAutomationRules(ownerUid) {
  if (!ownerUid) return [];
  const rows = await all(
    `SELECT id, keyword, keyword_norm, pdf_name, pdf_mime, pdf_size,
            enabled, created_at, updated_at
     FROM pdf_automation_rules
     WHERE owner_uid = ?
     ORDER BY id ASC`,
    [String(ownerUid)]
  );
  return rows.map((row) => mapPdfAutomationRule(row));
}

/** Prefilter runtime cung chi doc metadata, khong doc bytes PDF. */
export async function listEnabledPdfAutomationRules(ownerUid) {
  if (!ownerUid) return [];
  const rows = await all(
    `SELECT id, keyword, keyword_norm, pdf_name, pdf_mime, pdf_size,
            enabled, created_at, updated_at
...
/** Projection-safe: chi dem rule dang bat, khong doc metadata hay BLOB PDF. */
export async function getEnabledPdfAutomationRuleCount(ownerUid) {
  if (!ownerUid) return 0;
  const row = await get(
    `SELECT COUNT(*) AS enabled_rule_count
     FROM pdf_automation_rules
     WHERE owner_uid = ?
       AND enabled = 1`,
    [String(ownerUid)]
  );
  return Number(row?.enabled_rule_count) || 0;
}

export async function getPdfAutomationRule(ownerUid, id) {
  if (!ownerUid) return null;
  const row = await get(
    `SELECT id, keyword, keyword_norm, pdf_name, pdf_mime, pdf_size,
            enabled, created_at, updated_at
     FROM pdf_automation_rules
     WHERE owner_uid = ? AND id = ?`,
    [String(ownerUid), Number(id)]
  );
  return mapPdfAutomationRule(row);
}

/** Chi duoc goi sau khi khach da xac nhan OK. */
export async function getPdfAutomationRuleWithBlob(ownerUid, id) {
  if (!ownerUid) return null;
  const row = await get(
    `SELECT id, keyword, keyword_norm, pdf_name, pdf_mime, pdf_size, pdf_data,
            enabled, created_at, updated_at
     FROM pdf_automation_rules
     WHERE owner_uid = ? AND id = ?`,
    [String(ownerUid), Number(id)]
  );
  return mapPdfAutomationRule(row, { withBlob: true });
}

export async function insertPdfAutomationRule(ownerUid, rule) {
  if (!ownerUid) throw new Error("Thieu ownerUid cho PDF automation.");
  const now = Date.now();
  const result = await run(
    `INSERT INTO pdf_automation_rules
       (owner_uid, keyword, keyword_norm, pdf_name, pdf_mime, pdf_size,
        pdf_data, enabled, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      String(ownerUid),
      rule.keyword,
  ...
  return getPdfAutomationRule(ownerUid, result.lastID);
}

/** Mot UPDATE duy nhat giu replacement PDF atomic voi metadata cua no. */
export async function updatePdfAutomationRule(ownerUid, id, changes) {
  if (!ownerUid) throw new Error("Thieu ownerUid cho PDF automation.");
  const assignments = [];
  const params = [];

  if (changes.keyword !== undefined) {
    assignments.push("keyword = ?", "keyword_norm = ?");
    params.push(changes.keyword, changes.keywordNorm);
  }
  if (changes.enabled !== undefined) {
    assignments.push("enabled = ?");
    params.push(changes.enabled ? 1 : 0);
  }
  if (changes.pdfData !== undefined) {
    assignments.push("pdf_name = ?", "pdf_mime = ?", "pdf_size = ?", "pdf_data = ?");
    params.push(changes.pdfName, changes.pdfMime, changes.pdfSize, changes.pdfData);
  }
  if (assignments.length === 0) return getPdfAutomationRule(ownerUid, id);

  assignments.push("updated_at = ?");
  params.push(Date.now(), String(ownerUid), Number(id));
  const result = await run(
    `UPDATE pdf_automation_rules SET ${assignments.join(", ")}
     WHERE owner_uid = ? AND id = ?`,
    params
  );
  return result.changes ? getPdfAutomationRule(ownerUid, id) : null;
}

export async function deletePdfAutomationRule(ownerUid, id) {
  if (!ownerUid) return false;
  const result = await run(
    `DELETE FROM pdf_automation_rules WHERE owner_uid = ? AND id = ?`,
    [String(ownerUid), Number(id)]
  );
  return result.changes > 0;
}
...
export async function getAiChatConfig(ownerUid) {
  ...
  return {
    ...
    opencodeFallbackModel: row?.opencode_fallback_model || "",
    opencodeFallbackCapabilities: normalizeFallbackCapabilities(
      row?.opencode_fallback_capabilities ?? "[]"
    ),
    opencodeFailoverEnabled: normalizeFailoverEnabled(row?.opencode_failover_enabled ?? 0),
    ...runtime,
    updatedAt: row?.updated_at || null,
  };
}

/* --- API CREDENTIAL RIENG TUNG ZALO OWNER --- */

function ownerCredentialIdentity(ownerUid, providerId) {
  const owner = String(ownerUid || "").trim();
  const provider = String(providerId || "").trim();
  if (!owner) throw new Error("Thiếu ownerUid cho API credential.");
  if (!provider) throw new Error("Thiếu providerId cho API credential.");
  return { owner, provider };
}

function decryptOwnerProviderSecret(secretEnc) {
  if (!daMachHoa(secretEnc)) {
    throw new Error("API credential không ở định dạng mã hoá v1 hợp lệ.");
  }
  return giaiMa(secretEnc);
}

/** Luu plaintext duy nhat tai bien gioi nay; gia tri xuong SQLite luon la v1:. */
export async function saveOwnerProviderCredential(ownerUid, providerId, apiKey) {
  const { owner, provider } = ownerCredentialIdentity(ownerUid, providerId);
  const plain = String(apiKey || "").trim();
  if (!plain) throw new Error("API key không được để trống.");
  const encrypted = machHoa(plain);
  if (!encrypted.startsWith("v1:")) {
    throw new Error("Không mã hoá được API credential theo định dạng hiện hành.");
  }
  const now = Date.now();
  await run(
    `INSERT INTO owner_provider_credentials
       (owner_uid, provider_id, secret_enc, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(owner_uid, provider_id) DO UPDATE SET
       secret_enc = excluded.secret_enc,
       updated_at = CASE
         WHEN excluded.updated_at <= owner_provider_credentials.updated_at
           THEN owner_provider_credentials.updated_at + 1
         ELSE excluded.updated_at
       END`,
    [owner, provider, encrypted, now, now]
  );
  const row = await get(
    `SELECT provider_id, created_at, updated_at
     FROM owner_provider_credentials
     WHERE owner_uid = ? AND provider_id = ?`,
    [owner, provider]
  );
  return {
...
/** Status browser-safe: khong SELECT secret_enc nen khong the vo tinh bung bi mat. */
export async function listOwnerProviderCredentialStatus(ownerUid) {
  const owner = String(ownerUid || "").trim();
  if (!owner) return [];
  const rows = await all(
    `SELECT provider_id, created_at, updated_at
...
/** Doc noi bo mot credential; loi giai ma bat buoc noi len, khong doi thanh null. */
export async function getOwnerProviderCredential(ownerUid, providerId) {
  const { owner, provider } = ownerCredentialIdentity(ownerUid, providerId);
  const row = await get(
    `SELECT provider_id, secret_enc, created_at, updated_at
     FROM owner_provider_credentials
     WHERE owner_uid = ? AND provider_id = ?`,
    [owner, provider]
  );
  ...
  return {
    providerId: row.provider_id,
    apiKey: decryptOwnerProviderSecret(row.secret_enc),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

/** Tap authority dung cho projection; bat ky ciphertext hong nao cung lam ca tap fail. */
export async function listOwnerProviderCredentialsForProjection(ownerUid) {
  const owner = String(ownerUid || "").trim();
  if (!owner) return [];
  const rows = await all(
    `SELECT provider_id, secret_enc, created_at, updated_at
...
export async function deleteOwnerProviderCredential(ownerUid, providerId) {
  const { owner, provider } = ownerCredentialIdentity(ownerUid, providerId);
  const result = await run(
    `DELETE FROM owner_provider_credentials WHERE owner_uid = ? AND provider_id = ?`,
    [owner, provider]
  );
  return result.changes > 0;
}

export async function deleteAllOwnerProviderCredentials(ownerUid) {
  const owner = String(ownerUid || "").trim();
  if (!owner) throw new Error("Thiếu ownerUid cho API credential.");
  const result = await run(
    `DELETE FROM owner_provider_credentials WHERE owner_uid = ?`,
    [owner]
  );
  return result.changes;
}
...
/** Projection-safe: chi dem lich con cho cua owner; legacy NULL owner bi loai. */
export async function getPendingScheduledMessageCount(ownerUid) {
  if (!ownerUid) return 0;
  const row = await get(
...
export async function clearTrainingMessages(ownerUid) {
  ...
  return withSharedConnectionTransaction(async ({ run: txRun }) => {
    await txRun(`DELETE FROM training_messages WHERE owner_uid = ?`, [String(ownerUid)]);
    await txRun(`DELETE FROM training_session WHERE owner_uid = ?`, [String(ownerUid)]);
  });
}
...
export async function saveAiChatConfig(ownerUid, {
  ...
  const knowledgeIdsStr = knowledgeFileIds === undefined || knowledgeFileIds === null
    ? null
    : JSON.stringify((Array.isArray(knowledgeFileIds) ? knowledgeFileIds : []).map(Number).filter(Number.isInteger));
  const fallbackCapabilitiesStr = opencodeFallbackCapabilities === undefined
    || opencodeFallbackCapabilities === null
    ? null
    : JSON.stringify(normalizeFallbackCapabilities(opencodeFallbackCapabilities, { publicApi: true }));
  const failoverDbValue = opencodeFailoverEnabled === undefined || opencodeFailoverEnabled === null
    ? null
    : normalizeFailoverEnabled(opencodeFailoverEnabled, { publicApi: true }) ? 1 : 0;
  ...
  await run(
    `INSERT INTO ai_chat_config
       (owner_uid, allowed_topics, role_tone, use_knowledge, knowledge_file_ids, soul,
         opencode_model, opencode_fallback_model, opencode_fallback_capabilities,
         opencode_failover_enabled, updated_at)
     VALUES ($ownerUid, COALESCE($allowedTopics,''), COALESCE($roleTone,''), COALESCE($useKnowledge,0),
             COALESCE($knowledgeFileIds,'[]'), COALESCE($soul,''), COALESCE($opencodeModel,''),
              COALESCE($opencodeFallbackModel,''), COALESCE($opencodeFallbackCapabilities,'[]'),
              COALESCE($opencodeFailoverEnabled,0), strftime('%s','now'))
     ON CONFLICT(owner_uid) DO UPDATE SET
       allowed_topics = COALESCE($allowedTopics, ai_chat_config.allowed_topics),
       role_tone = COALESCE($roleTone, ai_chat_config.role_tone),
       use_knowledge = COALESCE($useKnowledge, ai_chat_config.use_knowledge),
       knowledge_file_ids = COALESCE($knowledgeFileIds, ai_chat_config.knowledge_file_ids),
       soul = COALESCE($soul, ai_chat_config.soul),
        opencode_model = COALESCE($opencodeModel, ai_chat_config.opencode_model),
        opencode_fallback_model = COALESCE($opencodeFallbackModel, ai_chat_config.opencode_fallback_model),
        opencode_fallback_capabilities = COALESCE(
          $opencodeFallbackCapabilities, ai_chat_config.opencode_fallback_capabilities
        ),
        opencode_failover_enabled = COALESCE(
          $opencodeFailoverEnabled, ai_chat_config.opencode_failover_enabled
        ),
       updated_at = strftime('%s','now')`,
    {
      $ownerUid: String(ownerUid),
      $allowedTopics: allowedTopics === undefined || allowedTopics === null ? null : String(allowedTopics),
      $roleTone: roleTone === undefined || roleTone === null ? null : String(roleTone),
      $useKnowledge: useKnowledge === undefined || useKnowledge === null ? null : useKnowledge ? 1 : 0,
      $knowledgeFileIds: knowledgeIdsStr,
      $soul: soul === undefined || soul === null ? null : String(soul),
      $opencodeModel: opencodeModel === undefined || opencodeModel === null ? null : String(opencodeModel),
      $opencodeFallbackModel: opencodeFallbackModel === undefined || opencodeFallbackModel === null
        ? null
        : String(opencodeFallbackModel),
      $opencodeFallbackCapabilities: fallbackCapabilitiesStr,
      $opencodeFailoverEnabled: failoverDbValue,
    }
  );
}
...
export async function setAdminZalo(ownerUid, uid, label) {
  ...
  if (uid == null) return;
  await saveAccountConfig(ownerUid, { adminZaloUid: uid, adminZaloLabel: uid === "" ? "" : (label || "") });
}
...
/**
 * Projection read-only cho App Context: chi noi co/khong va co doc duoc hay
 * khong, tuyet doi khong dua plaintext ra khoi DB boundary.
 */
export async function getAppSecretPresence(key) {
  const row = await get(`SELECT value FROM app_secrets WHERE key = ?`, [String(key)]);
  if (!row?.value) return { present: false, readable: true };
  try {
    return { present: Boolean(giaiMa(row.value)), readable: true };
  } catch {
...
/** Safe presence-only resolver; khong tra password/username/address body. */
export async function getSafeSmtpConfigurationState() {
  const row = await get(`SELECT * FROM smtp_config WHERE id = 1`);
  if (!row) {
    return { readable: true, hasHost: false, hasFromAddress: false, hasUsername: false, hasPassword: false };
  }
  let passwordReadable = true;
  let hasPassword = false;
  if (row.password) {
    try {
      hasPassword = Boolean(giaiMa(row.password));
    } catch {
      passwordReadable = false;
    }
  }
  const hasUsername = Boolean(String(row.username || "").trim());
  return {
    // Password chi la required config khi SMTP username/auth dang duoc dung.
    readable: !hasUsername || passwordReadable,
    hasHost: Boolean(String(row.host || "").trim()),
    hasFromAddress: Boolean(String(row.from_address || "").trim()),
    hasUsername,
    hasPassword,
  };
}

/** Safe presence-only resolver cho Zoho; khong tra bat ky credential nao. */
export async function getSafeZohoConfigurationState() {
  const row = await get(`SELECT * FROM zoho_config WHERE id = 1`);
  if (!row) {
    return {
      readable: true,
  ...
  const presence = (stored) => {
    if (!stored) return { present: false, readable: true };
    try {
      return { present: Boolean(giaiMa(stored)), readable: true };
    } catch {
      return { present: false, readable: false };
    }
  };
  const clientId = presence(row.client_id);
  const clientSecret = presence(row.client_secret);
  const refreshToken = presence(row.refresh_token);
  return {
    readable: clientId.readable && clientSecret.readable && refreshToken.readable,
    enabled: Boolean(row.bat),
    hasClientId: clientId.present,
    hasClientSecret: clientSecret.present,
    hasRefreshToken: refreshToken.present,
    hasAccountId: Boolean(String(row.account_id || "").trim()),
  };
}
