...
async function xoaGroqKeyCu() {
  const row = await get(`SELECT length(groq_api_key) AS n FROM ai_chat_config WHERE id = 1`);
  if (!row?.n) return;
  await run(`UPDATE ai_chat_config SET groq_api_key = '' WHERE id = 1`);
...
export async function rebuildThreadsFromMessages(ownerUid) {
  ...
  for (const row of rows) {
    await run(
      `UPDATE threads SET last_message = COALESCE(?, last_message),
                          last_message_at = COALESCE(?, last_message_at),
                          updated_at = ?
       WHERE local_id = ?`,
      [row.content, row.ts, Date.now(), row.thread_id]
    );
  }
}
