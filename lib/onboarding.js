...
function duLieuAnToan(value) {
  ...
  return {
    firstSetupInviteSeen: data.firstSetupInviteSeen === true,
    guidanceCompleted: data.guidanceCompleted === true,
...
function duLieuPassMoi(value) {
  const previous = duLieuAnToan(value);
  return duLieuAnToan({
    firstSetupInviteSeen: previous.firstSetupInviteSeen,
    guidanceCompleted: previous.guidanceCompleted,
  });
}
...
export async function xuLyHanhDongOnboarding(ownerUid, action, payload = {}) {
  ...
  // Hai co owner-scoped nay la hanh dong ngu nghia doc lap voi state machine.
  // Chung van phai ghi duoc khi onboarding da completed va khong phu thuoc step.
  if (action === "invite_seen") {
    data.firstSetupInviteSeen = true;
    return congKhai(await saveAccountConfig(ownerUid, { setupData: duLieuAnToan(data) }));
  }
  if (action === "guidance_completed") {
    data.guidanceCompleted = true;
    return congKhai(await saveAccountConfig(ownerUid, { setupData: duLieuAnToan(data) }));
  }

  if (action === "start") {
    if (!state.completed && step !== 0) return state;
    const nextData = state.completed ? duLieuPassMoi(data) : data;
    return (await coModelHieuLuc(ownerUid))
      ? luu(ownerUid, 4, nextData)
      : luu(ownerUid, 1, nextData);
  }
