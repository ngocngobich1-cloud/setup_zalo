...
function validateProviderSnapshot(snapshot, providerId) {
  const catalog = providerCatalog(snapshot);
...
async function validateProviderUnlocked(config, providerId) {
  return validateProviderSnapshot(await getProviderState(config), providerId);
}

async function projectionProviderStateUnlocked(config) {
  const discoveryDirectory = credentialProjectionDiscoveryDirectory();
  return discoveryDirectory
    ? getProviderState(config, { directory: discoveryDirectory })
    : getProviderState(config, { allowDefaultContext: true });
}
...
async function projectOwnerCredentialsUnlocked(targetOwnerUid, config, options = {}) {
  ...
  try {
    ...
    // Save da chup snapshot nay truoc DB write. Reuse operation-local de validation
    // va projection cung doc dung mot provider state, khong them GET /provider.
    const before = options.before || await projectionProviderStateUnlocked(config);
...
export async function saveCurrentOwnerCredential(ownerUid, providerId, apiKey, options = {}) {
  ...
  try {
    return await withCredentialPlaneWrite(async () => {
      ...
      const config = options.config || await getAiRuntimeConfig();
      const before = await projectionProviderStateUnlocked(config);
      validateProviderSnapshot(before, provider);
      const saved = await saveOwnerProviderCredential(owner, provider, plain);
      await projectOwnerCredentialsUnlocked(owner, config, { before });
      return saved;
    });
  } finally {
    plain = "";
...
export async function testCurrentOwnerCredential(ownerUid, providerId, options = {}) {
  ...
  return withCredentialPlaneRead(
    ...
    () => testProviderKey(config, provider, { deadlineAt })
  );
}
