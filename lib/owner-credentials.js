...
/** Outer boundary cho Zalo/training/onboarding: giu cung owner+provider suot inference. */
export async function withCurrentOwnerCredentialRead(ownerUid, config, operation) {
  ...
  return withCredentialPlaneRead({ ownerUid: owner, providerId }, operation);
}
...
export function ownerCredentialHttpError(error) {
  ...
  if (["INVALID_KEY", "NO_QUOTA", "PROVIDER_UNAVAILABLE", "TIMEOUT", "UNKNOWN"].includes(code)) {
    return { status: 400, code, message: error.message };
  }
