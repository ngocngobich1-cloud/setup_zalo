...
export async function generateReply(
  ...
  const base = {
    ...
    decision: null, malformedDecision: false, needAdmin: false,
  };
  ...
  if (useKnowledge && Array.isArray(knowledgeFileIds) && knowledgeFileIds.length > 0) {
    try {
      const query = [khoiTep, loiKhach].filter(Boolean).join(" ").slice(0, 2000);
      const retrieved = await knowledge.retrieveForAi(ownerUid, knowledgeFileIds, query, KNOWLEDGE_MAX_CHARS);
...
export async function tryReply(userMessage, messageObj) {
  ...
  if (emailStatusLookup?.outcome === "RATE_LIMITED") {
    return "Hiện mình chưa thể kiểm tra thêm trạng thái email này. Bạn thử lại sau một lúc nhé.";
  }
  if (emailStatusLookup?.outcome === "NEEDS_ADMIN") {
    ...
    return opened?.acknowledgement || null;
  }
  ...
  if (result.malformedDecision) {
    ...
    try {
      ...
      if (!retryResult.malformedDecision && !retryResult.error) {
        ...
        return MALFORMED_DECISION_FALLBACK;
      }
    } catch (error) {
      await recordDecisionProtocolOutcome({
        ownerUid,
