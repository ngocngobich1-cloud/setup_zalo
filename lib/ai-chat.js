...
export async function loadConfig() {
  const chung = await getAiChatConfig();
  const rieng = await getAccountConfig(layChuTaiKhoan());
  configCache = chung
    ? { ...chung, botEnabled: rieng.botEnabled, allowedGroupId: rieng.allowedGroupId, allowedSenderIds: rieng.allowedSenderIds }
    : null;
}
...
export async function buildBootstrapContext(threadId, boQuaMessageId) {
  ...
  const thread = threadId ? await getThread(layChuTaiKhoan(), threadId).catch(() => null) : null;
  const recentHistory = await buildRecentHistory(layChuTaiKhoan(), threadId, boQuaMessageId);
  return {
    soul,
    roleTone,
    allowedTopics,
...
export async function generateReply(userMessage, messageObj) {
  ...
  try {
    ...
    await bumpSessionTurns(layChuTaiKhoan(), threadId);
