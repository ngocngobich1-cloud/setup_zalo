...
export async function generateReply(
  ...
  try {
    ...
    let result;
    try {
      result = await opencode.sendPrompt(config, session.sessionId, promptDayDu, { timeoutMs: 30000 });
    } catch (primaryError) {
      ...
      try {
        result = await opencode.sendPrompt(secondaryConfig, session.sessionId, promptDayDu, { timeoutMs: 30000 });
        await addLog({
  ...
  if (result.malformedDecision) {
    const contractFailureLog = typeof canonicalDecisionResultLog === "function"
      ...
      : {
          ...
          level: "error",
          summary: "AI trả về định dạng quyết định không hợp lệ — bot đã im lặng",
          detail: {
            ownerUid,
