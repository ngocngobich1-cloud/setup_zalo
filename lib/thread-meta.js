...
export async function syncThreadCatalog(api, boQuaCache = false) {
  try {
    if (api.getAliasList) {
  ...
  const existing = await listThreads({ recentOnly: true });
  for (const thread of existing) {
    const meta = await resolveThreadMeta(api, thread.id, thread.threadType, { boQuaCache });
    await upsertThread({ id: thread.id, threadType: thread.threadType, ...meta });
  }
}

export async function enrichExistingThread(api, threadId, threadType, patch = {}) {
