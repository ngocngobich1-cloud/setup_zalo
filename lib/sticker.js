...
/** A failed cached asset may have expired. Refresh at most once for that URL. */
export async function refreshStickerUrl(api, stickerId, failedUrl) {
  if (stickerUrlCache.get(stickerId) === failedUrl) stickerUrlCache.delete(stickerId);
  return resolveStickerUrl(api, stickerId);
}
