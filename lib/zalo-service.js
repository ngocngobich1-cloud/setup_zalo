    ...
    for (const [i, bubble] of bubbles.entries()) {
      ...
      for (let attempt = 1; attempt <= MAX_OUTBOUND_ATTEMPTS; attempt += 1) {
        try {
          ...
          const coTheRetry = outboundFailureOutcome === "DEFINITIVE_PROVIDER_REJECTION"
...
export async function sendChatMessage({ threadId, text, threadType, quote, mentions, urgency, attachment, originToken }) {
  ...
  // Provider da return thanh cong la fact. Confirm unconditional va truoc moi
  // msgId handling/persistence; null response khong duoc bien thanh retry.
  conversationGeneration?.xacNhanOutbound();
