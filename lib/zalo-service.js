...
const dieuPhoiHoiThoai = taoDieuPhoiHoiThoai({
  ...
  khiHuy: blockDurableCoordinatorWork,
});
...
async function runClarificationResume(id, automaticContext, originToken, conversationGeneration) {
  ...
  return resumeAdminClarification(id, {
    ...
    generate: async (row) => ownerCredentials.withCurrentOwnerCredentialRead(
      row.ownerUid,
...
async function traLoiCumTin(workOrTins, automaticWork = null, conversationGeneration = null) {
  ...
  /**
   * Realtime typing cua Web Inbox song theo TOAN BO AI span. Dat helper trong
   * outer function de cac isolated lifecycle harness cung chay dung semantics.
   */
  const batDauWebTyping = (ownerUid, threadId) => {
    const chu = String(ownerUid || "").trim();
    const hoiThoai = String(threadId || "").trim();
    if (!chu || !hoiThoai) return () => {};

    const realtime = typeof io === "undefined" ? null : io;
    const heartbeatMs = typeof NHAC_GO_PHIM_MS === "number" ? NHAC_GO_PHIM_MS : 3000;
    let daDung = false;
    const phat = (typing) => {
      realtime?.emit("bot_typing_status", {
        ownerUid: chu,
        threadId: hoiThoai,
        typing,
        at: Date.now(),
    ...
    nhac();
    const dongHo = setInterval(nhac, heartbeatMs);
    return () => {
      if (daDung) return;
      daDung = true;
      clearInterval(dongHo);
  ...
  const tatWebTyping = batDauWebTyping(inferenceOwnerUid, tin.threadId);
  ...
  try {
    ...
    aiReply = await ownerCredentials.withCurrentOwnerCredentialRead(
      inferenceOwnerUid,
      inferenceConfig,
