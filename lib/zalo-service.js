...
import { chonTinhHuong, layStickerHopLe } from "./sticker-zalo.js";
...
function setupListener() {
  ...
  api.listener.on("message", async (message) => {
    ...
    try {
      ...
      if (normalizedMsg?.isSelf === true && normalizedMsg?.msgType !== "share.file") return;
...
const boGom = taoBoGom({
  conDangGo,
  khiChot: (tins, originToken) => traLoiCumTin(tins, originToken)
 ...
 */
async function dungNhacTen(text, groupId, boQuaUid = []) {
  const bo = new Set(boQuaUid.map(String));
  return khopTenTrongCau(text, (await layThanhVien(groupId)).filter((t) => !bo.has(t.uid)));
}
 ...
 */
async function dungTheNhacTen(bubble, tin, laBubbleDau) {
  const the = [];
  let noiDung = bubble;
  let lech = 0;
  ...
  if (laBubbleDau && uidNguoiNoi && tenNguoiNoi) {
    noiDung = `${tenNguoiNoi} ${bubble}`;
    lech = tenNguoiNoi.length + 1;
    the.push({ uid: uidNguoiNoi, pos: 0, len: tenNguoiNoi.length });
  }

  // Do ten TREN BUBBLE GOC roi day vi tri sang, khong do tren doan da ghep:
  // do tren doan ghep thi mot thanh vien ten "Ngoc" se khop nham vao trong ten
  // nguoi noi "Bich Ngoc" o dau cau.
  const khac = await dungNhacTen(bubble, tin.threadId, [uidNguoiNoi]).catch(() => []);
  for (const m of khac) the.push({ ...m, pos: m.pos + lech });
...
async function thuGuiSticker(tin, originToken = null) {
  try {
    ...
    const khoa = String(tin.threadId);
    const lanCuoi = lanStickerCuoi.get(khoa) || 0;
    ...
    if (originToken && !originConHieuLuc(originToken)) return;
    await api.sendSticker({ id: stk.id, cateId: stk.cateId, type: stk.type }, khoa, Number(tin.threadType));
    if (originToken && !originConHieuLuc(originToken)) return;
    lanStickerCuoi.set(khoa, Date.now());

    await addLog({
      event: "gui_sticker",
      level: "ok",
      summary: `Gửi sticker "${stk.moTa}" (${viec})`,
      detail: { threadId: khoa, viec, sticker: stk, loiKhach: String(tin.content).slice(0, 100) },
    });
  } catch (error) {
...
async function traLoiCumTin(tins, originToken = null) {
  if (originToken && !originConHieuLuc(originToken)) return;
  const tin = gopThanhMotTin(tins);
  if (!tin.content?.trim()) return;

  // Exact OK doc tren tung tin goc; keyword doc tren noi dung da gop. PDF
  // chay truoc reaction/typing/AI de HANDLED dung toan bo pipeline thuong.
  ...
  try {
    ...
    for (const [i, bubble] of bubbles.entries()) {
      ...
      const { text: noiDung, mentions } = laNhom
        ? await dungTheNhacTen(bubble, tin, i === 0).catch(() => ({ text: bubble, mentions: [] }))
...
async function handleNewIncomingMessage(normalizedMsg) {
  // Signature cu duoc giu cho focused harness; listener production truyen
  // immutable runtime origin o doi so thu hai.
  const originToken = arguments[1] || null;
  const guiTheoOrigin = (payload) => sendChatMessage({ ...payload, originToken });
  ...
  if (!normalizedMsg.isSelf) {
    ...
    if (await laLenhAdmin(normalizedMsg)) {
      if (originToken && !originConHieuLuc(originToken)) return;
      const traLoi = await xuLyLenh(normalizedMsg, guiTheoOrigin, sendResolvedPrivateMessage);
      if (originToken && !originConHieuLuc(originToken)) return;
      if (traLoi) {
        await guiTheoOrigin({
          threadId: normalizedMsg.threadId,
    ...
    if (!handledByAutoReply && typeof normalizedMsg.content === "string" && normalizedMsg.content.trim()) {
      ...
      if (originToken && !originConHieuLuc(originToken)) return;
      boGom.them(normalizedMsg, originToken);
    }
  }
}
...
export async function persistAndBroadcastMessage(message, originToken = null) {
  ...
  if (!chu) return null;
  const meta = await resolveThreadMeta(originApi, message.threadId, message.threadType);
  if (originToken && !originConHieuLuc(originToken)) return null;
...
export async function sendChatMessage({ threadId, text, threadType, quote, mentions, urgency, attachment, originToken }) {
  ...
  // Voi media, attachment response chi co msgId, khong co URL provider. Chi
  // persist text companion neu co; file doi listener canonical share.file.
  ...
  const daPhat = await persistAndBroadcastMessage(message, originToken);
...
export async function sendResolvedPrivateMessage(
  ...
  { ownerUid = chuHienTai(), registerThread = upsertThread, canonicalSend = sendChatMessage } = {}
) {
