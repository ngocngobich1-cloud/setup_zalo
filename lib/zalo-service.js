 ...
 * nhu khong chay. Dat 15 giay de phu duoc nhip 10 giay do, con du le.
 */
const CON_DANG_GO_MS = 15000;
...
/**
 * Chi coi la "dang soan tin tiep" khi tin hieu go phim den SAU tin nhan cuoi
 * cung cua nguoi do.
 *
 * Chi nang nguong len thi hong theo huong nguoc lai: khach go xong, bam gui roi
 * ngoi im, nhung tin hieu go phim luc soan tin do van con "moi" - bot lai ngoi
 * cho them chuc giay khong vi ly do gi. Con moc "sau tin nhan cuoi" thi phan
 * biet duoc hai canh:
 *    go -> gui -> im            : tin hieu CU hon tin nhan -> tra loi ngay
 *    go -> gui -> go tiep       : tin hieu MOI hon tin nhan -> cho ke not
 */
function conDangGo(threadId, senderId, mocTinCuoi) {
  const moc = lanGoCuoi.get(khoaGom(threadId, senderId));
  if (!moc) return false;
  if (mocTinCuoi && moc <= mocTinCuoi) return false;
  return Date.now() - moc < CON_DANG_GO_MS;
}
