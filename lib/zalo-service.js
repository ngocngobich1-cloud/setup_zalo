import { Zalo, ZaloApiError, LoginQRCallbackEventType, ThreadType, AvatarSize } from "zca-js";
...
import { taoDieuPhoiHoiThoai } from "./conversation-inflight.js";
...
import {
  getThread,
  getThreadMessages,
  getThreadMessagesThroughBoundary,
  getAdminClarificationById,
  claimAdminClarificationAnswer,
  createAdminClarification,
  expireWaitingAdminClarifications,
  getAdminClarificationByToken,
  getWaitingAdminClarification,
  getRecentFailedAdminClarificationAck,
  listRecoverableAdminClarifications,
  touchWaitingAdminClarification,
  transitionAdminClarification,
  completeAdminClarificationAck,
  insertMessage,
  listThreads,
...
import {
  ...
  enrichHistoryFromCache,
  warmHistoryEnrichment,
  resolveSenderAvatar,
  resolveThreadMeta,
  syncThreadCatalog,
} from "./thread-meta.js";
import {
  ...
  shouldSyncGroupHistory,
  getGroupHistorySyncStatus,
} from "./chat-history.js";
import {
  enrichMessageSticker,
  getCachedStickerUrl,
  refreshStickerUrl,
  resolveStickerUrl,
} from "./sticker.js";
...
import {
  ...
  configureAdminClarificationStore,
  recoverAdminClarifications,
  resumeAdminClarification,
  touchAdminClarificationContext,
} from "./admin-clarification.js";
import { sendAdminNotification } from "./admin-notification.js";
import {
  blockDurableCoordinatorWork,
  capHinhDurableDispatcher,
  durableIds,
  ganDurableGenerationChoWork,
  giaHanLeaseDurableGeneration,
  ghiNhanDurableAdmission,
  hoanTatDurableGeneration,
  requeueDurableStaleGeneration,
} from "./durable-message-queue.js";
import {
  ...
  withGlobalAiSlot,
} from "./global-ai-limiter.js";
import { chuanBiDurableOutbox, guiDurableOutbound } from "./outbound-outbox.js";
import { configureZaloRuntimeReader } from "./app-context.js";
import {
...
const SEND_REJECTED_BEFORE_PROVIDER = Object.freeze({
  authorityRejectedBeforeProvider: true,
});
...
let adminClarificationCoordinatorHarnessActive = false;

export function phanLoaiLoiGuiProvider(error) {
  if (error instanceof ZaloApiError
    && Number.isFinite(error.code)) {
    return OUTBOUND_OUTCOME.DEFINITIVE_PROVIDER_REJECTION;
  }
  if (error instanceof ZaloApiError
    && loiClientZcaChacChan.some((message) => String(error.message || "").startsWith(message))) {
    return OUTBOUND_OUTCOME.DETERMINISTIC_CLIENT_ERROR;
  }
  return OUTBOUND_OUTCOME.SEND_UNKNOWN;
}

function ganOutcomeChoLoi(error, outcome) {
  if (error && (typeof error === "object" || typeof error === "function")) {
    try {
      Object.defineProperty(error, OUTBOUND_OUTCOME_FIELD, {
        value: outcome,
        configurable: true,
      });
      if (error[OUTBOUND_OUTCOME_FIELD] === outcome) return error;
    } catch {
      // Object ngoai co the frozen/non-extensible; boc lai o duoi.
    }
  }
  const wrapped = new Error(error?.message || String(error ?? "Unknown provider error"));
  if (error?.code !== undefined) wrapped.code = error.code;
  Object.defineProperty(wrapped, OUTBOUND_OUTCOME_FIELD, {
    value: outcome,
    configurable: true,
  });
  return wrapped;
...
async function thongBaoAdminLoiOutbound({ ownerUid, threadId, failedBubbleIndex, outcome }) {
  const key = `${String(ownerUid || "")}\u0000${String(threadId || "")}`;
  if (outboundFailureNotifications.has(key)) return { sent: false, reason: "RECURSION_GUARD" };
  outboundFailureNotifications.add(key);
  try {
    return await sendAdminNotification({
      ownerUid,
      text: `Cảnh báo Vizenbot: không gửi được phản hồi khách ở hội thoại ${threadId}`
        + ` (phần ${failedBubbleIndex || "?"}, ${outcome}). Vui lòng kiểm tra nhật ký.`,
      send: sendChatMessage,
      originToken: taoOriginRuntime(),
      sendOptions: { suppressOutboundFailureNotification: true },
    });
  } catch (error) {
    return { sent: false, reason: error?.message || "ADMIN_NOTIFY_FAILED" };
  } finally {
    outboundFailureNotifications.delete(key);
  }
}
...
function khoaThreadEligibility(ownerUid, threadId) {
  return `${String(ownerUid)}\u0000${String(threadId)}`;
}

function threadEligibilityEpochHienTai(threadKey) {
  return threadEligibilityEpochs.get(String(threadKey)) || 0;
}

function threadEligibilityConHieuLuc(threadKey, capturedEpoch) {
  return typeof threadKey === "string"
    && Number.isInteger(capturedEpoch)
    && capturedEpoch === threadEligibilityEpochHienTai(threadKey);
}

function automaticWorkConHieuLuc(automaticWork) {
  if (automaticWork == null) return true;
  if (typeof automaticWork !== "object") return false;
  const globalHopLe = Number.isInteger(automaticWork.capturedBotEligibilityEpoch)
    && botEligibilityConHieuLuc(automaticWork.capturedBotEligibilityEpoch);
  const threadHopLe = typeof automaticWork.threadEligibilityKey === "string"
    && Number.isInteger(automaticWork.capturedThreadEligibilityEpoch)
    && threadEligibilityConHieuLuc(
      automaticWork.threadEligibilityKey,
      automaticWork.capturedThreadEligibilityEpoch
    );
  const generationHopLe = !automaticWork.conversationGeneration
    || automaticWork.conversationGeneration.conHieuLuc();
  return globalHopLe && threadHopLe && generationHopLe;
}

function tuyChonGuiTuDong(automaticWork) {
  if (!automaticWork || typeof automaticWork !== "object") return undefined;
  return {
    botEligibilityEpoch: automaticWork.capturedBotEligibilityEpoch,
    threadEligibilityKey: automaticWork.threadEligibilityKey,
    threadEligibilityEpoch: automaticWork.capturedThreadEligibilityEpoch,
    ...(automaticWork.conversationGeneration
      ? { conversationGeneration: automaticWork.conversationGeneration }
      : {}),
  };
}
...
export function applyBotEligibilityTransition(previousEnabled, nextEnabled) {
  ...
  if (!previousEnabled && nextEnabled && typeof recoverAdminClarifications === "function") {
    recoverAdminClarifications({ ownerUid: chuHienTai() }).catch(() => {});
  }
  return botEligibilityEpoch;
}

/** Thread transition chi huy authority/work cua chinh owner + thread do. */
export function applyThreadBotEligibilityTransition(ownerUid, threadId, previousEnabled, nextEnabled) {
  const threadKey = khoaThreadEligibility(ownerUid, threadId);
  if (Boolean(previousEnabled) === Boolean(nextEnabled)) {
    return threadEligibilityEpochHienTai(threadKey);
  }
  const nextEpoch = threadEligibilityEpochHienTai(threadKey) + 1;
  threadEligibilityEpochs.set(threadKey, nextEpoch);
  boGom.huyTheoThread(threadId, ownerUid);
  clearPendingPdfConfirmationsForThread(ownerUid, threadId);
  if (!previousEnabled && nextEnabled && typeof recoverAdminClarifications === "function") {
    recoverAdminClarifications({ ownerUid, customerThreadId: threadId }).catch(() => {});
  }
  return nextEpoch;
}
...
const handlePdfAutomation = createPdfAutomationHandler({
  ...
  sendMessage: (payload, automaticWork) => sendChatMessage(payload, tuyChonGuiTuDong(automaticWork)),
  isOriginCurrent: originConHieuLuc,
  isAutomaticWorkCurrent: automaticWorkConHieuLuc,
...
export function configureZaloService(socketServer) {
  io = socketServer;
  configureGlobalAiLimiter({
    log: ({ event, ...detail }) => addLog({
      event,
      level: event.includes("timeout") || event.includes("cancel") ? "warn" : "info",
      summary: event,
      detail,
    }),
  });
  // Tiem ham lay uid tai khoan hien tai cho ai-chat (tranh vong import).
  aiChat.enableAdminClarificationDecisionMode();
  aiChat.capHinhChuTaiKhoan(chuHienTai);
  capHinhChuTaiKhoanLog(chuHienTai);
  capHinhChuTaiKhoanAdmin(chuHienTai);
  ownerCredentials.configureCurrentOwnerResolver(chuHienTai);
  configureZaloRuntimeReader({
    currentOwner: chuHienTai,
    loggedIn: isLoggedIn,
...
async function finalizeLogin(loggedApi) {
  ...
  appState.loggingIn = false;
  resetHistorySyncState();
  // UID canonical co san dong bo tu API moi. Lay no ngay de writer duoc queue
  // truoc moi network profile cham, dung deadline 20s tinh tu owner switch.
  try {
  ...
  // Credential projection phai xong TRUOC khi listener cua owner moi duoc gan.
  // Neu projection hong, module credential da fail-closed va login khong duoc
  // phep mo duong inbound AI cho owner nay.
  try {
    await ownerCredentials.projectOwnerCredentials(chuHienTai());
  } catch (error) {
    api = null;
    appState.uid = null;
...
export async function dangXuatZalo() {
  voHieuHoaViecRuntimeCu();
  // Writer se drain/abort moi inference cua owner cu, roi chieu tap rong. Neu
  // sidecar hong thi state van NOT READY; logout Zalo van phai tiep tuc.
  await ownerCredentials.projectOwnerCredentials(null).catch((error) => {
    console.error("[owner-credentials] Deactivation projection failed:", error.code || "UNKNOWN");
  });
  try {
    api?.listener?.stop?.();
  } catch {
...
const TOM_TAT_SU_KIEN_DIEU_PHOI = {
  conversation_inflight_start: "Bắt đầu generation tuần tự cho hội thoại",
  conversation_pending_queued: "Đã xếp tin mới vào pending của hội thoại",
  generation_marked_stale: "Generation hiện tại đã stale vì có tin mới",
  pending_drain_start: "Bắt đầu xử lý pending của hội thoại",
  conversation_inflight_end: "Kết thúc generation của hội thoại",
  conversation_inflight_cancel: "Đã hủy work đang chờ của hội thoại",
  conversation_inflight_error: "Generation của hội thoại gặp lỗi",
  generation_stale_deferred: "Đã hoãn stale trong lúc provider đang xử lý outbound",
};

const dieuPhoiHoiThoai = taoDieuPhoiHoiThoai({
  chay: (work, generation) => traLoiCumTin(
    work,
    work.segments[0]?.automaticWork || null,
    generation
  ),
  ghiSuKien: ({ event, ...detail }) => addLog({
    event,
    level: event === "conversation_inflight_error" ? "error" : "info",
    summary: TOM_TAT_SU_KIEN_DIEU_PHOI[event] || event,
    detail,
  }),
  truocGeneration: ganDurableGenerationChoWork,
  khiStale: requeueDurableStaleGeneration,
  khiHoanTat: hoanTatDurableGeneration,
  khiHuy: async (cancelledWork, reason) => {
    const generation = cancelledWork?.activeGeneration || null;
    if (generation) {
      cancelGlobalAiWaiter(generation, reason);
      generation.p2StopLeaseHeartbeat?.();
      generation.p2StopWaitingTyping?.();
      generation.p2StopWebTyping?.();
    }
    return blockDurableCoordinatorWork(cancelledWork, reason);
  },
});

async function authorityForClarification(row, automaticContext, originToken, conversationGeneration) {
  const freshOrigin = originToken
    && originToken.originOwnerUid === chuHienTai()
    && originToken.originRuntimeGeneration === runtimeGeneration
    && originToken.originApiIdentity === api;
  if (!freshOrigin) return false;
  const freshAutomatic = automaticContext
    && botEligibilityConHieuLuc(automaticContext.capturedBotEligibilityEpoch)
    && threadEligibilityConHieuLuc(
      automaticContext.threadEligibilityKey,
      automaticContext.capturedThreadEligibilityEpoch
    )
    && (!conversationGeneration || conversationGeneration.conHieuLuc());
  if (!freshAutomatic) return false;
  if (String(row.ownerUid) !== String(chuHienTai() || "")) return false;
  if (!aiChat.getConfig(row.ownerUid)?.botEnabled) return false;
  const thread = await getThread(row.ownerUid, row.customerThreadId);
  return thread?.botEnabled !== false
    && (!conversationGeneration || conversationGeneration.conHieuLuc());
}

async function clarificationPrompt(row) {
  const frozen = await getThreadMessagesThroughBoundary(
    row.ownerUid,
    row.customerThreadId,
    row.frozenContextBoundary,
    40
  );
  const history = frozen.map((message) =>
    `${message.isSelf ? "BUSINESS" : "CUSTOMER"}: ${String(message.content || "").trim()}`
  ).filter((line) => !line.endsWith(": ")).join("\n");
  return [
    "# ADMIN CLARIFICATION RESUME — TRẢ LỜI KHÁCH, KHÔNG HỌC VĨNH VIỄN",
    "Chỉ dùng lịch sử canonical đã đóng băng và câu trả lời Admin dưới đây.",
    "Không ghi câu trả lời Admin thành quy tắc, hồ sơ, tri thức chung hay sự thật cho lần khác.",
    "Hãy trả lời trực tiếp, tự nhiên cho đúng câu hỏi của khách. Không nhắc mã correlation.",
    "",
    "<BEGIN_FROZEN_CANONICAL_HISTORY>",
    history || "(không có lịch sử canonical trước boundary)",
    "<END_FROZEN_CANONICAL_HISTORY>",
    "",
    "<BEGIN_ONE_TIME_ADMIN_ANSWER>",
    String(row.adminAnswer || ""),
    "<END_ONE_TIME_ADMIN_ANSWER>",
  ].join("\n");
}

async function runClarificationResume(id, automaticContext, originToken, conversationGeneration) {
  const coordinatorHarness = automaticContext?.__adminClarificationCoordinatorHarness;
  if (coordinatorHarness?.runResume) {
    return coordinatorHarness.runResume({
      id,
      automaticContext,
      originToken,
  ...
  return resumeAdminClarification(id, {
    isAuthorityCurrent: async () => {
      const row = await getAdminClarificationById(id);
      return row
        ? authorityForClarification(row, automaticContext, originToken, conversationGeneration)
        : false;
    },
    generate: async (row) => withGlobalAiSlot({
      ownerUid: row.ownerUid,
      threadId: row.customerThreadId,
      orderKey: Date.now(),
    }, async () => ownerCredentials.withCurrentOwnerCredentialRead(
      row.ownerUid,
      aiChat.getConfig(row.ownerUid),
      async () => aiChat.generateAdminClarificationFinalReply(
        await clarificationPrompt(row),
        {
          id: `admin-clarification-${row.id}`,
          threadId: row.customerThreadId,
          threadType: 0,
          senderId: row.requesterUid,
          senderName: row.requesterName,
          content: "",
        },
        row.ownerUid
      )
    )),
    send: (row) => sendChatMessage(
      {
        threadId: row.customerThreadId,
        threadType: 0,
        text: row.customerFinalReplyText,
        originToken,
      },
      {
        ...tuyChonGuiTuDong({ ...automaticContext, conversationGeneration }),
...
function syntheticAdminClarificationMessage(row) {
  return {
    id: `internal-admin-clarification-${row.id}`,
    threadId: row.customerThreadId,
    threadType: 0,
    senderId: row.requesterUid,
    senderName: row.requesterName,
    content: "",
    isSelf: false,
    msgType: "internal.admin_clarification_resume",
    ts: Date.now(),
    __adminClarificationResumeId: row.id,
  };
}

function enqueueAdminClarificationCoordinatorWork(
  row,
  automaticWork,
  { messagesBefore = [], messagesAfter = [], includeSynthetic = true } = {}
) {
  const synthetic = includeSynthetic ? [syntheticAdminClarificationMessage(row)] : [];
  return dieuPhoiHoiThoai.them({
    ownerUid: row.ownerUid,
    threadId: row.customerThreadId,
    tins: [...messagesBefore, ...synthetic, ...messagesAfter],
    automaticWork,
  });
}

async function scheduleAdminClarificationResume(id, harnessOptions = null) {
  const row = harnessOptions?.row || await getAdminClarificationById(id);
  if (!row) return false;
  let automaticWork = null;
  if (!harnessOptions) {
    if (!api || !appState.loggedIn || String(row.ownerUid) !== String(chuHienTai() || "")) return false;
    const originToken = taoOriginRuntime();
    const threadKey = khoaThreadEligibility(row.ownerUid, row.customerThreadId);
    automaticWork = Object.freeze({
      originToken,
      capturedBotEligibilityEpoch: botEligibilityEpoch,
      threadEligibilityKey: threadKey,
      capturedThreadEligibilityEpoch: threadEligibilityEpochHienTai(threadKey),
    });
  } else {
    automaticWork = harnessOptions.automaticWork || null;
  }
  const scheduled = enqueueAdminClarificationCoordinatorWork(row, automaticWork, {
    messagesBefore: harnessOptions?.messagesBefore || [],
    messagesAfter: harnessOptions?.messagesAfter || [],
  });
  if (harnessOptions?.awaitCompletion) {
    await scheduled;
  } else {
    scheduled.catch((error) =>
      console.error("[admin-clarification] Resume coordinator error:", error.message)
    );
  }
  return true;
}
...
/**
 * Focused behavior harness: runs the production singleton coordinator and its
 * real traLoiCumTin runner, replacing only external resume/customer effects.
 * It is not wired to HTTP/runtime paths and cannot create a second coordinator.
 */
export async function runAdminClarificationCoordinatorHarnessForTests(
  {
    ownerUid,
    customerThreadId,
    resumeId = null,
    requesterUid = "fixture-customer",
    requesterName = "Fixture Customer",
    messagesBefore = [],
    messagesAfter = [],
    segmentBatches = null,
  },
  ...
  if (!hooks?.runResume || !hooks?.runCanonicalCustomer) {
    throw new TypeError("Coordinator harness requires resume and canonical-customer hooks.");
  }
  if (adminClarificationCoordinatorHarnessActive) {
    throw new Error("Coordinator harness is already active.");
  }
  const row = {
    id: resumeId,
    ownerUid: String(ownerUid),
    customerThreadId: String(customerThreadId),
    requesterUid,
    requesterName,
  };
  const harnessAutomaticWork = Object.freeze({
    originToken: null,
    __adminClarificationCoordinatorHarness: hooks,
  });
  adminClarificationCoordinatorHarnessActive = true;
  try {
    if (Array.isArray(segmentBatches)) {
      for (const batch of segmentBatches) {
        if (batch?.resumeId !== null && batch?.resumeId !== undefined) {
          void scheduleAdminClarificationResume(batch.resumeId, {
            row: { ...row, id: batch.resumeId },
            automaticWork: harnessAutomaticWork,
          });
        } else {
          void enqueueAdminClarificationCoordinatorWork(row, harnessAutomaticWork, {
            messagesBefore: batch?.messages || [],
            includeSynthetic: false,
          });
        }
      }
      await dieuPhoiHoiThoai.choRanh(row.ownerUid, row.customerThreadId);
    } else if (resumeId !== null && resumeId !== undefined) {
      await scheduleAdminClarificationResume(resumeId, {
        row,
        messagesBefore,
        messagesAfter,
        automaticWork: harnessAutomaticWork,
      ...
      await enqueueAdminClarificationCoordinatorWork(row, harnessAutomaticWork, {
        messagesBefore,
        messagesAfter,
        includeSynthetic: false,
      });
    ...
    adminClarificationCoordinatorHarnessActive = false;
  }
}

configureAdminClarificationStore({
  claimAnswer: claimAdminClarificationAnswer,
  create: createAdminClarification,
  expireWaiting: expireWaitingAdminClarifications,
  getById: getAdminClarificationById,
  getByToken: getAdminClarificationByToken,
  getWaiting: getWaitingAdminClarification,
  getRecentFailedAck: getRecentFailedAdminClarificationAck,
  listRecoverable: listRecoverableAdminClarifications,
  touchWaiting: touchWaitingAdminClarification,
  transition: transitionAdminClarification,
});

configureAdminClarificationRuntime({
  notifyAdmin: async ({ ownerUid, text, automaticWork }) => {
    if (!api || !appState.loggedIn || String(ownerUid) !== String(chuHienTai() || "")) {
      return { sent: false, reason: "RUNTIME_NOT_CURRENT", message: null };
    }
    const originToken = automaticWork?.originToken || taoOriginRuntime();
    return sendAdminNotification({
      ownerUid,
      text,
      send: sendChatMessage,
      originToken,
      sendOptions: automaticWork ? tuyChonGuiTuDong(automaticWork) : undefined,
    });
  },
  scheduleResume: scheduleAdminClarificationResume,
  log: (level, event, detail) => addLog({ event, level, summary: event, detail }),
});

function ownerCuaAutomaticWork(automaticWork) {
  return String(automaticWork?.originToken?.originOwnerUid || chuHienTai() || "").trim();
}

const boGom = taoBoGom({
  conDangGo,
  threadDangBan: (tin, automaticWork) => dieuPhoiHoiThoai.threadDangBan(
    ownerCuaAutomaticWork(automaticWork),
    tin.threadId
  ),
  conDangBan: (ownerUid, threadId, senderId) => dieuPhoiHoiThoai.dangBan(
    ownerUid,
    threadId,
    senderId
  ),
  khiHuyTheoThread: (threadId, ownerUid) => dieuPhoiHoiThoai.huyTheoThread(
    ownerUid || chuHienTai(),
    threadId,
    "thread_eligibility_transition"
  ),
  khiHuyTatCa: () => dieuPhoiHoiThoai.huyTatCa("global_or_runtime_transition"),
  khiChot: (tins, automaticWork) => {
    const ownerUid = ownerCuaAutomaticWork(automaticWork);
    const threadId = String(tins?.[0]?.threadId || "").trim();
    dieuPhoiHoiThoai.them({ ownerUid, threadId, tins, automaticWork })
      .catch((error) => console.error("[zalo] Loi tra loi:", error));
  },
});

async function layDurableRuntimeAuthority({ accountId, conversationId }) {
  const ownerUid = String(accountId || "").trim();
  const threadId = String(conversationId || "").trim();
  if (!ownerUid || !threadId || !api || !appState.loggedIn) return null;
  if (ownerUid !== String(chuHienTai() || "")) return null;
  if (!aiChat.getConfig(ownerUid)?.botEnabled) return null;
  const thread = await getThread(ownerUid, threadId);
  if (!thread || thread.botEnabled === false) return null;

  const originToken = captureRuntimeAuthority();
  if (!originToken || !originConHieuLuc(originToken)) return null;
  const threadEligibilityKey = khoaThreadEligibility(ownerUid, threadId);
  const automaticWork = Object.freeze({
    originToken,
    capturedBotEligibilityEpoch: botEligibilityEpoch,
    threadEligibilityKey,
    capturedThreadEligibilityEpoch: threadEligibilityEpochHienTai(threadEligibilityKey),
  });
  return automaticWorkConHieuLuc(automaticWork) ? { originToken, automaticWork } : null;
}

capHinhDurableDispatcher({
  layAuthority: layDurableRuntimeAuthority,
  enqueue: ({ message, automaticWork }) => boGom.them(message, automaticWork),
  gui: (outbox, authority, durableHooks = {}) => {
    if (outbox.outboundKind === "REACTION") {
      return reactToMessage(outbox.payload, authority.originToken, durableHooks);
    }
    return sendChatMessage(
      { ...outbox.payload, originToken: authority.originToken },
      {
        ...tuyChonGuiTuDong(authority.automaticWork),
        capturedRuntimeAuthority: authority.originToken,
        reportAuthorityRejectionBeforeProvider: true,
        ...durableHooks,
      }
    );
  },
  thongBaoAdmin: thongBaoAdminLoiOutbound,
});
...
function batDauGoPhim(threadId, threadType, originToken = null, automaticWork = null) {
  let dungRoi = false;
  const automaticWorkHopLe = () => automaticWorkConHieuLuc(automaticWork);
  const nhac = () => {
    if (originToken && !originConHieuLuc(originToken)) return;
    if (!automaticWorkHopLe()) return;
...
/** Canonical Web Inbox typing/status controller for WAITING + GENERATING. */
function batDauWebTyping(
  ownerUid,
  threadId,
  originToken = null,
  automaticWork = null,
  ...
  const chu = String(ownerUid || "").trim();
  const hoiThoai = String(threadId || "").trim();
  if (!chu || !hoiThoai) {
    const noop = () => {};
    noop.setPhase = () => {};
    return noop;
  ...
  let daDung = false;
  let phase = initialPhase === "generating" ? "generating" : "waiting";
  let dongHo = null;
  const conHieuLuc = () => (!originToken || originConHieuLuc(originToken))
    && automaticWorkConHieuLuc(automaticWork);
  const phat = (typing) => {
    const counts = getGlobalAiLimiterSnapshot();
    io?.emit("bot_typing_status", {
      ownerUid: chu,
      threadId: hoiThoai,
      typing,
      phase: typing ? phase : null,
      activeCount: counts.activeCount,
      waitingCount: counts.normalWaitingCount,
      limit: counts.limit,
      at: Date.now(),
    });
  };
  const nhac = () => {
    ...
    updateOwnerAiRuntimePhase({ ownerUid: chu, threadId: hoiThoai, phase });
    phat(true);
  };
  const stop = () => {
    if (daDung) return;
    daDung = true;
    if (dongHo !== null) clearInterval(dongHo);
    clearOwnerAiRuntimePhase(chu, hoiThoai);
    phat(false);
  };
  stop.setPhase = (nextPhase) => {
    if (daDung || !["waiting", "generating"].includes(nextPhase)) return;
    phase = nextPhase;
    nhac();
  };
  nhac();
  dongHo = setInterval(nhac, NHAC_GO_PHIM_MS);
  return stop;
}
...
/**
 * Nuot ban "@Ten nguoi noi" ma AI tu viet o DAU bubble, tra ve phan con lai.
 *
 * Ung dung moi la chu cua the nhac ten dau bubble. Khi AI cung viet "@Ten" o
 * dau cau thi khach doc duoc hai lan: "@Bich Ngoc @Bich Ngoc ...". Ham nay cat
 * dung phan dau do de hai ben khong ghep chong len nhau.
 *
 * Lam viec bang CHI SO tren chuoi goc: ban NFC chi de SO SANH, con phan tra ve
 * luon la mot lat cat cua chinh chuoi goc - giu nguyen khoang trang, dau cau va
 * cach bieu dien Unicode ma AI da viet.
 *
 * Tha khong don con hon don nham loi nguoi khac: trong nhom co ai trung ten
 * thi tra lai nguyen van cho luong cu xu ly.
 */
function canonicalizeLeadingSpeakerMention(body, tenNguoiNoi, uidNguoiNoi, thanhVien) {
  const goc = String(body ?? "");
  const ten = String(tenNguoiNoi ?? "").normalize("NFC");
  // CHI xet tu dau chuoi. Khong co dau @ ngay o vi tri 0 thi khong co gi de nuot.
  if (!goc || !ten || goc[0] !== "@") return goc;

  // Ten nguoi KHAC chi dung de CHAN, khong bao gio dung de nhan dang nguoi noi.
  const tenNguoiKhac = (thanhVien || [])
    .filter((tv) => String(tv?.uid ?? "") !== String(uidNguoiNoi ?? ""))
    .map((tv) => String(tv?.ten ?? "").normalize("NFC"))
    .filter(Boolean);
  // Trong nhom co nguoi trung y ten nguoi noi -> khong doan AI dang nhac ai.
  if (tenNguoiKhac.includes(ten)) return goc;

  let ranh = -1;    // vi tri ngay sau bieu dien vua nuot duoc
  let viTri = 1;    // con tro doc, dang dung ngay sau dau '@' mo dau
  let dauManh = 1;  // dau cua manh "@..." dang doc
  let dung = "";    // ban dung lai - CHI de so sanh, khong bao gio xuat ra
  let manh = [];    // [start, end) tung manh, de xet nhap nhang khi ten bi tach roi

  for (;;) {
    const dauChu = viTri;
    while (viTri < goc.length && !/\s/u.test(goc[viTri])) viTri += 1;
    if (viTri === dauChu) break; // dau '@' khong co chu nao theo sau
    dung += goc.slice(dauChu, viTri);
    const dungNFC = dung.normalize("NFC");

    if (dungNFC === ten) {
      // Cho noi cua ung dung la DUNG MOT dau cach U+0020. Bien sau bieu dien
      // chi duoc la het chuoi hoac dau cach do; tab/xuong dong/CR khong phai
      // cho noi, gap la khong dam nuot bieu dien nay nua.
      if (viTri < goc.length && goc[viTri] !== " ") break;
      manh.push([dauManh, viTri]);
      // Manh nao trung khop ten mot thanh vien khac thi tha khong don.
      if (manh.some(([a, b]) => tenNguoiKhac.includes(goc.slice(a, b).normalize("NFC")))) break;
      ranh = viTri;

      // "@Sender @Sender ..." - chi thu bieu dien ke tiep khi cach DUNG MOT
      // dau cach roi toi ngay dau '@'. Hai dau cach thi dung lai o day, khong
      // gop chung lai thanh mot.
      if (goc[viTri] !== " " || goc[viTri + 1] !== "@") break;
      viTri += 2;
      dauManh = viTri;
      dung = "";
      manh = [];
    ...
    if (dungNFC.length > ten.length) break; // da dai hon ten -> het cua khop

    // Khoang trang BEN TRONG ten, khong phai cho noi: khong bo di ky tu nao,
    // ca dai duoc cong nguyen van vao ban dung lai roi phai khop DUNG BANG
    // senderName. Ten co hai dau cach thi body cung phai co hai dau cach.
    const dauTrang = viTri;
    while (viTri < goc.length && /\s/u.test(goc[viTri])) viTri += 1;
    if (viTri === dauTrang || viTri >= goc.length) break;
    dung += goc.slice(dauTrang, viTri);
    if (goc[viTri] === "@") {
      // Dang tach: "@Tran @Mai Anh" - bo DUNG dau '@' cua manh ke tiep.
      manh.push([dauManh, dauTrang]);
      viTri += 1;
      dauManh = viTri;
    }
  }

  if (ranh < 0) return goc;

  // Ung dung chi so huu DUNG MOT dau cach U+0020 lam cho noi. Dau cach thu
  // hai tro di van la cua than bai: cat dung mot o roi tra phan con lai
  // nguyen van bang chi so tren chuoi goc.
  return goc.slice(goc[ranh] === " " ? ranh + 1 : ranh);
}
...
async function dungTheNhacTen(bubble, tin, laBubbleDau, ownerUid = chuHienTai(), ownerApi = api) {
  ...
  // MOT anh chup thanh vien duy nhat cho ca hai viec ben duoi: chan nhap nhang
  // khi don ten nguoi noi, va khop ten thanh vien trong cau. Goi hai lan thi
  // hai ban co the lech nhau ngay trong cung mot luot tra loi.
  ...
  if (laBubbleDau && uidNguoiNoi && tenNguoiNoi) {
    // AI co the da tu viet "@Ten" ngay dau bubble. The nhac ten dau bubble la
    // cua ung dung, nen nuot ban cua AI roi tu dat lai DUNG MOT cai.
    const conLai = canonicalizeLeadingSpeakerMention(bubbleCuoi, tenNguoiNoi, uidNguoiNoi, thanhVien);
    noiDung = conLai ? `${tenNguoiNoi} ${conLai}` : tenNguoiNoi;
...
/** Canonical AI-only preprocessing for both segment and aggregate calls. */
function gopThanhMotTin(tins) {
  const laChatRieng = String(tins[0]?.threadType) === "0";
  const coChuThuc = tins.some((tin) =>
    tin.msgType !== "chat.sticker"
    && String(tin.content || "").trim()
    && !/^https?:\/\/\S+$/.test(String(tin.content).trim())
  );
  const coMediaKhac = tins.some((tin) => tin.msgType === "chat.photo" || tin.msgType === "share.file");
  let daDatPlaceholder = false;
  tins = tins.map((tin) => {
    if (tin.msgType !== "chat.sticker") return { ...tin };
    let id = null;
    try {
      const parsed = Number(JSON.parse(String(tin.content || ""))?.id);
      if (Number.isInteger(parsed) && parsed > 0) id = parsed;
    } catch { /* malformed Sticker still receives a safe failure marker */ }
    const placeholder = laChatRieng && !coChuThuc && !coMediaKhac && !daDatPlaceholder;
    if (placeholder) daDatPlaceholder = true;
    return {
      ...tin,
      content: placeholder ? "(Khách vừa gửi một Sticker.)" : "",
      __stickerItem: { id },
    };
  });
  ...
  const stickerItems = tins.filter((t) => t.__stickerItem).map((t) => t.__stickerItem);
  const customerText = tins
    .filter((t) => t.msgType !== "chat.sticker")
    .map((t) => String(t.content || "").trim())
  ...
  return {
    ...
    ...(stickerItems.length ? { __stickerItems: stickerItems } : {}),
    ...(stickerItems.length ? { __customerText: customerText } : {}),
  };
}

/** Resolve only the first Sticker, under the captured Zalo runtime authority. */
async function chuanBiStickerVisionChoAi(tin, runtimeApi, isCurrent = () => true) {
  if (String(tin?.threadType) !== String(ThreadType.User)
    || !tin?.__stickerItems?.length
    || tin.msgType === "chat.photo"
    || tin.msgType === "share.file") return tin;
  const stickerId = tin.__stickerItems[0].id;
  if (!isCurrent()) return null;
  if (!stickerId) return { ...tin, __stickerVision: { url: null } };
  const cachedUrl = getCachedStickerUrl(stickerId);
  const url = await resolveStickerUrl(runtimeApi, stickerId);
  if (!isCurrent()) return null;
  return {
    ...tin,
    __stickerVision: {
      url,
      refreshUrl: cachedUrl && cachedUrl === url
        ? async (failedUrl) => isCurrent()
          ? refreshStickerUrl(runtimeApi, stickerId, failedUrl)
          : null
        : null,
    },
  };
 ...
 */
async function thuThaCamXuc(tin, originToken = null, automaticWork = null) {
  const automaticWorkHopLe = () => automaticWorkConHieuLuc(automaticWork);
  if (originToken && !originConHieuLuc(originToken)) return false;
  if (!automaticWorkHopLe()) return false;
  ...
  if (originToken && !originConHieuLuc(originToken)) return false;
  if (!automaticWorkHopLe()) return false;
  const conversationGeneration = automaticWork?.conversationGeneration || null;
  if (conversationGeneration && !conversationGeneration.giuChoOutbound()) return false;
  let reactionProviderCommitted = false;
  ...
  const [durableReaction] = conversationGeneration?.durableGenerationKey
    ? await chuanBiDurableOutbox(conversationGeneration, [{
      outboundKind: "REACTION",
  ...
  try {
    const react = async (durableHooks = {}) => await reactToMessage(
        reactionPayload,
        originToken,
        {
          onProviderSuccess: (providerResult) => {
            conversationGeneration?.xacNhanOutbound();
            reactionProviderCommitted = true;
            durableHooks.onProviderSuccess?.(providerResult);
          },
        }
      );
    if (durableReaction) {
      await guiDurableOutbound({ outbox: durableReaction, conversationGeneration, send: react });
    } else {
      await react();
    }
  } catch (error) {
    // Provider success la physical fact. Post-provider runtime check co the throw,
    // nhung khong duoc release, fallback text hay requeue reaction da giao.
    if (reactionProviderCommitted) {
      console.warn("[cam-xuc] Da tha thanh cong nhung runtime doi sau provider:", error.message);
      return true;
    }
    conversationGeneration?.traLaiOutbound();
    if (durableReaction) {
      if (conversationGeneration) conversationGeneration.durableFailure = error;
      // Durable reaction intent owns the retry now; do not manufacture a text
      // fallback that could race and duplicate the customer-facing outcome.
      return true;
    }
  ...
  if (!automaticWorkHopLe()) return true;
  await addLog({
    event: "tha_cam_xuc",
    level: "ok",
 ...
 */
async function thuGuiSticker(tin, originToken = null, automaticWork = null) {
  try {
    const automaticWorkHopLe = () => automaticWorkConHieuLuc(automaticWork);
    if (originToken && !originConHieuLuc(originToken)) return;
    if (!automaticWorkHopLe()) return;
 ...
 */
function guiDaXemChoTins(tins, originToken = null, automaticWork = null) {
  const nguon = phongBiDaXemMoiNhat(tins);
  if (!nguon) return;
  Promise.resolve()
    .then(() => {
      if (!automaticWorkConHieuLuc(automaticWork)) return null;
      return markMessageSeen({ envelope: nguon.phongBi, threadType: nguon.threadType }, originToken);
    })
    .catch(() => {});
}

const BO_LICH_SU_STALE = [
  "# GHI CHÚ ĐỒNG BỘ NỘI BỘ — BẮT BUỘC",
  "Một hay nhiều phản hồi assistant gần nhất cho các tin được lặp lại bên dưới đã bị ứng dụng loại bỏ và CHƯA HỀ được gửi tới khách.",
  "Không được giả định khách đã đọc hoặc nhận các phản hồi đó. Hãy coi toàn bộ tin khách bên dưới là chưa được trả lời và trả lời lại đầy đủ từ đầu.",
  "",
  "# TIN KHÁCH CHƯA ĐƯỢC TRẢ LỜI",
].join("\n");

async function traLoiCumTin(workOrTins, automaticWork = null, conversationGeneration = null) {
  const guiDaXemChoTinsTrongCum = (tins, token, work) => guiDaXemChoTins(tins, token, work);
  const laCoordinatorWork = !Array.isArray(workOrTins)
    && Array.isArray(workOrTins?.segments);
  let segments = laCoordinatorWork
    ? workOrTins.segments
    : [{
        senderId: String(workOrTins?.[0]?.senderId ?? ""),
        tins: workOrTins || [],
        automaticWork,
        preAiDone: false,
        aiEligible: true,
  ...
  const automaticContext = coAutomaticWork
    ? {
        ...automaticWork,
    ...
    : null;
  const coordinatorHarness = automaticContext?.__adminClarificationCoordinatorHarness;
  coordinatorHarness?.captureOriginalSegments?.({
    work: laCoordinatorWork ? workOrTins : null,
    segments: [...segments],
  });
  const syntheticResumes = segments
    .flatMap((segment) => segment.tins || [])
    .filter((message) => message?.__adminClarificationResumeId);
  // Internal recovery work is not a provider message. Branch before receipts,
  // PDF automation, reactions, typing, or any other provider-facing pre-effect.
  if (syntheticResumes.length) {
    const resumeIds = [...new Set(syntheticResumes.map((message) =>
      message.__adminClarificationResumeId
    ))];
    for (const resumeId of resumeIds) {
      await runClarificationResume(
        resumeId,
        automaticContext,
        originToken,
        conversationGeneration
    ...
    // Keep real provider items inside this active work. Re-enqueueing them is
    // invalid because coordinator dedup intentionally includes active.work.tins.
    for (const segment of segments) {
      segment.tins = (segment.tins || []).filter((message) =>
        !message?.__adminClarificationResumeId
      );
    }
    if (laCoordinatorWork) {
      workOrTins.tins = (workOrTins.tins || []).filter((message) =>
        !message?.__adminClarificationResumeId
      );
    }
    segments = segments.filter((segment) => segment.tins.length > 0);
    if (!segments.length) return;
  }
  if (coordinatorHarness?.runCanonicalCustomer) {
    return coordinatorHarness.runCanonicalCustomer({
      segments,
      messages: segments.flatMap((segment) => segment.tins || []),
      conversationGeneration,
    });
  }
  const botWorkConHieuLuc = () => automaticWorkConHieuLuc(automaticContext)
    && conversationGeneration?.durableAuthorityLost !== true;
  const automaticSendOptions = () => ({
    ...(tuyChonGuiTuDong(automaticContext) || {}),
    reportAuthorityRejectionBeforeProvider: true,
  });
  const ghiStaleOutboundSkipped = async (tin, reason) => {
    if (!conversationGeneration?.danhDauStaleOutboundSkipped()) return;
    await addLog({
      event: "stale_outbound_skipped",
      level: "warn",
      summary: "Đã bỏ customer outbound của generation không còn hợp lệ",
      detail: {
        ownerUid: conversationGeneration.ownerUid,
        threadId: tin?.threadId || conversationGeneration.threadId,
        senderId: tin?.senderId || null,
        generationId: conversationGeneration.id,
        reason,
      },
    }).catch(() => {});
  };
  ...
  // Moi segment la mot bucket arrival goc. Pre-AI stateful effects chay dung mot
  // lan tren segment do; stale replay chi tai su dung logical AI input.
  for (const segment of segments) {
    if (segment.preAiDone) continue;
    const segmentTins = segment.tins || [];
    if (!segmentTins.length) {
      segment.preAiDone = true;
      segment.aiEligible = false;
      continue;
    }
    const segmentTin = gopThanhMotTin(segmentTins);
    if (!segmentTin.content?.trim()) {
      segment.preAiDone = true;
      segment.aiEligible = false;
      continue;
    }

    const segmentAutomaticWork = segment.automaticWork || automaticWork;
    const segmentCoAutomaticWork = segmentAutomaticWork
      && Object.prototype.hasOwnProperty.call(segmentAutomaticWork, "originToken");
    const segmentOriginToken = segmentCoAutomaticWork
      ? segmentAutomaticWork.originToken
      : segmentAutomaticWork;
    const segmentAutomaticContext = segmentCoAutomaticWork
      ? {
          ...segmentAutomaticWork,
          ...(conversationGeneration ? { conversationGeneration } : {}),
        }
    ...
    // Exact OK duoc kiem tra tren tung bucket goc. Khong merge cross-sender.
    // Pending phai duoc chup TRUOC handler vi handler consume no ngay khi gap OK.
    // Neu generation stale truoc khi customer outbound duoc chap nhan, khoi phuc
    // dung pending cu va de preAiDone=false de coordinator replay chinh handler.
    if (typeof handlePdfAutomation === "function") {
      const pdfPendingOwnerUid = String(
        segmentOriginToken?.originOwnerUid || chuHienTai() || ""
      ).trim();
      const pdfPendingThreadId = String(segmentTin.threadId || "").trim();
      const pdfPendingSenderId = String(segmentTin.senderId || "").trim();
      const pendingPdfTruoc = getPendingPdfConfirmation(
        pdfPendingOwnerUid,
        pdfPendingThreadId,
        pdfPendingSenderId
      );
      const pdfAutomationResult = await handlePdfAutomation({
        tins: segmentTins,
        tin: segmentTin,
        originToken: segmentOriginToken,
        automaticWork: segmentAutomaticContext,
      });

      const pdfCanReplay = Boolean(
        conversationGeneration?.stale
        && !conversationGeneration.cancelled
        && !conversationGeneration.daChapNhanOutbound()
      );
      if (pdfCanReplay) {
        if (pendingPdfTruoc && hasExactPdfConfirmation(segmentTins, pdfPendingSenderId)) {
          setPendingPdfConfirmation(
            pdfPendingOwnerUid,
            pdfPendingThreadId,
            pdfPendingSenderId,
        ...
        segment.preAiDone = false;
        return;
      }

      segment.preAiDone = true;
      if (pdfAutomationResult === PDF_AUTOMATION_HANDLED) {
        segment.aiEligible = false;
        continue;
      }
    } else {
    ...
    // Seen giu behavior cu nhung khong replay thua tren stale logical input.
    guiDaXemChoTinsTrongCum(
      segmentTins,
      segmentOriginToken,
      segmentCoAutomaticWork ? segmentAutomaticWork : null
    );
  }

  const tins = segments
    .filter((segment) => segment.aiEligible !== false)
    .flatMap((segment) => segment.tins || []);
  if (!tins.length) return;
  const tin = gopThanhMotTin(tins);
  if (!tin.content?.trim()) return;

  // Reaction la customer outbound cua generation: chi commit ngay tai bien gui,
  // sau khi generation da duoc revalidate; stale generation gui zero reaction.
  if (tins.length === 1 && (await thuThaCamXuc(tin, originToken, automaticContext))) return;
  if (originToken && !originConHieuLuc(originToken)) return;
  if (!botWorkConHieuLuc()) return;

  const inferenceOwnerUid = String(originToken?.originOwnerUid || chuHienTai() || "").trim();
  const ghiDungPartialViContextTienLen = (attempt, sendError = null) => addLog({
    event: "outbound_partial_stopped_context_advanced",
    level: "warn",
    summary: "Đã dừng phần trả lời cũ bị Zalo từ chối vì hội thoại có tin khách mới.",
    detail: {
      ownerUid: inferenceOwnerUid,
      threadId: tin.threadId,
      failedBubbleIndex,
      totalBubbleCount,
      sentBubbleCount,
      providerAttempts: attempt,
      outcome: outboundFailureOutcome,
      providerCode: Number.isFinite(sendError?.code) ? sendError.code : null,
    },
  }).catch(() => {});
  const durableOrigin = durableIds(laCoordinatorWork ? workOrTins?.tins : tins).length > 0;
  if (durableOrigin && (!conversationGeneration?.durableGenerationKey
    || Number(conversationGeneration?.expectedMemberCount || 0) <= 0)) {
    if (conversationGeneration) conversationGeneration.durableAuthorityLost = true;
    return;
  }
  ...
  const stopLeaseHeartbeat = () => {
    ...
    if (leaseHeartbeat !== null) clearInterval(leaseHeartbeat);
    leaseHeartbeat = null;
  };
  const renewLease = async () => {
    if (!durableOrigin || leaseHeartbeatStopped) return !durableOrigin;
    const healthy = await giaHanLeaseDurableGeneration(conversationGeneration, 300_000);
    if (!healthy) {
      conversationGeneration.durableAuthorityLost = true;
      cancelGlobalAiWaiter(conversationGeneration, "durable_authority_lost");
      stopLeaseHeartbeat();
      await addLog({
        event: "lease_heartbeat_failed",
        level: "error",
        summary: "Durable generation mất authority khi gia hạn lease.",
        detail: {
          ownerUid: inferenceOwnerUid,
          threadId: tin.threadId,
          generationKey: conversationGeneration?.durableGenerationKey || null,
          expectedMemberCount: conversationGeneration?.expectedMemberCount || 0,
        },
      }).catch(() => {});
    }
    return healthy;
  ...
  if (durableOrigin) {
    leaseHeartbeat = setInterval(() => { void renewLease(); }, 30_000);
    leaseHeartbeat.unref?.();
    conversationGeneration.p2StopLeaseHeartbeat = stopLeaseHeartbeat;
  }

  // Mot canonical controller cho tung channel bao phu WAITING roi GENERATING.
  let tatGoPhim = batDauGoPhim(tin.threadId, tin.threadType, originToken, automaticContext);
  const tatWebTyping = batDauWebTyping(
    inferenceOwnerUid,
    tin.threadId,
    originToken,
    automaticContext,
  ...
  let waitingTypingStopped = false;
  const waitingTypingCap = setTimeout(() => {
    if (waitingTypingStopped) return;
    waitingTypingStopped = true;
    tatGoPhim();
  }, 300_000);
  waitingTypingCap.unref?.();
  const stopWaitingTyping = () => {
    if (waitingTypingStopped) return;
    waitingTypingStopped = true;
    clearTimeout(waitingTypingCap);
    tatGoPhim();
  };
  if (conversationGeneration) {
    conversationGeneration.p2StopWaitingTyping = stopWaitingTyping;
    conversationGeneration.p2StopWebTyping = tatWebTyping;
  }
  ...
  let outboundFailureOutcome = null;
  let clarificationAckId = null;
  let clarificationAckCompleted = false;
  let clarificationAckHandedOff = false;
  let aiMessageForReply = null;
  // Repair A: mot luot AI dang le phai tra loi nhung that bai phai den duoc
  // durable failure machinery. Outcome duoc CHUP trong luc goi tryReply nhung
  // chi duoc TIEU THU sau khi cac authority check hien huu da pass, de khong
  // bao gio ghi durable failure cho mot generation da mat quyen.
  const MA_LOI_KHONG_PHAN_LOAI = "UNCLASSIFIED_REPLY_OUTCOME";
  const MA_LOI_RONG_BUBBLE = "EMPTY_BUBBLES_AFTER_FILTER";
  let aiReplyAttempted = false;
  let aiOutcome = null;
  let aiOutcomeConsumed = false;
  const ghiDurableFailure = (failureCode, ownerText = null) => {
    if (!conversationGeneration) return;
    const code = String(failureCode || "").trim() || MA_LOI_KHONG_PHAN_LOAI;
    const failure = new Error(String(ownerText || "").trim() || code);
    failure.code = code;
    conversationGeneration.durableFailure ||= failure;
    conversationGeneration.durableFailureCode ||= code;
  };
  const tieuThuKetQuaAi = () => {
    if (!aiReplyAttempted || aiOutcomeConsumed) return;
    aiOutcomeConsumed = true;
    if (aiReply) return;
    if (aiOutcome?.kind === "TERMINAL_NO_REPLY") return;
    if (aiOutcome?.kind === "FAILED") {
      ghiDurableFailure(aiOutcome.failureCode, aiOutcome.ownerText);
      return;
    }
    // Fail closed: null ma khong co outcome tuong minh KHONG phai no-op thanh cong.
    ghiDurableFailure(MA_LOI_KHONG_PHAN_LOAI);
  };

  try {
    const inferenceConfig = aiChat.getConfig(inferenceOwnerUid);
    const noiDungChoAi = laCoordinatorWork && workOrTins.staleProviderHistory
      ? `${BO_LICH_SU_STALE}\n${tin.content}`
      : tin.content;
    aiMessageForReply = {
      ...tin,
      __adminClarificationAutomaticWork: automaticContext,
    };
    const admittedTimes = (laCoordinatorWork ? workOrTins?.tins : tins)
      .map((message) => Number(message?.__durableAdmittedAt))
      .filter(Number.isFinite);
    const orderKey = admittedTimes.length ? Math.min(...admittedTimes) : Date.now();
    aiReply = await withGlobalAiSlot({
      ownerUid: inferenceOwnerUid,
      threadId: tin.threadId,
      orderKey,
      waiterKey: conversationGeneration,
      onGranted: () => {
        stopWaitingTyping();
        tatGoPhim = batDauGoPhim(tin.threadId, tin.threadType, originToken, automaticContext);
        tatWebTyping.setPhase?.("generating");
      },
    }, async () => {
      // Grant khong mang authority qua thoi gian cho: revalidate truoc credential READ.
      if (originToken && !originConHieuLuc(originToken)) return null;
      if (!botWorkConHieuLuc()) return null;
      if (conversationGeneration && !conversationGeneration.conHieuLuc()) return null;
      if (inferenceConfig?.botEnabled === false) return null;
      if (durableOrigin && !await renewLease()) return null;
      if (conversationGeneration?.durableAuthorityLost) return null;
      conversationGeneration?.danhDauProviderHistory();
      if (aiMessageForReply.__stickerItems?.length) {
        aiMessageForReply = await chuanBiStickerVisionChoAi(
          aiMessageForReply,
          originToken?.originApiIdentity || api,
          () => (!originToken || originConHieuLuc(originToken)) && botWorkConHieuLuc()
        );
        if (!aiMessageForReply) return null;
      }
      return ownerCredentials.withCurrentOwnerCredentialRead(
        inferenceOwnerUid,
        inferenceConfig,
        () => {
          aiReplyAttempted = true;
          return aiChat.tryReply(noiDungChoAi, aiMessageForReply, {
            recordOutcome: (outcome) => { aiOutcome = outcome; },
          });
        }
      );
    });
    if (aiReply && aiMessageForReply?.senderId) {
      aiChat.scheduleCustomerSummary?.(
        inferenceConfig,
        aiMessageForReply,
        inferenceOwnerUid
      );
    }
    clarificationAckId = aiMessageForReply?.__adminClarificationAckId || null;
    const adminClarificationFallback = aiMessageForReply?.__adminClarificationFallback === true;
    if (originToken && !originConHieuLuc(originToken)) {
      if (aiReply) await ghiStaleOutboundSkipped(tin, "runtime_origin_invalid");
      return;
    }
    // Bot co the bi OFF roi ON trong luc model dang chay; boolean hien tai ON
    // khong du de cho phep continuation cua epoch cu.
    if (!botWorkConHieuLuc()) {
      if (aiReply) await ghiStaleOutboundSkipped(tin, "generation_or_automatic_work_invalid");
      return;
    }
    // Moi authority/current-work check hien huu da pass: gio moi duoc tieu thu.
    tieuThuKetQuaAi();
    if (!aiReply) return;

    const filteredDecisionReply = locRuotGan(aiReply);
    if (filteredDecisionReply.soMarkerQuyetDinhCat > 0) {
      aiReply = filteredDecisionReply.sach;
      await addLog({
        event: "chan_decision_marker",
        level: "warn",
        summary: `Đã cắt ${filteredDecisionReply.soMarkerQuyetDinhCat} marker quyết định nội bộ trước khi chuẩn bị outbound.`,
        detail: {
          threadId: tin.threadId,
          generationKey: conversationGeneration?.durableGenerationKey || null,
          markerClass: "VIZEN_DECISION",
          markerCount: filteredDecisionReply.soMarkerQuyetDinhCat,
        },
      }).catch(() => {});
    }
    ...
    if (!totalBubbleCount) {
      // AI co noi dung nhung sau loc khong con bubble nao cho khach: that bai.
      ghiDurableFailure(MA_LOI_RONG_BUBBLE);
      await addLog({
        event: "ai_skip",
        level: "warn",
    ...
    // Commit every bubble intent before the first provider call. A crash can
    // therefore resume the exact remaining slots without regenerating output.
    const durableTextOutboxes = conversationGeneration?.durableGenerationKey
      ? await chuanBiDurableOutbox(
        conversationGeneration,
        outboundBubbles.map(({ payload }) => ({ outboundKind: "TEXT", payload }))
      )
      : [];

    for (const [i, outboundBubble] of outboundBubbles.entries()) {
      const { bubble, payload } = outboundBubble;
      ...
      tatGoPhim();
      if (originToken && !originConHieuLuc(originToken)) {
        await ghiStaleOutboundSkipped(tin, "runtime_origin_invalid_before_bubble");
        return;
      }
      if (!botWorkConHieuLuc()) {
        await ghiStaleOutboundSkipped(tin, "generation_invalid_before_bubble");
        return;
      }

      if (originToken && !originConHieuLuc(originToken)) {
        await ghiStaleOutboundSkipped(tin, "runtime_origin_invalid_after_mentions");
        return;
      }
      if (!botWorkConHieuLuc()) {
        await ghiStaleOutboundSkipped(tin, "generation_invalid_after_mentions");
        return;
      }

      failedBubbleIndex = i + 1;
      let sentMessage = null;
      const MAX_OUTBOUND_ATTEMPTS = 2;
      for (let attempt = 1; attempt <= MAX_OUTBOUND_ATTEMPTS; attempt += 1) {
        try {
          const sendBubble = (durableHooks = {}) => sendChatMessage({
              ...payload,
              originToken,
            }, {
              ...automaticSendOptions(),
              ...(attempt > 1 ? { requireUnadvancedOutboundContext: true } : {}),
              ...durableHooks,
            });
          if (durableTextOutboxes[i]) {
            sentMessage = await guiDurableOutbound({
              outbox: durableTextOutboxes[i],
              conversationGeneration,
              send: sendBubble,
              nextAttemptDelayMs: i + 1 < outboundBubbles.length
                ? nghiTruocBubble(outboundBubbles[i + 1].bubble)
                : 0,
            });
          } else {
          ...
          if (conversationGeneration) conversationGeneration.durableFailure = sendError;
          if (sendError?.outboxDeliveryRecorded === true) {
            await addLog({
              event: "outbound_post_provider_local_failure",
              level: "error",
              summary: `Zalo đã gửi phần ${failedBubbleIndex}/${totalBubbleCount}, nhưng bước local sau provider bị lỗi.`,
              detail: {
                ownerUid: inferenceOwnerUid,
                threadId: tin.threadId,
                failedBubbleIndex,
          ...
          outboundFailureOutcome = sendError?.vizenOutboundOutcome
            || (sendError?.providerSucceeded ? "SEND_UNKNOWN" : "PRE_PROVIDER_FAILURE");
          // Release co the materialize staleDeferred. Khi do canonical stale
          // replay thang, tuyet doi khong retry reply cu.
          if (conversationGeneration && !conversationGeneration.conHieuLuc()) return;

          const contextDaTienLen = conversationGeneration?.outboundContextDaTienLen?.() === true;
          if (outboundFailureOutcome === "DEFINITIVE_PROVIDER_REJECTION" && contextDaTienLen) {
            await ghiDungPartialViContextTienLen(attempt, sendError);
            return;
          }

          const coTheRetry = !conversationGeneration?.durableGenerationKey
            && outboundFailureOutcome === "DEFINITIVE_PROVIDER_REJECTION"
            && attempt < MAX_OUTBOUND_ATTEMPTS;
          if (coTheRetry) {
            await addLog({
              event: "outbound_provider_retry",
              level: "warn",
              summary: `Zalo từ chối phần ${failedBubbleIndex}/${totalBubbleCount}; thử lại đúng phần này.`,
              detail: {
                threadId: tin.threadId,
                failedBubbleIndex,
                totalBubbleCount,
          ...
          await addLog({
            event: "outbound_terminal_failure",
            level: "error",
            summary: `Không gửi được phần ${failedBubbleIndex}/${totalBubbleCount}; đã dừng để tránh mất hoặc gửi trùng.`,
            detail: {
              ownerUid: inferenceOwnerUid,
              threadId: tin.threadId,
              failedBubbleIndex,
          ...
          if (typeof thongBaoAdminLoiOutbound === "function") {
            await thongBaoAdminLoiOutbound({
              ownerUid: inferenceOwnerUid,
              threadId: tin.threadId,
              failedBubbleIndex,
              outcome: outboundFailureOutcome,
        ...
        if (sentMessage?.outboundHandedOff === true) {
          await addLog({
            event: "outbound_claim_handoff",
            level: "warn",
            summary: `Durable outbound phần ${i + 1}/${totalBubbleCount} đã được worker khác tiếp quản.`,
            detail: {
              outboxId: sentMessage.outboxId || durableTextOutboxes[i]?.id || null,
              durableStatus: sentMessage.durableStatus || null,
              bubbleIndex: i + 1,
              sentBubbleCount,
              generationKey: conversationGeneration?.durableGenerationKey || null,
            },
          }).catch(() => {});
          if (clarificationAckId && !clarificationAckCompleted) clarificationAckHandedOff = true;
          return;
        }
        // Sentinel nay chi co nghia provider CHUA duoc goi. null van co the la
        // provider success khong msgId va phai duoc tinh la success.
        if (sentMessage?.authorityRejectedBeforeProvider === true) {
          if (attempt > 1 && conversationGeneration?.outboundContextDaTienLen?.() === true) {
            await ghiDungPartialViContextTienLen(attempt - 1);
          }
          return;
        }
        outboundFailureOutcome = null;
        break;
      }
      if (clarificationAckId && i === 0) {
        await completeAdminClarificationAck(
          clarificationAckId,
          sentMessage?.id || null,
          Boolean(sentMessage?.id)
        );
        clarificationAckCompleted = true;
      }
      if (originToken && !originConHieuLuc(originToken)) {
        await ghiStaleOutboundSkipped(tin, "runtime_origin_invalid_after_send");
        return;
      }
      if (!botWorkConHieuLuc()) {
        await ghiStaleOutboundSkipped(tin, "generation_invalid_after_send");
        return;
      }
      sentBubbleCount += 1;
    ...
    if (originToken && !originConHieuLuc(originToken)) return;
    if (!botWorkConHieuLuc()) return;
    if (!adminClarificationFallback) await thuGuiSticker(tin, originToken, automaticContext);
  } catch (error) {
    if (conversationGeneration) conversationGeneration.durableFailure ||= error;
    if (originToken && !originConHieuLuc(originToken)) {
      if (aiReply) await ghiStaleOutboundSkipped(tin, "runtime_origin_invalid_on_error");
      return;
    }
    ...
    tatGoPhim();
    tatWebTyping();
    // Metadata may already exist even if tryReply throws before returning.
    clarificationAckId ||= aiMessageForReply?.__adminClarificationAckId || null;
    if (clarificationAckId && !clarificationAckCompleted && !clarificationAckHandedOff) {
      await completeAdminClarificationAck(clarificationAckId, null, false);
    }
  }
}
...
async function handleNewIncomingMessage(normalizedMsg) {
  ...
  const inboundAdmission = arguments[2] || null;
  if (!inboundAdmission?.sequenced) {
    // Chup authority va busy relationship dong bo ngay luc event vao handler,
    // truoc await dau tien. Busy snapshot giu follow-up da den trong luc AI dang
    // chay; lane theo owner + thread ngan event sau vuot event truoc o DB/IO.
    const ownerUid = originToken
      ? String(originToken.originOwnerUid || "").trim()
      : String(typeof appState === "object" ? (appState.uid || "") : "").trim();
    const threadId = String(normalizedMsg?.threadId || "").trim();
    const senderDangBanLucTinDen = Boolean(ownerUid && boGom.dangBan?.(
      ownerUid,
      threadId,
    ...
    const admission = Object.freeze({
      sequenced: true,
      ownerUid,
      senderDangBanLucTinDen,
    });
    const runSequenced = () => handleNewIncomingMessage(normalizedMsg, originToken, admission);

    // Tin invalid/tokenless van giu behavior cu, khong dua vao mot global lane.
    if (!ownerUid || !threadId) return runSequenced();

    const inboundLanes = handleNewIncomingMessage.inboundLanes
      || (handleNewIncomingMessage.inboundLanes = new Map());
    const sequencingKey = `${ownerUid}\u0000${threadId}`;
    const previous = inboundLanes.get(sequencingKey) || Promise.resolve();
    // catch chi release lane sau; Promise tra cho caller cu van reject nhu cu.
    const current = previous.catch(() => undefined).then(runSequenced);
    inboundLanes.set(sequencingKey, current);
    return current.finally(() => {
      if (inboundLanes.get(sequencingKey) === current) inboundLanes.delete(sequencingKey);
    });
  }

  const ownerLucTinDen = inboundAdmission.ownerUid;
  const senderDangBanLucTinDen = inboundAdmission.senderDangBanLucTinDen;
  ...
  if (!normalizedMsg.isSelf) {
    ...
    const adminAuthenticated = await laLenhAdmin(normalizedMsg);
    if (adminAuthenticated) {
      if (originToken && !originConHieuLuc(originToken)) return;
      const traLoi = await xuLyLenh({
        ...normalizedMsg,
        __adminClarificationOwnerUid: originToken?.originOwnerUid || chuHienTai(),
      }, guiTheoOrigin, guiRiengTheoOrigin);
      if (originToken && !originConHieuLuc(originToken)) return;
      if (traLoi) {
        await guiTheoOrigin({
          threadId: normalizedMsg.threadId,
    ...
    // Customer coalescing only after canonical persist/dedup, self/system gates,
    // private-thread check and Admin auth exclusion.
    if (Number(normalizedMsg.threadType) === Number(ThreadType.User)
      && typeof touchAdminClarificationContext === "function") {
      await touchAdminClarificationContext({
        ownerUid: originToken?.originOwnerUid || chuHienTai(),
        message: normalizedMsg,
      }).catch((error) => console.warn("[admin-clarification] Context touch failed:", error.message));
    }
    ...
    // Admission capture epoch TRUOC khi doc bot_enabled. Sau await chi so epoch,
    // khong doc lai DB: thread epoch la runtime proxy cho moi ON <-> OFF.
    const capturedBotEligibilityEpoch = botEligibilityEpoch;
    ...
    const threadEligibilityKey = khoaThreadEligibility(autoReplyOwnerUid, normalizedMsg.threadId);
    const capturedThreadEligibilityEpoch = threadEligibilityEpochHienTai(threadEligibilityKey);
    const admissionThread = await getThread(autoReplyOwnerUid, normalizedMsg.threadId);
    if (originToken && !originConHieuLuc(originToken)) return;
    if (!botEligibilityConHieuLuc(capturedBotEligibilityEpoch)) return;
    if (!threadEligibilityConHieuLuc(threadEligibilityKey, capturedThreadEligibilityEpoch)) return;
    // Null row policy = ENABLED, dong bo voi DEFAULT 1.
    if (admissionThread?.botEnabled === false) {
      await addLog({
        event: "thread_bot_off",
        level: "warn",
        summary: "Bot của hội thoại đang TẮT — không tự trả lời tin này",
        detail: {
          threadId: normalizedMsg.threadId,
          senderName: normalizedMsg.senderName,
          content: normalizedMsg.content,
    ...
    const automaticWork = Object.freeze({
      originToken,
      capturedBotEligibilityEpoch,
      threadEligibilityKey,
      capturedThreadEligibilityEpoch,
    });
    const botWorkConHieuLuc = () => automaticWorkConHieuLuc(automaticWork);
    const automaticSendOptions = () => tuyChonGuiTuDong(automaticWork);
    ...
    if (!handledByAutoReply && typeof normalizedMsg.content === "string" && normalizedMsg.content.trim()) {
      ...
      if (originToken && !originConHieuLuc(originToken)) return;
      if (!botWorkConHieuLuc()) return;
      const durableMessage = typeof ghiNhanDurableAdmission === "function"
        ? await ghiNhanDurableAdmission({ accountId: autoReplyOwnerUid, message: normalizedMsg })
        : normalizedMsg;
      if (!durableMessage) return;
      boGom.them(durableMessage, automaticWork);
    }
  }
}
...
export async function getMessagesForThread(threadId) {
  ...
  const origin = taoOriginRuntime();
  const isCurrent = () => originConHieuLuc(origin);
  const empty = { messages: [], myAvatar: null, syncScheduled: false, syncInFlight: false, syncRetryAfterMs: 0, enrichmentPending: false };
  ...
  if (!thread) return { ...empty, myAvatar: origin.originAvatar };
  const messages = await getThreadMessages(chu, threadId, 500);
  if (!isCurrent()) return empty;
  const dbMs = performance.now() - started;
  const enriched = enrichHistoryFromCache(chu, messages, thread, origin.originAvatar);
  const historyApi = appState.loggedIn ? origin.originApiIdentity : null;
  const payload = {
    ...empty,
    messages: enriched.messages,
    myAvatar: origin.originAvatar,
    enrichmentPending: Boolean(historyApi && enriched.candidates.length),
  };
  payload.syncScheduled = shouldSyncGroupHistory(historyApi, chu, threadId, thread.threadType);
  const emitUpdate = (event) => { if (isCurrent()) io?.emit("thread-history-updated", event); };
  if (payload.syncScheduled) {
    void syncHistoryForThread(historyApi, chu, threadId, thread.threadType, { isCurrent, onTerminal: emitUpdate }).catch(() => {});
  }
  const syncStatus = getGroupHistorySyncStatus(historyApi, chu, threadId, thread.threadType);
  payload.syncInFlight = syncStatus.inFlight;
  payload.syncRetryAfterMs = syncStatus.retryAfterMs;
  if (payload.enrichmentPending) {
    void warmHistoryEnrichment(historyApi, chu, threadId, enriched.candidates, { isCurrent, onUpdated: emitUpdate }).catch(() => {});
  }
  console.log("[history-load]", JSON.stringify({
    thread: createHash("sha256").update(String(chu) + String(threadId)).digest("hex").slice(0, 12),
    dbMs, count: messages.length, buildMs: performance.now() - started - dbMs,
    syncScheduled: payload.syncScheduled, syncInFlight: payload.syncInFlight,
    syncRetryAfterMs: payload.syncRetryAfterMs, enrichHit: enriched.enrichHit, enrichMiss: enriched.enrichMiss,
  }));
  return payload;
}
...
async function locTruocKhiGui(text, threadId) {
  const { sach, daCat, soDongCat, soMarkerQuyetDinhCat } = locRuotGan(text);
  if (!daCat) return sach;

  if (soMarkerQuyetDinhCat > 0) {
    await addLog({
      event: "chan_decision_marker",
      level: "warn",
      summary: `Đã cắt ${soMarkerQuyetDinhCat} marker quyết định nội bộ trước khi gửi`,
      detail: {
        threadId,
        markerClass: "VIZEN_DECISION",
        markerCount: soMarkerQuyetDinhCat,
        soDongNoiBoCat: soDongCat,
      },
    }).catch(() => {});
    return sach;
  }
...
export async function sendChatMessage({ threadId, text, threadType, quote, mentions, urgency, attachment, originToken }) {
  ...
  const capturedThreadEligibilityKey = internalOptions.threadEligibilityKey;
  const capturedThreadEligibilityEpoch = internalOptions.threadEligibilityEpoch;
  const conversationGeneration = internalOptions.conversationGeneration || null;
  const retryContextKhongDuocTienLen = internalOptions.requireUnadvancedOutboundContext === true;
  const authorityRejectedBeforeProvider = () =>
    internalOptions.reportAuthorityRejectionBeforeProvider === true
      ? SEND_REJECTED_BEFORE_PROVIDER
      : null;
  ...
  const threadEligibilityBatBuoc = typeof capturedThreadEligibilityKey === "string"
    && Number.isInteger(capturedThreadEligibilityEpoch);
  ...
  const threadWorkConHieuLuc = () => !threadEligibilityBatBuoc
    || threadEligibilityConHieuLuc(capturedThreadEligibilityKey, capturedThreadEligibilityEpoch);
  const generationConHieuLuc = () => !conversationGeneration
    || (conversationGeneration.conHieuLuc()
      && (!retryContextKhongDuocTienLen
        || conversationGeneration.outboundContextDaTienLen?.() !== true));
  const revalidateBeforeSend = () => {
    if (!botWorkConHieuLuc() || !threadWorkConHieuLuc() || !generationConHieuLuc()) return false;
  ...
  if (!revalidateBeforeSend()) return authorityRejectedBeforeProvider();
  ...
  if (!revalidateBeforeSend()) return authorityRejectedBeforeProvider();
  if (conversationGeneration && !conversationGeneration.giuChoOutbound()) {
    return authorityRejectedBeforeProvider();
  }
  ...
  try {
    ...
    const outcome = phanLoaiLoiGuiProvider(error);
    const stampedError = ganOutcomeChoLoi(error, outcome);
    if (conversationGeneration) {
      if (outcome === OUTBOUND_OUTCOME.SEND_UNKNOWN) {
        conversationGeneration.danhDauSendUnknown();
      } else {
        conversationGeneration.traLaiOutbound();
      }
    }
  ...
  // Day la canonical physical-success boundary: provider da return thanh cong,
  // nhung chua co bat ky persistence/broadcast local nao bat dau.
  internalOptions.onProviderSuccess?.({
    id: msgIdThat == null ? null : String(msgIdThat),
    providerResult: ketQuaGui,
  });
  conversationGeneration?.xacNhanOutbound();
...
/** App Context chi duoc doc ma ket noi; khong lo lyDo hay danh tinh Zalo. */
export function getConnectionState() {
  return appState.ketNoi?.trangThai;
}
...
export async function reactToMessage(
  { icon, identity, threadId, threadType },
  ...
  internalOptions?.onProviderSuccess?.({ id: null, providerResult: ketQua });
  chotQuyenRuntime(capturedAuthority);
  return ketQua;
}
