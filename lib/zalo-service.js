...
function automaticWorkConHieuLuc(automaticWork) {
  ...
  return globalHopLe && threadHopLe;
}
...
async function traLoiCumTin(tins, automaticWork = null) {
  ...
  const automaticContext = coAutomaticWork ? automaticWork : null;
  ...
  if (typeof handlePdfAutomation === "function") {
    const pdfAutomationResult = await handlePdfAutomation({
      ...
      tin,
      originToken,
      automaticWork: automaticContext,
    });
    if (originToken && !originConHieuLuc(originToken)) return;
    if (!botWorkConHieuLuc()) return;
  ...
  guiDaXemChoTins(tins, originToken, automaticContext);
  ...
  try {
    ...
    aiReply = await ownerCredentials.withCurrentOwnerCredentialRead(
      inferenceOwnerUid,
      inferenceConfig,
      () => aiChat.tryReply(tin.content, tin)
    );
    if (originToken && !originConHieuLuc(originToken)) return;
...
export async function sendChatMessage({ threadId, text, threadType, quote, mentions, urgency, attachment, originToken }) {
  ...
  const revalidateBeforeSend = () => {
    if (!botWorkConHieuLuc() || !threadWorkConHieuLuc()) return false;
