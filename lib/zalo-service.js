...
async function finalizeLogin(loggedApi) {
  ...
  await emitThreads();
  setTimeout(() => {
    syncThreadCatalog(api, chuHienTai())
      .then(emitThreads)
 ...
 */
async function thuGuiSticker(tin) {
  try {
...
async function handleNewIncomingMessage(normalizedMsg) {
  ...
  if (!normalizedMsg.isSelf) {
    ...
    if (await laLenhAdmin(normalizedMsg)) {
      const traLoi = await xuLyLenh(normalizedMsg, sendChatMessage, sendResolvedPrivateMessage);
      if (traLoi) {
        await sendChatMessage({
          threadId: normalizedMsg.threadId,
          threadType: normalizedMsg.threadType,
...
export async function sendChatMessage({ threadId, text, threadType, quote, mentions, urgency }) {
  ...
  if (!cleanText) throw new Error("Thieu cuoc chat hoac noi dung.");
  ...
  const msgIdThat = layMsgIdTuKetQuaGui(ketQuaGui);
  ...
  if (!msgIdThat) {
    ...
    await addLog({
      event: "gui_khong_co_msgid",
      level: "warn",
      summary: "Zalo khong tra msgId cho tin vua gui — cho tin doi lai de ghi",
      detail: { threadId, threadType: numericThreadType },
    }).catch(() => {});
    return null;
  }
  ...
  const daPhat = await persistAndBroadcastMessage(message);
...
export async function emitThreads() {
  io?.emit("threads", await listThreads(chuHienTai(), { recentOnly: true }));
}
