...
function durableStateError(code, message, outbox, current = null) {
  const error = new Error(message);
  error.code = code;
  error.outboxId = current?.id || outbox?.id || null;
  error.durableStatus = current?.status || null;
  return error;
}

function reconcileImmediateCaller(outbox, current, conversationGeneration) {
  if (!current) {
    throw durableStateError(
      "OUTBOX_INTENT_MISSING",
      "Durable outbox intent khong con ton tai.",
      outbox
    );
  }
  if (current.status === "SENT") {
    ...
    return current.providerMessageId
      ? { id: current.providerMessageId, durableReplay: true }
      : null;
  }
  if (["SENDING", "PENDING", "RETRY", "BLOCKED"].includes(current.status)) {
    return {
      outboundHandedOff: true,
      durableStatus: current.status,
      outboxId: current.id || outbox.id,
    };
  }
  throw durableStateError(
    "OUTBOX_GENERATION_TERMINAL",
    "Durable outbox generation da o trang thai terminal.",
    outbox,
    current
  );
}
...
export async function guiDurableOutbound({ outbox, conversationGeneration, send, nextAttemptDelayMs = 0 }) {
  ...
  let claimed = outbox;
  if (claimed.__claimedHere !== true) {
    if (claimed.status === "SENDING") {
      const current = await durableRow(outbox.id, outbox.generationKey);
      return reconcileImmediateCaller(outbox, current, conversationGeneration);
    }
