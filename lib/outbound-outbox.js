...
async function ghiNhanThatBai(outbox, error) {
  ...
  if (!transient && result.enteredBlocked && outbox.attemptCount === 0) {
...
export async function quetOutboundNgay({ max = 100 } = {}) {
  await recoverExpiredOutboundIntents();
  let handled = 0;
  while (handled < max) {
    const outbox = await claimNextOutboundIntent({ leaseMs: OUTBOX_LEASE_MS });
    if (!outbox) break;
    handled += 1;
