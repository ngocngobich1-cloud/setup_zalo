...
/** Dat owner hien tai; app.js goi truoc khi vo hieu hoa UI owner cu. */
export function setSettingsOwnerUid(nextOwnerUid) {
  settingsOwnerUid = nextOwnerUid ? String(nextOwnerUid) : null;
}

/** Xoa PDF DOM cua owner cu va lam moi generation de bo qua response den tre. */
...
export const CONFIG_TABS = [
  ...
  },
  {
    ...
    mount(panel) {
      panel.innerHTML = `
        <div style="margin-bottom:16px; color:var(--muted); font-size:14px;">
          Khi tin khách chứa từ khóa, bot hỏi xác nhận. Chỉ khi khách trả lời đúng <strong>OK</strong>, bot mới gửi PDF.
        </div>
      ...
      function renderPdfRules() {
        ...
        for (const rule of pdfRules) {
          ...
          remove.onclick = async () => {
            ...
            const data = await res.json();
            if (!settingsOwnerConHieuLuc(owner)) return;
            if (!res.ok) return showPdfStatus(data.error || "Không xoá được rule.", "var(--danger)");
            resetPdfForm();
            await loadPdfRules(owner.ownerGeneration);
          };
