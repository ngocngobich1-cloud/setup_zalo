...
const OWNER_RESTART = "pdf-owner-restart";
const SENDER_A = "pdf-sender-a";
const SENDER_B = "pdf-sender-b";
const THREAD = "customer-thread";
const GROUP_THREAD = "customer-group";
...
function makeRuntime({
  ...
  failPdf = false,
  owner = OWNER_A,
  handlerFactory = createPdfAutomationHandler,
  listRules = null,
  getRule = null,
} = {}) {
  let ownerUid = owner;
  ...
  const sends = [];
  const logs = [];
  let listEnabledRules = listRules || (async (requestedOwner) => (
    requestedOwner === ownerUid ? rules.filter((rule) => rule.enabled) : []
  ));
  const getRuleWithBlob = getRule || (async (requestedOwner, id) => (
    requestedOwner === ownerUid ? rules.find((rule) => rule.id === Number(id)) || null : null
  ));
  ...
  const run = (contents, token = origin(), senderId = SENDER_A, threadType = 0, threadId = THREAD) => {
    const tins = contents.map((content, index) => ({
      id: String(index + 1), content, threadId, threadType, senderId,
    }));
    return handler({
      tins,
...
async function trigger(
  runtime,
  text = "Cho chị xin template nhé",
  senderId = SENDER_A,
  threadType = 0,
  threadId = THREAD
) {
  assert.equal(await runtime.run([text], runtime.origin(), senderId, threadType, threadId), PDF_AUTOMATION_HANDLED);
...
async function main() {
  ...
  await test("PDF02", "keyword trigger chi gui mot confirmation va tao pending theo sender", async () => {
    const runtime = makeRuntime();
    await trigger(runtime);
    assert.equal(getPendingPdfConfirmation(OWNER_A, THREAD, SENDER_A)?.ruleId, 1);
  });

  await test("PDF03A", "single exact OK cua triggering sender gui dung mot PDF va HANDLED", async () => {
    const runtime = makeRuntime();
    await trigger(runtime);
    ...
    assert.equal(getPendingPdfConfirmation(OWNER_A, THREAD, SENDER_A), null);
  });
  ...
  await test("PDF03B", "OK + fragment trong cung batch van xac nhan ca batch", async () => {
    ...
    assert.equal(runtime.sends.filter((item) => item.attachment).length, 1);
    assert.equal(hasExactPdfConfirmation([
      { content: "OK", senderId: SENDER_A },
      { content: "dạ em", senderId: SENDER_A },
    ], SENDER_A), true);
  });
  ...
  await test("PDF04", "non-OK cua triggering sender huy pending va batch tiep tuc", async () => {
    const runtime = makeRuntime();
    await trigger(runtime);
  ...
  await test("PDF20", "group chi triggering sender duoc xac nhan pending cua minh", async () => {
    const runtime = makeRuntime();
    await trigger(runtime, "Cho em template", SENDER_A, 1, GROUP_THREAD);
    assert.equal(getPendingPdfConfirmation(OWNER_A, GROUP_THREAD, SENDER_A)?.ruleId, 1);

    assert.equal(
      await runtime.run(["OK"], runtime.origin(), SENDER_B, 1, GROUP_THREAD),
      PDF_AUTOMATION_CONTINUE
    );
    assert.equal(runtime.sends.filter((item) => item.attachment).length, 0);
    assert.equal(getPendingPdfConfirmation(OWNER_A, GROUP_THREAD, SENDER_A)?.ruleId, 1);
    assert.equal(getPendingPdfConfirmation(OWNER_A, GROUP_THREAD, SENDER_B), null);

    assert.equal(
      await runtime.run(["OK"], runtime.origin(), SENDER_A, 1, GROUP_THREAD),
      PDF_AUTOMATION_HANDLED
    );
    assert.equal(runtime.sends.filter((item) => item.attachment).length, 1);
    assert.equal(getPendingPdfConfirmation(OWNER_A, GROUP_THREAD, SENDER_A), null);
  });
  ...
  await test("PDF19", "fresh startup tao schema, init lai idempotent va giu canonical data", async () => {
    assert.deepEqual(await db.listPdfAutomationRules("fresh-owner"), []);
    await db.saveAccountConfig("fresh-owner", { botEnabled: true, allowedGroupId: "fresh-group" });
    const created = await db.insertPdfAutomationRule("fresh-owner", {
      ...preparePdfKeyword("fresh-pdf"),
      pdfName: "fresh.pdf",
      pdfMime: "application/pdf",
    ...
    const account = await db.getAccountConfig("fresh-owner");
    assert.equal(account.botEnabled, true);
    assert.equal(account.allowedGroupId, "fresh-group");
    assert.equal((await db.listPdfAutomationRules("fresh-owner")).length, 1);
    assert.deepEqual(
      Buffer.from((await db.getPdfAutomationRuleWithBlob("fresh-owner", created.id)).pdfData),
      PDF_A
    );
  });
  ...
  await test("PDF21", "restart mat pending cu nhung giu rule/BLOB va cho retrigger", async () => {
    const created = await db.insertPdfAutomationRule(OWNER_RESTART, {
      ...preparePdfKeyword("restart-pdf"),
      pdfName: "restart.pdf",
      pdfMime: "application/pdf",
      pdfSize: PDF_B.length,
      pdfData: PDF_B,
    ...
    const dbRuntimeOptions = {
      owner: OWNER_RESTART,
      listRules: (ownerUid) => db.listEnabledPdfAutomationRules(ownerUid),
      getRule: (ownerUid, id) => db.getPdfAutomationRuleWithBlob(ownerUid, id),
    };
    const oldRuntime = makeRuntime(dbRuntimeOptions);
    await trigger(oldRuntime, "Cho xin restart-pdf");
    assert.equal(getPendingPdfConfirmation(OWNER_RESTART, THREAD, SENDER_A)?.ruleId, created.id);

    const freshModuleUrl = `${pathToFileURL(path.join(REPO, "lib", "pdf-automation.js")).href}?restart=${Date.now()}`;
    const freshPdfModule = await import(freshModuleUrl);
    const freshRuntime = makeRuntime({
      ...dbRuntimeOptions,
      handlerFactory: freshPdfModule.createPdfAutomationHandler,
    });
    assert.equal(freshPdfModule.getPendingPdfConfirmation(OWNER_RESTART, THREAD, SENDER_A), null);
    assert.equal(await freshRuntime.run(["OK"]), freshPdfModule.PDF_AUTOMATION_CONTINUE);
    assert.equal(freshRuntime.sends.filter((item) => item.attachment).length, 0);

    await trigger(freshRuntime, "Cho xin restart-pdf");
    assert.equal(await freshRuntime.run(["OK"]), freshPdfModule.PDF_AUTOMATION_HANDLED);
    assert.equal(freshRuntime.sends.filter((item) => item.attachment).length, 1);
    assert.equal(freshPdfModule.getPendingPdfConfirmation(OWNER_RESTART, THREAD, SENDER_A), null);
  });
  ...
  await test("PDF12_PDF15", "canonical insertion, dedupe, UI isolation va marker locks", async () => {
    ...
    assert.match(config, /btn-ai-model-save"\)\.addEventListener[\s\S]*?\}\);\s*useKnowledge\.addEventListener/);
